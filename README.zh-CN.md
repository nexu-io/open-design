...
<p align="center">
  ...
  <a href="#内置-skills"><img alt="Skills" src="https://img.shields.io/badge/skills-134-teal?style=flat-square" /></a>
...
<td width="50%">
<img src="docs/screenshots/06-design-systems-library.png" alt="06 · 152 套 design system 库" /><br/>
<sub><b>152 套 design system 库</b> —— 每套产品系统都展示 4 色色卡。点进去看完整的 <code>DESIGN.md</code>、色板网格、live showcase。</sub>
</td>
</tr>
<tr>
<td width="50%">
...
**134 个 skill，每个一个文件夹**，都遵循 Claude Code 的 [`SKILL.md`][skill] 规范，并叠加 OD 的 `od:` frontmatter，daemon 原样解析 —— `mode`、`platform`、`scenario`、`preview.type`、`design_system.requires`、`default_for`、`featured`、`fidelity`、`speaker_notes`、`animations`、`example_prompt`（[`apps/daemon/src/skills.ts`](apps/daemon/src/skills.ts)）。
...
DISCOVERY 指令         （turn-1 表单、turn-2 品牌分支、TodoWrite、五维评审）
  ...
  + 激活的 DESIGN.md   （152 套备选）
  + 激活的 SKILL.md    （134 套备选）
...
2. 加载 134 个 skill + 152 套 design system。
...
│
├── skills/                        ← 134 个 SKILL.md skill 包（27 prototype + 4 deck）
...
<p align="center">
  <img src="docs/assets/design-systems-library.png" alt="152 套 Design Systems 库 — 编辑版式双页" width="100%" />
</p>

152 套开箱即用，每套一个 [`DESIGN.md`](design-systems/README.md)：
...
- **用户自存 templates。** 喜欢某次渲染？`POST /api/templates` 把 HTML + 元数据快照进 SQLite `templates` 表。下个项目的 picker 里多一行「你的模板」 —— 跟内置 134 套同一个挑选面，但是你的。
...
| Skill | 私有 | 12 套自定义 TS 模块 + `SKILL.md` | **134 套基于文件的 [`SKILL.md`][skill]，可丢入** |
| Design system | 私有 | `DESIGN.md`（v0.2 路线图） | **`DESIGN.md` × 152 套，开箱即有** |
...
- [x] 134 个 skill + 152 套 design system + 5 套视觉方向 + 5 个设备外壳
