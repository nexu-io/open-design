...
- Data producers publish one current-run manifest map through job outputs, not payload artifacts. Python binds the complete expected miss inventory to the pending workload digest and execution class before re-projecting consumer inputs. These manifests are not trusted cache hits. Trusted admission verifies the product namespace and bytes before result publication; it does not download/re-upload a GitHub artifact for directly published products.
