...
- Every release job and explicit step, including `uses`, has an operation-prefixed display name: `[plan/setup/prepare/build/test/validate/fetch/restore/store/sign/publish/report] object · scope`. Distinguish internal workflow artifact storage from public CDN publication. Keep job IDs and execution conditions stable when changing display names; synchronize `admission.productionJob` with the production job title. A skipped job keeps its operation name; do not introduce skip-only relay jobs for display.
- `convergence.atom.yml` is the only workload-result writer for CI and all three release entrypoints. Select the producer's explicit contract and handoff identity from trusted code, then retain the default-branch admission checks.
- Trusted publication reports read-only cache capacity before/after publication in its job summary. Inventory is bounded, workflow-scoped across policies and legacy/current reusable product keys; it is not an exact channel total. The 50 GiB reference is a review signal, never an eviction or publication gate. Partial scans are lower bounds, failures are unavailable (not zero), and observed growth includes concurrent writers. Version-distribution objects are excluded and no capacity observation deletes data.
- Maintainer-authorized bootstrap for this delivery task: reusable-product upload steps may reuse the existing single plan bucket credential. The Python producer interface derives content-addressed product keys and only performs create-only product writes; it never exposes a caller-selected key, deletion, or result publication operation. Inject credentials only into the upload step. This is an application-level guard against mistakes, **not credential-level isolation**; do not describe it as prefix-restricted credentials. Stronger credential isolation is deferred. Version-bound release objects remain excluded from this bucket.
