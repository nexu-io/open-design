#!/usr/bin/env bash
# Marketing AX — One-Click Installer
...
# ---------------------------------------------------------------------------
DEFAULT_PORT=7456
DEFAULT_IMAGE="docker.io/vanjayak/marketing-ax:latest"
...
# ---------------------------------------------------------------------------
# 2b. Check if Marketing AX is already running
# ---------------------------------------------------------------------------
if $CONTAINER_CMD ps --filter "name=marketing-ax" --format '{{.Names}}' 2>/dev/null | grep -q 'marketing-ax'; then
  STATUS="$($CONTAINER_CMD inspect --format '{{.State.Status}}' marketing-ax 2>/dev/null || echo 'unknown')"
  IMAGE="$($CONTAINER_CMD inspect --format '{{.Config.Image}}' marketing-ax 2>/dev/null || echo 'unknown')"
  PORTS="$($CONTAINER_CMD port marketing-ax 2>/dev/null || echo 'unknown')"

  error "Marketing AX is already running."
  printf "\n"
  printf "  Container:  marketing-ax\n"
  printf "  Status:     %s\n" "$STATUS"
  printf "  Image:      %s\n" "$IMAGE"
  printf "  Ports:      %s\n" "$PORTS"
  printf "\n"
...
MARKETING_AX_IMAGE=${IMAGE}
MARKETING_AX_PORT=${PORT}
MARKETING_AX_ALLOWED_ORIGINS=${ALLOWED_ORIGINS}
MARKETING_AX_MEM_LIMIT=${MEM_LIMIT}
NODE_OPTIONS=--max-old-space-size=192
MAX_API_TOKEN=${GENERATED_TOKEN}
ENVFILE
...
if [ "$OS" = "Linux" ] && [ "$OPT_NO_SYSTEMD" = "0" ]; then
  if command -v systemctl >/dev/null 2>&1; then
    ...
    SYSTEMD_UNIT="${SYSTEMD_DIR}/marketing-ax.service"
    ...
    {
      echo "[Unit]"
      echo "Description=Marketing AX daemon (${CONTAINER_RUNTIME} Compose)"
    ...
    systemctl --user enable marketing-ax 2>/dev/null || true
    ok "systemd unit installed: marketing-ax.service"
    step "Manage with: systemctl --user start|stop|status marketing-ax"
  else
...
printf "  Config:       %s\n" "$ENV_FILE"
if [ "$OS" = "Linux" ] && [ -f "${HOME}/.config/systemd/user/marketing-ax.service" ]; then
  printf "  Service:      systemd (marketing-ax.service)\n"
fi
printf "\n"
printf "  Next steps:\n"
