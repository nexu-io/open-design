...
- [docs] Manual Edit direct-manipulation spec 协议摘要与实现对齐: `od-edit-paste-image` 字段从 `{id, file}` 更正为 `{id, name, mime, buffer}`,`od-edit-apply-dom` op 列表补全 `insert-at-index` 与 `apply-content`。(#6141)
