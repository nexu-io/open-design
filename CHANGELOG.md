...
- [fix] design system picker 与 library 内长分类标题/卡片名截断一致性修复（左侧行单行省略，详情面板保留完整标签），同时清理 picker renderer 不再使用的 `.project-ds-picker-option-cat/-summary/-swatches/-swatch` 死规则。
