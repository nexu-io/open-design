...
- **Agent install bootstrap is now in the repo.** `install.sh` is the shell entry point referenced by the README's one-liner (`curl -fsSL https://open-design.ai/install.sh | bash -s -- <agent>`). Until the site mirrors it, fetch the canonical copy from the repo with `curl -fsSL https://raw.githubusercontent.com/nexu-io/open-design/main/install.sh | bash -s -- <agent>`. Supports all 14 agents in `apps/daemon/src/mcp-agent-install.ts` (`AGENT_SLUGS`): claude, codex, cursor, copilot, openclaw, antigravity, gemini, pi, vibe, hermes, cline, kimi, trae, opencode. The bootstrap mirrors the supported agent roster and the basic MCP server shape from the daemon's `mcp-agent-install.ts`; for the full payload (`OD_DATA_DIR`, sidecar env, `ELECTRON_RUN_AS_NODE`), use `od mcp install <agent>` once the daemon is installed. Adds a vitest-style test under `scripts/install.sh.test.ts` that runs in `pnpm guard`; the first assertion (`install.sh exists at the repo root`) goes red on `main` and green on this branch, satisfying the bug-fix verification bar.

### Changed

- **install.sh safety pass.** The bootstrap now refuses to (a) take a `--daemon-url` with no value or with a value starting with `-`, (b) take a `--daemon-url` containing quotes, backslashes, whitespace, or control characters (strict RFC 3986 allowlist, validated once up-front so no downstream consumer can be tricked), (c) silently overwrite a non-object at the dot-path inside an existing JSON config (refuses with exit 2, leaves the file untouched, the same way it already refused malformed JSON), (d) rewrite an unchanged config file (skips the write if the merged content matches existing, so agents that watch config files don't reload on no-op runs), or (e) hit the network during dry-run (the daemon health check is now strictly write-mode-only — dry-run is a no-side-effect contract). The merge helper's tmp file is now `${path}.install-sh.${pid}.${uuid}.tmp` (was just `${path}.install-sh.tmp`), and it preserves the existing file's mode. `--write-config` is now an explicit no-op warning for CLI agents instead of a silent lie. The script is also self-contained for `curl | bash` invocations: when `scripts/install-sh-merge.mjs` is not on disk next to the script (the typical curl-pipe path), install.sh inlines the exact same merge algorithm via heredoc to `node --input-type=module`. The heredoc MUST stay byte-equivalent to `scripts/install-sh-merge.mjs`; both paths are exercised by the test suite. README one-liner changed from `| sh -s` to `| bash -s --` because dash (Debian/Ubuntu `/bin/sh`) fails on bash-only syntax before doing anything useful. `--non-interactive` flag removed (parsed but never acted on).

- **install.sh safety pass (v2).** Hardened the bootstrap further after the first-pass review. (a) The URL regex now requires a non-empty host (`http:///path` and `http://?x` are rejected). (b) CLI snippets shell-quote the daemon URL with `printf '%q'`, so a URL like `http://host:7456/;id` prints as `'http://host:7456/;id'` and is safe to copy-paste into a shell (no `;id` command-injection footgun). (c) The `OD_INSTALL_SKIP_HEALTH_CHECK=1` env var short-circuits the daemon health check; the test suite sets it so DNS / network stubs cannot make the tests flaky. (d) The Cline / Trae Windows branch no longer claims to support Git Bash / MSYS via `$APPDATA` (it doesn't — the platform detection maps MINGW/CYGWIN to `other`); the message now points users at `od mcp install cline` / `od mcp install trae` for Windows. (e) Wording softened throughout: the bootstrap "mirrors the supported agent roster and basic MCP server shape" from `mcp-agent-install.ts`, not "matches it exactly" — for the full payload (env vars, sidecar, `OD_DATA_DIR`) users still need `od mcp install <agent>`. The heredoc fallback inside install.sh is now described as "behavior-equivalent" to `scripts/install-sh-merge.mjs` (the two files have different headers / comments and were never byte-equivalent); the test suite exercises both paths. README "16+ CLIs supported" → "14 agents supported".

- **Cursor planner parity (v3).** The bootstrap was writing the generic `ENTRY_JSON` for Cursor, which omits the `type: "stdio"` field the daemon's `planAgentInstall('cursor', ...)` adds via `jsonEntry(spec, { type: 'stdio' })`. Users running the bootstrap first and `od mcp install cursor` later would see a different `~/.cursor/mcp.json` shape depending on which installer they used. The cursor branch now writes a Cursor-specific entry whose **shape** matches what the daemon's own test (`apps/daemon/tests/mcp-agent-install.test.ts`, "cursor merges a stdio entry under mcpServers") asserts — the daemon test is the source of truth for that shape. The Cursor dry-run and `--write-config` tests now `assert.deepEqual` the full entry (not just the fields we care about) so this specific parity field cannot drift again. The same fix would apply to any future per-agent extras the daemon adds; the cursor test is the template for that.
