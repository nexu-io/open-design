...
> **Open Design 是 [Claude Design][cd] 的開源、本地優先替代品。** 可部署到 Vercel、每一層都 BYOK —— **16 套 coding-agent CLI** 在 `PATH` 上自動檢測（Claude Code, Codex, Devin for Terminal, Cursor Agent, Gemini CLI, OpenCode, Qwen, Qoder CLI, GitHub Copilot CLI, Hermes, Kimi, Pi, Kiro, Kilo, Mistral Vibe, DeepSeek TUI）就是設計引擎，由 **134 個可組合 Skills** 和 **152 套品牌級 Design System** 驅動。一個都沒裝？還有 OpenAI 相容的 BYOK 代理 `/api/proxy/stream` 備援，同一條 loop，少一次 spawn 而已。
...
<p align="center">
  ...
  <a href="#design-system"><img alt="Design systems" src="https://img.shields.io/badge/design%20systems-152-orange?style=flat-square" /></a>
  <a href="#內建-skills"><img alt="Skills" src="https://img.shields.io/badge/skills-134-teal?style=flat-square" /></a>
...
<td width="50%">
<img src="docs/screenshots/06-design-systems-library.png" alt="06 · 152 套 design system 庫" /><br/>
<sub><b>152 套 design system 庫</b> —— 每套產品系統都展示 4 色色卡。點進去看完整的 <code>DESIGN.md</code>、色票網格、live showcase。</sub>
</td>
</tr>
<tr>
<td width="50%">
...
**134 個 skill，每個一個資料夾**，都遵循 Claude Code 的 [`SKILL.md`][skill] 規範，併疊加 OD 的 `od:` frontmatter，daemon 原樣解析 —— `mode`、`platform`、`scenario`、`preview.type`、`design_system.requires`、`default_for`、`featured`、`fidelity`、`speaker_notes`、`animations`、`example_prompt`（[`apps/daemon/src/skills.ts`](apps/daemon/src/skills.ts)）。
...
DISCOVERY 指令         （turn-1 表單、turn-2 品牌分支、TodoWrite、五維評審）
  ...
  + 啟用的 DESIGN.md   （152 套備選）
  + 啟用的 SKILL.md    （134 套備選）
...
2. 載入 134 個 skill + 152 套 design system。
...
│
├── skills/                        ← 134 個 SKILL.md skill 包（27 prototype + 4 deck）
...
│
├── design-systems/                ← 152 套 DESIGN.md
...
<p align="center">
  <img src="docs/assets/design-systems-library.png" alt="152 套 Design Systems 庫 — 編輯版式雙頁" width="100%" />
</p>

152 套開箱即用，每套一個 [`DESIGN.md`](design-systems/README.md)：
...
- **使用者自存 templates。** 喜歡某次渲染？`POST /api/templates` 把 HTML + 後設資料快照進 SQLite `templates` 表。下個專案的 picker 裡多一行「你的模板」 —— 跟內建 134 套同一個挑選面，但是你的。
...
| Skill | 私有 | 12 套自定義 TS 模組 + `SKILL.md` | **134 套基於檔案的 [`SKILL.md`][skill]，可丟入** |
| Design system | 私有 | `DESIGN.md`（v0.2 路線圖） | **`DESIGN.md` × 152 套，開箱即有** |
...
- [x] 134 個 skill + 152 套 design system + 5 套視覺方向 + 5 個裝置外殼
