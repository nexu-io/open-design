...
> naver-blog(§11)와 같은 구조: 채널 기술(카드 규격·이미지 파이프라인·13룰)은
> `craft/instagram-cardnews.md`, 여기는 보험·브랜드 사실만. §11 정본을 준용하는
...
- **짧은 문장 우선** [필수]: 카드 위 텍스트는 문단 대신 1~3줄 조각 — 본문 한 줄 22자 안팎.
  블로그식 만연체 금지. 조사·접속사를 줄이고 명사형 종결 허용.
...
### 12.2 카테고리·CTA — §11.2 준용

카테고리 체계·독자의도·CTA 우선순위는 §11.2 표를 그대로 쓴다. 카드뉴스에서 CTA는
**마지막 CTA 카드 1장 + 캡션 말미로 한정** — 본문 카드 중간 CTA 금지. CTA 카드의
계정 핸들은 `@bodoc_official`.

### 12.3 면책 — §11.3 축약형 (카드용)

CTA 카드 서브 영역 또는 캡션 ⚠️ 블록에 §11.3 정본의 카드용 축약 1문장:

> ※ 일반 정보 제공 목적이며 개별 보장 여부는 약관·상품에 따라 다릅니다.

캡션에는 §11.3 정본 2문장 전체 사용을 권장(캡션은 공간 제약 없음). 운전자 소재는
§11.2의 법률자문 면책 의무가 동일하게 더해진다.

### 12.4 금지 표현·핵심 용어 — §11.5·§11.6 준용

4종 외 기능 언급 금지·경쟁 서비스명 금지(§11.5), 숨은보험금 ≠ 놓친보험금 혼용 절대
금지(§11.6)를 카드 텍스트·캡션·해시태그 전부에 동일 적용한다.

### 12.5 표지 훅 공식

표지 훅은 반드시 아래 3계열 중 하나로 (§11.8 후킹 앵글과 동일 분류):

- **오해**: "OO면 보험금 못 받는다? 절반만 맞아요"
- **손해회피**: "이거 모르면 보험금 OO만 원 놓칩니다"
- **시의성**: "2026년부터 바뀐 OO, 청구 전 확인하세요"

### 12.6 비주얼 무드 (배경 생성 프롬프트용)

- **팔레트**: §0 토큰 기준 — 시그니처 `#16C5FF` 포인트 + 딥네이비·차분한 저채도 배경.
  무지개·고채도 다색 금지(§9 AI slop 기준 동일 적용).
- **무드 키워드**: 차분한·신뢰감 있는 소프트 플랫 일러스트풍, 보험/금융 안심감.
  과장된 3D·네온·사이버 금지.
