...
| **T-LONG kanji compound auto-split (Round 16 追加)** | T-LONG の大字号 auto-wrap (~12-15 cells/行) が **2-kanji 複合語** を行間で切る — 漢字 1 字で行末 + 次行頭が連続漢字 | R16 P19 「v3 初日、team は『こんな雑な根 / **拠** で…」← 「根拠」compound split。同類: 全 / 員、関 / 心、戦 / 略。lone-particle と同様 prompt-rules.md §4 の compound 不破則違反だが、agent の 3 round 自検が漏らした。検出時は (a) quote 圧縮 ≤80 cells / (b) 意味境界が複合語間に来ないよう書き直し / (c) 必要なら clause 境界に明示 \\n（line_balance_rule_round_14 の ≥10 cells を守る） |
