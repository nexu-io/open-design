...
it("acquires fixture bytes through the download primitive and disposes only its own stage", async () => {
  ...
  await writeFile(capsule.contentFile, JSON.stringify({ schemaVersion: 1, protocol: "electron-capsule-v5", target: "darwin-arm64", entrypoint: "capsule.cjs",
  ...
  try {
    ...
    await withStandaloneExactFixture(input, async files => {
      ...
      expect(manifest.document).toMatchObject({ protocol: "electron-capsule-v5", provides: { shellVersion: "0.1.0" } });
