...
export function formatUnsupportedNodeRuntimeMessage(runtime: NodeRuntimeDiagnosticInput = currentNodeRuntime()): string {
  return [
    `tools-dev must run with ${SUPPORTED_NODE_RANGE} before starting Marketing AX.`,
    ...
    "  corepack pnpm --filter @marketing-ax/daemon rebuild better-sqlite3 --pending",
  ].join("\n");
}
...
function formatNativeAddonAbiMismatchRecommendation(runtime: NodeRuntimeDiagnosticInput): string {
  const base = [
    "Marketing AX's dev stack must run with Node ~24.",
