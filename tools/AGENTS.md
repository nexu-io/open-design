...
- `tools/dev` provides `@marketing-ax/tools-dev` and the `tools-dev` bin. It is the only currently active local development lifecycle control plane.
...
- `tools/pack` provides `@marketing-ax/tools-pack` and the `tools-pack` bin. The active slice is packaged artifact build/install/start/stop/logs/uninstall/cleanup/list/reset plus beta release artifact preparation for mac and Windows lanes, plus a Linux AppImage lane with optional containerized builds.
- `tools/serve` provides `@marketing-ax/tools-serve` and the `tools-serve` bin. It owns local fixture services such as `tools-serve start updater`.

## Retired tools

- `tools/pr` / `@open-design/tools-pr` / `pnpm tools-pr` has been retired from this repository. Maintainer PR-duty workflows now live outside the product workspace in `PerishCode/duty`; do not restore a Marketing AX-local PR-duty tool without a new explicit maintainer decision.
...
- Orchestration layers must consume primitives from `@marketing-ax/sidecar-proto`, `@marketing-ax/sidecar`, and `@marketing-ax/platform`.
...
```bash
pnpm --filter @marketing-ax/tools-dev typecheck
