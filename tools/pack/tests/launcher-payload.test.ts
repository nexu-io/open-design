...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "M-AX-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "M-AX-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "M-AX Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "M-AX Beta", "M-AX.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "M-AX-release-beta-win-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "M-AX-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "M-AX-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "M-AX Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "M-AX.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "M-AX.exe"), "fake executable\n", "utf8");
  await writeFile(
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "M-AX.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("M-AX Beta.app");
      expect(manifest.entry.executable).toBe("payload/M-AX Beta.app/Contents/MacOS/M-AX Beta");
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/M-AX.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "M-AX.exe"));
