...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "M-AX-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "M-AX-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "M-AX"),
    installedExePath: join(namespaceRoot, "runtime", "install", "M-AX", "M-AX.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "M-AX-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "M-AX-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "M-AX-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "M-AX", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "M-AX.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "M-AX.exe"))).resolves.toMatchObject({
