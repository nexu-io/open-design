    ...
    `${JSON.stringify(
      {
        ...
        pruneRootNext: true,
        pruneRootSharp: true,
        macAdhocBundleSign: config.signMode === "unsigned",
...
export async function runElectronBuilder(
  ...
  const builderConfig = {
    ...
    afterSign: config.signMode === "notarized" ? macResources.notarizeHook : undefined,
    ...
    mac: {
      ...
      icon: macResources.icon,
      identity: config.signMode !== "unsigned" ? undefined : null,
      notarize: config.signMode === "notarized" ? undefined : false,
      target: targets,
    },
  ...
  await execFileAsync(process.execPath, [
    ...
    env: {
      ...process.env,
      ...(config.signMode !== "unsigned" ? {} : { CSC_IDENTITY_AUTO_DISCOVERY: "false" }),
