...
- Consuming sidecar/process/path primitives from `@open-design/sidecar` and `@open-design/platform`.
...
- The layered manifest producer seals canonical metadata only. Cold start requires `launcher`, native pack, and body for one target; the Shell supplies official Node as its bootstrap dependency. Resources stay lazy and content-addressed. Do not flatten those groups back into one eager download list.
...
- `OD_UPDATE_CURRENT_VERSION` may override the packaged version for tests, but user-flow package validation should prefer building the package with the intended `--release-version`.
...
```bash
pnpm tools-pack win build --dir C:\odtp-beta-release-fixed --namespace release-beta-win --to nsis --release-version 0.8.0-beta.5 --json
```
