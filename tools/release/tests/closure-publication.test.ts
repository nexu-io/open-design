...
async function writeFixture(root: string): Promise<{
  ...
  const closureBase = `${assetBase}-closure`;
  ...
  const archiveUrl = `https://releases.open-design.test/beta/versions/${version}/${closureBase}.zip`;
  ...
  return { assetsRoot, manifestRoot, metadataRoot };
}
