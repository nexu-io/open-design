...
import { projectReleaseTopology } from "../src/exact/topology.ts";
...
async function fixture(hot = false) {
  ...
  const active = ["terminal", "electron"].map(shell => ({ shell, target: "darwin-arm64", workload: `${shell}_scene`, runner_class: `${shell}_mac`, runs_on: "macos-15" }));
  const deferred = [{ shell: "electron", target: "win32-x64", workload: "electron_win", runner_class: "electron_win", runs_on: "windows-2025" }];
