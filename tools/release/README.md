...
| `beta-s` | `release-beta-s` / `betas` | Self-hosted or internal-network beta lane | Build through the self-hosted/internal-network lane | Independent beta-s lane; not a stable gate | Do not inherit stable artifact policy | Nexu S3 / `betas` metadata and platform artifacts | Source-build only; no packaged lane | Self-hosted constraints and default-parameter validation |
