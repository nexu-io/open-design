...
const metadata = (metadataPath.length > 0
  ...
  control?: { shell?: { installation?: { version?: { min?: string; url?: string } } } };
  readyTargets?: string[];
  releaseState?: string;
  r2?: { versionMetadataUrl?: string };
};

const installationVersionMin = metadata.control?.shell?.installation?.version?.min;
writeText(summaryPath, [
  ...
  ...(installationVersionMin == null
    ? []
    : [`- installation version floor: \`${installationVersionMin}\` (forces installer reinstall below this outer version)`]),
