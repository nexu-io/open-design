...
async function electronProof(input: JsonObject, published: JsonObject, required: JsonObject): Promise<JsonObject> {
  ...
  if (installation.schemaVersion !== 4 || installation.channel !== published.channel || installation.target !== required.target
  ...
  if (Object.hasOwn(installation, "seeds")) throw new Error("installed Electron retains retired seeds");
