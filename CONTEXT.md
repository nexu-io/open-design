...
Marketing AX is a local-first design workspace where projects contain generated design files and agent conversations. This glossary records domain language only, not implementation details.
...
**Active Project**:
The project the user most recently interacted with in the Marketing AX UI and that MCP tools may use when no project is specified.
...
**AMR Cloud**:
The user-facing cloud runtime option for Marketing AX's official model router, shown in onboarding and login-oriented product surfaces.
...
The separately owned release contract that provides the native AMR CLI builds Marketing AX can package.
_Avoid_: Marketing AX release channel, package build step, source checkout
...
_Avoid_: Marketing AX supported platforms, release channel, future platform promise
...
- The **AMR CLI Distribution Contract** is owned separately from Marketing AX; Marketing AX release packaging consumes it instead of defining the native CLI release itself.
