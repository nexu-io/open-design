...
- `packages/sidecar-proto`: Marketing AX sidecar business protocol. Owns app/mode/source constants, namespace validation, stamp descriptor/fields/flags, IPC message schema, status shapes, error semantics, and default product path constants.
- `packages/sidecar`: generic sidecar runtime primitives. Includes bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime file helpers; it must not hard-code Marketing AX app keys or IPC business messages.
...
- Do not hard-code Marketing AX app/source/mode constants in `sidecar` or `platform`.
- Keep stamp fields limited to five: `app`, `mode`, `namespace`, `ipc`, and `source`.

## Common package commands

```bash
pnpm --filter @marketing-ax/contracts typecheck
pnpm --filter @marketing-ax/host typecheck
pnpm --filter @marketing-ax/host test
pnpm --filter @marketing-ax/metatool typecheck
pnpm --filter @marketing-ax/metatool test
pnpm --filter @marketing-ax/sidecar-proto typecheck
pnpm --filter @marketing-ax/sidecar-proto test
pnpm --filter @marketing-ax/sidecar typecheck
pnpm --filter @marketing-ax/sidecar test
pnpm --filter @marketing-ax/platform typecheck
pnpm --filter @marketing-ax/platform test
```
