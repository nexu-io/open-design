...
describe("canonical content identity", () => {
  it("is deterministic and reacts to file, mode, symlink, and parameter changes", async () => {
    ...
    const input = { id: "fixture.build", parameters: { target: "darwin-arm64" }, root, schemaVersion: 1, sources: [{ path: "src" }] } as const;
    ...
    expect((await resolveContentIdentity(input)).digest).not.toBe(initial.digest);
    expect((await resolveContentIdentity({ ...input, parameters: { target: "darwin-x64" } })).digest).not.toBe(initial.digest);
  });

  it("supports exclusions, line ending normalization, and package version projection", async () => {
    const root = await fixture();
  ...
  it("rejects missing, escaping, and overlapping declarations", async () => {
    const root = await fixture();
    await expect(resolveContentIdentity({ id: "fixture", root, schemaVersion: 1, sources: [{ path: "missing" }] })).rejects.toThrow();
    await expect(resolveContentIdentity({ id: "fixture", root, schemaVersion: 1, sources: [{ path: "../outside" }] })).rejects.toThrow(/inside/u);
    await expect(resolveContentIdentity({ id: "fixture", root, schemaVersion: 1, sources: [{ path: "src" }, { path: "src/entry.ts" }] })).rejects.toThrow(/duplicate/u);
  });
});
...
describe("content identity declarations", () => {
  ...
  it("rejects unknown fields, cycles, and malformed source options", () => {
    expect(() => parseContentIdentityRegistry({ ...registry, typo: true })).toThrow(/unknown fields/u);
    expect(() => parseContentIdentityRegistry({ ...registry, sourceSets: { fixture: { paths: [{ normalizeTextLineEndings: "yes", path: "src" }] } } })).toThrow(/boolean/u);
    const cyclic = parseContentIdentityRegistry({ ...registry, sourceSets: { fixture: { inherits: ["fixture"], paths: ["src"] } } });
    expect(() => resolveContentIdentityDeclaration(cyclic, "fixture.build")).toThrow(/cycle/u);
  });

  it("projects source-set normalization into every declared source", () => {
    const parsed = parseContentIdentityRegistry({ ...registry, sourceSets: { fixture: { normalizePackageVersion: true, paths: ["package.json"] } } });
