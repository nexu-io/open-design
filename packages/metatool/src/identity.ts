...
export function canonicalizeMetadata(value: unknown): unknown {
  ...
  if (value != null && typeof value === "object") {
    ...
    if (prototype !== Object.prototype && prototype !== null) throw new Error("canonical metadata can contain only plain objects");
...
function assertRelativeRepoPath(value: string, label: string): string {
  ...
  if (normalized.length === 0 || isAbsolute(value) || normalized === ".." || normalized.startsWith("../")) {
...
async function normalizedFileBody(path: string, source: ContentIdentitySource): Promise<Buffer> {
  ...
  if (source.normalizePackageVersion === true && normalizePath(path).endsWith("/package.json")) {
    try {
      ...
      // The owning build reports malformed JSON; identity remains byte-exact.
    }
  }
...
async function inspectSource(root: string, source: ContentIdentitySource): Promise<ContentIdentityEntry[]> {
  ...
  const relativeSource = relative(resolve(root), absoluteSource);
...
export async function resolveContentIdentity(input: ContentIdentityInput): Promise<ContentIdentityResult> {
  ...
  if (!Number.isSafeInteger(input.schemaVersion) || input.schemaVersion < 1) throw new Error("content identity schemaVersion must be a positive integer");
  ...
  const digest = metadataDigest(canonicalMetadataJson({ entries, formatVersion: CONTENT_IDENTITY_FORMAT_VERSION, id: input.id, parameters, schemaVersion: input.schemaVersion, sources }));
  return Object.freeze({ digest, entries: Object.freeze(entries), formatVersion: CONTENT_IDENTITY_FORMAT_VERSION, id: input.id, parameters, schemaVersion: input.schemaVersion, sources });
}
