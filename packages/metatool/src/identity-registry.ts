...
import type { ContentIdentitySource } from "./identity.js";
...
function strings(value: unknown, label: string): string[] {
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== "string" || entry.length === 0)) throw new Error(`${label} must be an array of non-empty strings`);
...
function sourceDeclaration(value: unknown, label: string): ContentIdentitySource {
  ...
  assertKeys(source, ["excludeDirectoryNames", "excludePaths", "normalizePackageVersion", "normalizeTextLineEndings", "path"], label);
  ...
  return {
    ...
    ...(source.normalizePackageVersion == null ? {} : { normalizePackageVersion: optionalBoolean(source.normalizePackageVersion, `${label}.normalizePackageVersion`)! }),
    ...(source.normalizeTextLineEndings == null ? {} : { normalizeTextLineEndings: optionalBoolean(source.normalizeTextLineEndings, `${label}.normalizeTextLineEndings`)! }),
    path: source.path,
  };
}
...
export function parseContentIdentityRegistry(value: unknown): ContentIdentityRegistry {
  ...
  const defaults = { excludeDirectoryNames: defaultsValue.excludeDirectoryNames == null ? [] : strings(defaultsValue.excludeDirectoryNames, "identity defaults.excludeDirectoryNames") };
  ...
  const sourceSets = Object.fromEntries(Object.entries(sourceSetsValue).map(([id, raw]) => {
    ...
    return [id, {
      ...
      paths: sourceSet.paths.map((path, index) => typeof path === "string" ? path : sourceDeclaration(path, `identity sourceSet ${id}.paths[${index}]`)),
    }];
  }));
  ...
  const identities = Object.fromEntries(Object.entries(identitiesValue).map(([id, raw]) => {
    ...
    const declaration = record(raw, `identity ${id}`);
    assertKeys(declaration, ["parameters", "schemaVersion", "sourceSets"], `identity ${id}`);
    if (!Number.isSafeInteger(declaration.schemaVersion) || (declaration.schemaVersion as number) < 1) throw new Error(`identity ${id}.schemaVersion must be a positive integer`);
    return [id, { parameters: strings(declaration.parameters, `identity ${id}.parameters`), schemaVersion: declaration.schemaVersion as number, sourceSets: strings(declaration.sourceSets, `identity ${id}.sourceSets`) }];
  }));
  return Object.freeze({ defaults: Object.freeze(defaults), identities: Object.freeze(identities), schemaVersion: 1, sourceSets: Object.freeze(sourceSets) });
}
...
export function resolveContentIdentityDeclaration(registry: ContentIdentityRegistry, id: string): Readonly<{
  declaration: ContentIdentityDeclaration;
  id: string;
  ...
  const addSourceSet = (sourceSetId: string): void => {
    if (visited.has(sourceSetId)) return;
    if (visiting.has(sourceSetId)) throw new Error(`content identity sourceSet cycle at ${sourceSetId}`);
    const sourceSet = registry.sourceSets[sourceSetId];
    if (sourceSet == null) throw new Error(`unknown content identity sourceSet: ${sourceSetId}`);
    ...
    for (const raw of sourceSet.paths) {
      ...
      if (sources.has(source.path)) throw new Error(`content identity ${id} contains duplicate source: ${source.path}`);
      sources.set(source.path, Object.freeze({
        ...(registry.defaults?.excludeDirectoryNames == null ? {} : { excludeDirectoryNames: registry.defaults.excludeDirectoryNames }),
        ...source,
        ...(source.normalizePackageVersion == null && sourceSet.normalizePackageVersion === true ? { normalizePackageVersion: true } : {}),
        ...(source.normalizeTextLineEndings == null && sourceSet.normalizeTextLineEndings === true ? { normalizeTextLineEndings: true } : {}),
      }));
    }
  ...
  return Object.freeze({ declaration, id, sources: Object.freeze([...sources.values()].sort((left, right) => left.path.localeCompare(right.path))) });
}
