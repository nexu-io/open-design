...
export function sweptBox(part: SolvedPart):
  ...
  return { min, max, spinGrew, bobRise, bobDip, screwRise, exact };
}
...
export function motionEnvelopeIssues(
  ...
  for (const mover of movers) {
    ...
    for (const other of solved.parts) {
      ...
      const cyclePen = penetration(env.min, env.max, oMin, oMax);
      ...
      issues.push({
        ...
        hint:
          "the envelope is conservative for round shapes (a symmetric part spinning about its own axis sweeps nothing) — widen the gap, shrink the motion, or ignore this if the graze is intentional",
