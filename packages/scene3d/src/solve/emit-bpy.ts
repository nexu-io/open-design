...
export function emitBlenderScript(scene: SolvedScene, options: EmitOptions = {}): string {
  ...
  const lines: string[] = [
    ...
    "",
    "",
    "def _canonical_face_order(obj):",
    '    """Force a deterministic face SEQUENCE on a freshly created primitive.',
    "",
    "    `primitive_uv_sphere_add` returns a stable VERTEX list and an",
    "    UNSTABLE face order: three runs of the same call produce three",
    "    different face sequences (measured — cylinder, cone, torus and cube",
    "    are all stable, the UV sphere alone is not). That order reaches the",
    "    exported USD, and from the master into every lowered container, so a",
    "    scene containing one sphere shipped five byte-different deliverables",
    "    per compile while the compiler reported no issue at all.",
    "",
    "    REORDERS the existing bmesh rather than rebuilding one from raw",
    "    positions — the first version of this fix did the latter and silently",
    "    dropped every per-loop custom data layer (the primitive's own UV",
    "    unwrap included), which is a worse defect than the one it fixed. bmesh",
    "    only reorders the face SEQUENCE (each BMFace keeps the loop customdata",
    "    already attached to it), so nothing downstream of this call can tell",
    "    the difference except the byte order of the export.",
    "",
    "    The canonical key — each face's vertex-index loop rotated to start at",
    "    its lowest index — is computed in Python and used ONLY to rank the",
    "    faces; it is never written back onto the mesh. bmesh's own sort takes",
    "    a purely numeric key, so the rank becomes each face's index and the",
    "    sort that matters is the default (by index) pass with no key at all.",
    "    Rotating the loop ON the mesh, rather than just in the sort key, would",
    "    shear the UV unwrap: the vertex loop and its per-loop UVs must rotate",
    "    together or not at all, and only the vertex loop can be read here",
    "    without touching the UV layer this exists to preserve.",
    '    """',
    "    import bmesh",
    "    me = obj.data",
    "    bm = bmesh.new()",
    ...
    "",
    "    def _canonical_loop(face):",
    "        idx = [v.index for v in face.verts]",
    "        at = idx.index(min(idx))",
    "        return tuple(idx[at:] + idx[:at])",
    "",
    "    for _rank, _face in enumerate(sorted(bm.faces, key=_canonical_loop)):",
    "        _face.index = _rank",
    "    bm.faces.sort()  # no key: orders by the indices just assigned",
    "    bm.to_mesh(me)",
    "    bm.free()",
    "    me.update()",
    "",
    ...
    "    obj.name = name",
    "    # The UV-sphere generator is the one primitive whose face order varies",
    "    # run to run; canonicalise it before anything downstream reads the",
    "    # mesh, so the census, the stage and every lowered container see one",
    "    # order. Cheap (a rebuild of a few hundred faces) and confined to the",
    "    # two shapes that need it.",
    '    if shape in ("sphere", "capsule"):',
    "        _canonical_face_order(obj)",
    "    rot = _AXIS_ROT[axis]",
  ...
  if (options.camera !== false) {
    ...
    lines.push(
      ...
      'cam.name = "cam_hero"',
      // The compiler's own rig, same marker ensure_staging uses for a bare
      // asset's camera — so the master's purpose=guide authoring and the
      // pre-lowering staging-object removal both see it, regardless of
      // which of the two code paths actually created the camera.
