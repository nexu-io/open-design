...
materialized immutable candidates and persists exactly one committed
release-to-Standalone binding per `<channel, namespace>`.

It does not download or extract archives, select release updates, launch
processes, expose Desktop IPC, retain launch attempts/history, or reuse Desktop
launcher state. Persisted state must not contain ports or other transient
transport identity.
