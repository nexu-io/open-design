...
export type StampedProcessMatchCriteria = Partial;
...
if (!trimmed) return [];
if (trimmed === "") return ["", "localhost", "127.0.0.1", "[::1]", ".local"];
if (trimmed === "::1") return ["[::1]"];
if (trimmed.startsWith("\*.")) return [`.${trimmed.slice(2)}`];
return [trimmed];
}
function buildNoProxyValue(tokens: Iterable): string | null {
const seen = new Set();
const values: string[] = [];
...
if (!line) continue;
if (/^ExceptionsList\s\*:\s\*\s\*\{$/.test(line)) {
inExceptions = true;
continue;
}
if (inExceptions) {
...
...exceptions,
...(scalars.get("ExcludeSimpleHostnames") === "1" ? [""] : []),
]),
},
platform,
);
...
export function createProcessStampArgs(
stamp: TStamp,
contract: ProcessStampContract,
): string[] {
...
export function readProcessStamp(
...
export function readProcessStampFromCommand(
command: string,
contract: ProcessStampContract,
): TStamp | null {
return readProcessStamp(commandArgs(command), contract);
}
export function matchesProcessStamp = Partial>(
stamp: TStamp,
...
export function matchesStampedProcess = Partial>(
processInfo: Pick,
criteria: TCriteria | undefined,
contract: ProcessStampContract,
): boolean {
...
// Build the `cmd.exe /d /s /c ""` invocation Node uses internally for
...
async function waitForChildSpawn(child: ChildProcess): Promise {
...
export async function spawnLoggedProcess(request: SpawnProcessRequest): Promise {
const invocation = createCommandInvocation(request);
const child = spawn(invocation.command, invocation.args, {
cwd: request.cwd,
...
export async function waitForProcessExit(pid: number | null | undefined, timeoutMs = 5000): Promise {
...
async function listPosixProcessSnapshots(): Promise {
...
export async function listProcessSnapshots(): Promise {
try {
...
async function waitForProcessesToExit(pids: number[], timeoutMs = 5000): Promise {
const startedAt = Date.now();
while (Date.now() - startedAt < timeoutMs) {
...
export async function stopProcesses(pids: Array): Promise {
...
export async function waitForHttpOk(url: string, { timeoutMs = 20000 }: HttpWaitOptions = {}): Promise {
const startedAt = Date.now();
...
export async function readLogTail(filePath: string, maxLines = 80): Promise {
try {
...
// `/bin` if defined. Defaults to process.env so user-customised
...
// `npm i -g` today lives at `/bin`. Conventional locations
...
// Unix: npm global binaries live in /bin. Always add it as a
...
// the /bin entry above is left untouched for cross-platform
// parity.
if (process.platform === "win32") {
dirs.push(npmPrefix);
}
}
}  // end of npmPrefixRaw block
  // ponytail: GUI-launched processes miss env npm_config_prefix on Windows.
  // Fall back to reading the user's actual .npmrc config.
  if (process.platform === "win32" && (typeof npmPrefixRaw !== "string" || npmPrefixRaw.trim().length === 0)) {
    try {
      const winPrefix = execFileSync("npm", ["config", "get", "prefix"], {
        encoding: "utf8", timeout: 3_000, windowsHide: true,
      }).trim();
      if (winPrefix.length > 0) {
        dirs.push(join(winPrefix, "bin"));
        dirs.push(winPrefix);
      }
    } catch {}
  }
...
// Windows fnm keeps Node installs under \node-versions\\
...
function sortVersionedDirEntries(entries: import("node:fs").Dirent[]): import("node:fs").Dirent[] {
