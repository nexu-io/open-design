...
import { stageElectronNode, type StageElectronNodeInput } from "./node.js";
...
export async function buildElectronPlatform(input: BuildElectronPlatformInput) {
  if (![input.dependenciesRoot, input.npmCliPath, input.verificationEntryPath, input.preparationEntryPath].every(isAbsolute)) throw new Error("platform build paths must be absolute");
  ...
  const node = await stageElectronNode(input);
  ...
  await execute(node.executablePath, [input.npmCliPath, "ci", "--omit=dev", "--no-audit", "--no-fund"],
