...
import { copyFile, cp, lstat, mkdir, readFile, readdir, readlink, writeFile } from "node:fs/promises";
...
export async function buildElectronDistributionBase(input: Readonly<{ sceneDirectory: string; sceneManifestSha256: string; outputRoot: string }>) {
  ...
  const electronPackage = JSON.parse(await readFile(packagePath, "utf8"));
  const base = await assembleElectronDistributionBase({ scene, runtimeDirectory: join(dirname(packagePath), "dist"),
    electronVersion: electronPackage.version, target: `${process.platform}-${process.arch}`, outputRoot: input.outputRoot });
  for (const name of carrierFiles) {
