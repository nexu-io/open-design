...
// Pure replacement for node:path.basename — contracts must not import node:*.
// Handles both POSIX (/) and Windows (\) separators on every platform.
...
function pushSource(
  ...
  const basename = basenameOf(source.replace(/:\d+$/, ''));
