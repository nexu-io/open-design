/**
 * Shared Markdown-context helpers used by the streaming artifact parser, the
 * post-stream `<artifact>` stripper and the transcript builder. The single
 * source of truth for what counts as a fenced code block or an inline code
 * span — kept in lock step with apps/web/src/runtime/markdown.tsx so the
 * parser/stripper view of a buffer matches what the chat UI will actually
 * render.
 *
