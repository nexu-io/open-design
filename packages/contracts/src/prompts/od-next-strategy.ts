...
For a Full Plan route, plan first: you may read the bounded inputs needed to freeze the plan, but do not create, edit, render, or dispatch a deliverable until the Plan Contract and Execution Preflight are frozen. A simple Full Plan then performs its Build in the same response, on the request or clarification stage, and closes with \`outcome: completed\`. A complex Full Plan closes with \`outcome: plan_ready\` and builds only after Open Design continues the same native session into the production stage. When you declare \`outcome: completed\` on the request or clarification stage, the same canonical-deliverable check that gates production already applies: Open Design must be able to identify one runnable entry in the delivered files, otherwise the completed task is rejected — it looks for a root \`index.html\`, then a single root-level html file, then a single file matching the project kind. Write every deliverable inside the project directory and lay it out so exactly one of those resolves; files written outside the project directory are not delivered work and leave the task with no artifact.
...
Keep the Todo plan live while performing Build work. Direct Edit stays local and bounded. Full Plan freezes its decisions before Build, and every Build Package uses the same frozen Design Spec.`;
...
export function composeOdNextStrategyContinuationV2(
  ...
  if (input.stage === 'clarification') {
    const intent = input.executionIntent === 'plan_only'
      ...
      : '';
    payload = `# OD Next native continuation — clarification\n\nMerge the user's answer below into the existing Full Plan context.${intent} Preserve the locked route, ask no second question round, rerun only affected resolution and Preflight work, and emit the updated V2 machine structures. This turn runs at the clarification stage: the Runtime State reports inputStage clarification (not request), with outcome completed for an executionIntent plan_only answer without file writes; for produce, once the Full Plan is frozen, a simple plan performs its Build in this same response and reports outcome completed, and a complex plan reports outcome plan_ready for production; otherwise blocked or canceled.\n\n## Clarification answer\n\n${requireText(input.answer, 'answer')}`;
