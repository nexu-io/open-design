...
Open Design ships a stdio MCP server. Wire it into Claude Code, Cursor, VS Code (GitHub Copilot), Zed, Windsurf, or any MCP-compatible client and the agent in another repo can read files from your local Open Design projects directly - no zip export, no copy-paste. When the agent calls `search_files`, `get_file`, or `get_artifact` without a project argument, the MCP defaults to whatever project (and file) you have open in Open Design right now, so prompts like *"find the Polaroid section in this design"* just work.

Open **Settings → MCP server** in the Open Design app for a per-client install flow. The panel asks the running daemon for the absolute paths to its built `cli.js` + the `node` binary, then bakes those into every snippet - so it works on a fresh source clone where `od` isn't on your PATH (and dodges the macOS / Linux `/usr/bin/od` octal-dump tool that would otherwise shadow it). Cursor gets a one-click deeplink; Claude Code gets a single `claude mcp add` command with `--scope user`; VS Code, Zed, and Windsurf each get JSON in the schema their config file expects. Read-only by design; the daemon must be running locally.
