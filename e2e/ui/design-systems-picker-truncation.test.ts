...
test('[P1] Settings Design systems sidebar row truncates long category labels (#2688)', async ({
  ...
  // mrcfps 2026-07-31 review on head 1396d960b flagged that the long category
  // label grows the flex container itself, so scrollWidth == clientWidth and
  // the truncation precondition never fires. Inject a deterministic bounded
  // width onto the WeChat SystemRow card before measuring so the subtitle
  // must ellipsize. 240px is narrower than any real layout the Settings
  // sidebar would allocate at the default viewport but wide enough that the
  // row's preceding icon + label still render — the goal is to force the
  // truncation contract to fire, not to model a specific production
  // viewport. The injected style is scoped to this test's instance via a
  // unique class so concurrent workers cannot cross-pollinate.
  await page.addStyleTag({
    content: `
      .od-truncation-bound-240[data-testid="design-system-card-${WECHAT_PRESET.id}"] {
        max-width: 240px !important;
        width: 240px !important;
      }
      .od-truncation-bound-240[data-testid="design-system-card-subtitle-${WECHAT_PRESET.id}"] {
        max-width: 220px !important;
        width: 220px !important;
      }
    `,
  });
  await wechatCard.evaluate((el: Element) => el.classList.add('od-truncation-bound-240'));
  const subtitle = page.getByTestId(
    `design-system-card-subtitle-${WECHAT_PRESET.id}`,
  );
  await subtitle.evaluate((el: Element) => el.classList.add('od-truncation-bound-240'));
  ...
  await expectSingleLineRow(subtitle, 'settings sidebar subtitle');
  ...
  await expectSingleLineRow(subtitle, 'settings sidebar subtitle (narrow)');
...
test('[P1] New project design-system popover renders WeChat on a single-line row (#2688)', async ({
  ...
  // mrcfps 2026-07-31 review on head 1396d960b flagged that the portaled
  // popover widens to fit its longest option, so the WeChat title row hits
  // scrollWidth == clientWidth and the truncation precondition never fires.
  // Force a deterministic narrow popover width (320px — narrower than the
  // long WeChat DEFAULT row but wide enough to keep the option chevron /
  // selected-check visible) so the title must ellipsize. Scoped via a unique
  // class so concurrent workers cannot cross-pollinate.
  await page.addStyleTag({
    content: `
      .od-truncation-popover-320.ds-picker-popover {
        max-width: 320px !important;
        width: 320px !important;
      }
      .od-truncation-popover-320.ds-picker-popover .ds-picker-item-title {
        max-width: 240px !important;
        width: 240px !important;
      }
    `,
  });
  await popover.evaluate((el: Element) => el.classList.add('od-truncation-popover-320'));
