...
test("isDeniedChangedPath blocks common tool config files under allowlisted source trees", () => {
  ...
  assert.equal(isDeniedChangedPath("apps/closure/tsconfig.json"), true);
  assert.equal(isDeniedChangedPath("apps/closure/vitest.config.ts"), true);
...
test("isAllowedChangedPath allows ordinary app and package source/test paths while keeping denied surfaces blocked", () => {
  assert.equal(isAllowedChangedPath("apps/closure/src/main.ts"), true);
  assert.equal(isAllowedChangedPath("apps/closure/tests/main.test.ts"), true);
  assert.equal(isAllowedChangedPath("apps/web/src/App.tsx"), true);
  ...
  assert.equal(isAllowedChangedPath("apps/closure/tsconfig.json"), false);
  assert.equal(isAllowedChangedPath("apps/closure/vitest.config.ts"), false);
});
