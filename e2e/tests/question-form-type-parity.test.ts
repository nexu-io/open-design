 ...
 *
 * ── 为什么需要这个测试 ────────────────────────────────────────
 * 历史事故:往旧策略的提示词里加了规则,新策略那条路没加,结果视觉方向题
...
/**
 * 判据是**集合相等**:提示词枚举的类型 == 渲染器认得的类型。
 *
 * ── 判据变更(2026-09-08)────────────────────────────────────
 *
 * 2026-09-07(T69)曾把它放宽成「渲染器 − 休眠集」,`DORMANT_TYPES` 里躺着
 * 一个 `direction-cards`:当时的做法是**提示词撤掉、渲染层留着当安全网**。
 * 2026-09-08 产品推翻了那个做法,要求把选设计风格这条路**整条删除**
 * (逐字:「还额外加一个不许问.. 有病吗.. **都删掉啊**」),渲染器从此也不认
 * 这个类型 —— 休眠集因此清空,判据回到**集合相等**。
 *
 * 整条路真的不在了,由 `design-direction-picker-removed.test.ts` 正面守着。
 *
 * ⚠️ 不要再引入「休眠集」这种放宽:两侧不相等意味着某个能力对模型不可见,
 * 那必须是一次明确的产品裁决,而不是「这条路提示词写漏了」的消音器。
 */
...
describe('question-form 提示词跨路径一致性', () => {
  ...
  it('六条路径都完整枚举了渲染器支持的每一个类型', () => {
    ...
    for (const { rel } of PROMPT_PATHS) {
       ...
       *
       * 事实源就是渲染器本身:**多**写一个渲染器不认的类型、**少**写一个在用
       * 类型,两边都当场红。
       */
