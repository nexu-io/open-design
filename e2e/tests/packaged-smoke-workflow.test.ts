  ...
  it("rejects stale latest platform manifests from a previous beta version", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        artifacts: {
          dmg: {
            url: "https://releases.marketing-ax.example/beta/versions/1.2.3-beta.3.unsigned/M-AX Beta.dmg",
          },
        },
        channel: "beta",
        github: {
  ...
  it("rejects stale latest platform manifests from a previous same-version beta workflow run", async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        artifacts: {
          dmg: {
            url: "https://releases.marketing-ax.example/beta/versions/1.2.3-beta.4.unsigned/M-AX Beta.dmg",
          },
        },
        channel: "beta",
        github: {
  ...
  return {
    ...
    platforms: {
      mac: {
        arch: "arm64",
        artifacts: {
          dmg: artifact("M-AX.dmg"),
          zip: artifact("M-AX-mac-arm64.zip"),
        },
        enabled: true,
        signed: true,
      },
      macIntel: {
        arch: "x64",
        artifacts: {
          dmg: artifact("M-AX Intel.dmg"),
          zip: artifact("M-AX-mac-x64.zip"),
        },
        enabled: true,
        signed: true,
      },
      win: {
        arch: "x64",
        artifacts: {
          installer: artifact("M-AX Setup.exe"),
        },
        enabled: true,
      },
    },
