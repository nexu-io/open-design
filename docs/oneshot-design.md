...
- Library Search views can now be named, saved, grouped by collection or production lane, pinned above recent views, annotated with owner and usage notes, duplicated as quick variants, reapplied, deleted, exported, and imported as OneShot JSON packets, so common filters can become portable studio workbenches.
...
1. Add Library Search view export previews so operators can inspect what will move before downloading or importing a packet.
