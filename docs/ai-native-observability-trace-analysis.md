...
This document records how to read the new Marketing AX Langfuse trace shape for
...
content should remain in Marketing AX object storage and be referenced by
...
Raw sensitive content belongs in Marketing AX object storage with access control,
...
Issue #3733 targets the first durable bridge between Marketing AX-owned object
...
Marketing AX-owned project storage today and to a ProjectStorage or
...
| Storage reference | Emits stable `od://objects/...` references. | Treats `storage_ref` as an Marketing AX registry reference, future-compatible with ProjectStorage/S3 backends. | No conflict; current refs should remain non-dereferenceable from Langfuse. |
...
1. Make the manifest builder resolve through a concrete Marketing AX object
...
4. Preserve `storage_ref` as a stable, non-secret reference and require any
   future download or preview access to go through Marketing AX-controlled
   authorization.
...
- `pnpm install`
- `pnpm --filter @marketing-ax/daemon exec vitest run -c vitest.config.ts tests/langfuse-trace.test.ts tests/langfuse-bridge.test.ts tests/claude-stream-thinking.test.ts`
...
1. Store complete raw/redacted prompt payloads in Marketing AX object storage and
