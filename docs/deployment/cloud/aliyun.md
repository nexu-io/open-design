...
This guide covers self-hosting Marketing AX on Alibaba Cloud for users in mainland China and the broader Asia-Pacific region. It documents the supported deployment paths, image-pull optimisations, and ICP filing considerations for public-facing instances.
...
For users in mainland China, deploying Marketing AX to Alibaba Cloud (阿里云) instead of an overseas provider gives you:
...
This path puts Marketing AX on a single ECS instance using the same Docker Compose stack documented in [`docs/deployment/docker.md`](../docker.md).
...
| Image | Ubuntu 24.04 LTS 64-bit | Marketing AX's Docker image is `linux/amd64` and `linux/arm64` |
...
Marketing AX does not yet ship an official Helm chart. The minimal manifest below is a starting point — production users should harden it (resource limits, PodDisruptionBudget, NetworkPolicy, persistent storage class).
...
metadata:
  name: open-design
spec:
  replicas: 1  # Marketing AX uses local SQLite; multi-replica needs shared storage
  selector:
...
Resource Orchestration Service (资源编排 ROS) provisions the same resources declaratively. There is no first-party ROS template for Marketing AX yet. Operators with Alibaba Cloud access are welcome to contribute one as a follow-up to this guide; the natural location is `deploy/aliyun/ros/`.
...
Marketing AX's daemon binds to `127.0.0.1:7456` and is never exposed directly. Public access must go through:
...
- Marketing AX Docker deployment: [`docs/deployment/docker.md`](../docker.md)
- Marketing AX one-click installer: [`docs/install-guide.md`](../../install-guide.md)
