...
- 2026-05-20: Required `--expected-head` whenever `scripts/download-tauri-m4-reports.ts --advance` is used. Verify-only explicit run downloads still inspect a known run through `gh run view`, but the mutating M4→M5 path refuses to run unless the selected CI evidence is tied to the exact migration branch head. `node --import tsx --test scripts/download-tauri-m4-reports.test.ts` and `tsc -p scripts/tsconfig.json --noEmit` passed.
...
- 2026-05-20: Refined the same M4 report status path for the case where the report root itself has not been created yet. `scripts/tauri-migration-status.ts --report-dir <dir>` now reports `platform report directory missing` before looking for Windows/Linux artifact manifests, which matches the current pre-CI handoff state more directly. `node --import tsx --test scripts/tauri-migration-status.test.ts` and `tsc -p scripts/tsconfig.json --noEmit` passed.
- 2026-05-20: Extended `scripts/continue-tauri-migration.ts` so continuation dry-runs surface platform report blockers before planning M4 handoff work. In the current pre-CI state, the runner now prints the missing report directory alongside the remote-branch blocker instead of requiring operators to cross-reference status output. `node --import tsx --test scripts/tauri-migration-status.test.ts` and `tsc -p scripts/tsconfig.json --noEmit` passed.
...
- 2026-05-20: Required `--expected-head` for every `scripts/download-tauri-m4-reports.ts` download, including latest-run and verify-only paths. The downloader no longer prints an advance command with a placeholder SHA and will not consume native report artifacts unless the selected run is tied to the current migration branch head. `node --import tsx --test scripts/download-tauri-m4-reports.test.ts` and `tsc -p scripts/tsconfig.json --noEmit` passed.
- 2026-05-20: Made `scripts/continue-tauri-migration.ts` transfer guidance self-identifying when local push is unavailable. The dry-run and failed-push paths now print the expected migration branch head plus the archive and command-sidecar SHA-256 values before listing the four files to move, so a write-capable receiver can cross-check the handoff set before import/push. `node --import tsx --test scripts/tauri-migration-status.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, `pnpm guard`, `pnpm typecheck`, and `git diff --check` passed.
- 2026-05-20: Extended the same self-identifying transfer guidance into `scripts/tauri-migration-status.ts` next actions. Status JSON now carries the expected migration branch head plus archive and command-sidecar SHA-256 values in the copy-to-receiver action, matching the executable dry-run guidance. `node --import tsx --test scripts/tauri-migration-status.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, `pnpm guard`, `pnpm typecheck`, and `git diff --check` passed.
- 2026-05-20: Added the self-identifying handoff summary to the packaged command sidecar itself. After checksum, manifest, and bundle validation, the receiver script now prints the expected remote branch head plus archive, command-script, and bundle SHA-256 values before touching the receiver branch or pushing. `node --import tsx --test scripts/package-tauri-migration-handoff.test.ts scripts/push-tauri-migration-handoff.test.ts scripts/tauri-migration-status.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, `pnpm guard`, `pnpm typecheck`, and `git diff --check` passed.
- 2026-05-20: Added the same pre-push identity output to the push-only receiver helper. `scripts/push-tauri-migration-handoff.ts --archive ...` now writes the target remote, migration branch head, archive SHA-256, and command-sidecar SHA-256 before attempting the remote push, so a failed receiver push still leaves the exact handoff identity in stdout. `node --import tsx --test scripts/push-tauri-migration-handoff.test.ts scripts/package-tauri-migration-handoff.test.ts scripts/tauri-migration-status.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, `pnpm guard`, `pnpm typecheck`, and `git diff --check` passed.
- 2026-05-20: Aligned the status, package, handoff note, and runbook push-only receiver commands with the continuation runner's explicit receiver option surface. The copy-pasteable `scripts/push-tauri-migration-handoff.ts --archive ...` examples now carry `GITHUB_WORKFLOW`, `TAURI_M4_REPORT_DIR`, and `TAURI_PR_BODY_PATH` defaults instead of relying on hidden helper defaults. `node --import tsx --test scripts/package-tauri-migration-handoff.test.ts scripts/verify-tauri-migration-handoff.test.ts scripts/tauri-migration-status.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, `pnpm guard`, `pnpm typecheck`, and `git diff --check` passed.
- 2026-05-20: Tightened the M4 remote evidence marker so status, policy, and direct M5 application require the branch/head detail that `scripts/advance-tauri-migration-m4-m5.ts` records, not just the generic marker text. This keeps manually edited or partial M4 logs from opening the default-flip path. `node --import tsx --test scripts/tauri-migration-policy.test.ts scripts/tauri-migration-status.test.ts scripts/apply-tauri-migration-m5.test.ts scripts/advance-tauri-migration-m4-m5.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, and `git diff --check` passed.
- 2026-05-20: Bound that remote evidence detail to the current local migration HEAD where git state is available. `scripts/tauri-migration-status.ts` and direct `scripts/apply-tauri-migration-m5.ts` now stay in M4 if the recorded pushed branch-head evidence names an older SHA than the current branch head, so post-evidence commits cannot accidentally reuse stale native report evidence. `node --import tsx --test scripts/tauri-migration-policy.test.ts scripts/tauri-migration-status.test.ts scripts/apply-tauri-migration-m5.test.ts scripts/advance-tauri-migration-m4-m5.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, and `git diff --check` passed.
- 2026-05-20: Tightened the guarded M4→M5 advance path for documents that already contain stale remote evidence. `scripts/advance-tauri-migration-m4-m5.ts` now appends a new current-head remote evidence detail when the existing detail names an older SHA, so the subsequent direct M5 applicator sees evidence for the exact branch head it is about to flip. `node --import tsx --test scripts/advance-tauri-migration-m4-m5.test.ts scripts/apply-tauri-migration-m5.test.ts scripts/tauri-migration-status.test.ts scripts/tauri-migration-policy.test.ts`, `tsc -p scripts/tsconfig.json --noEmit`, and `git diff --check` passed.
...
pnpm exec tsx scripts/push-tauri-migration-handoff.ts \
  ...
  --report-dir "${TAURI_M4_REPORT_DIR:-/tmp/open-design-tauri-m4-reports}" \
  --pr-body-path "${TAURI_PR_BODY_PATH:-.tmp/tauri-migration-pr-body.md}"
```

Set `REMOTE=<remote>` instead of `--remote origin` if the receiving checkout uses a differently named write-capable remote. The command sidecar, push helper, continuation runner, and report downloader all honor that environment default. Set `GITHUB_WORKFLOW=<workflow-file>`, `TAURI_M4_REPORT_DIR=<report-dir>`, or `TAURI_PR_BODY_PATH=<path>` when the receiver should use non-default workflow, report, or draft-PR-body paths. Add `--gh /path/to-gh` or set `GH_BIN=<path-to-gh>` if the receiving machine uses a non-default GitHub CLI binary and you want the printed workflow-dispatch and draft-PR fallback commands to use that binary.
