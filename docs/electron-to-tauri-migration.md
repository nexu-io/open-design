...
If the bundle is copied outside the handoff directory, keep using the manifest for branch/checksum data and add `--bundle /path/to/open-design-tauri-migration.bundle` to the `push-tauri-migration-handoff.ts` command to override only the file location.
