...
- 2026-05-20: Extended `scripts/package-tauri-migration-handoff.ts` output with the receiver archive push command, the post-CI report download command with `--advance`, and the status check command. It also writes an executable `.commands.sh` sidecar beside the tarball so the receiver can push the archive and later run report download/advance with `GITHUB_RUN_ID=<run-id>`.
...
On the receiving checkout, copy both the tarball and `.sha256` sidecar, then verify the checksum, extract the handoff, import the bundle, push the branch, and verify the remote head in one command:
