...
Rollout is **behavior-preserving and slice-by-slice**, not a repo-wide migration. The first PR on a file extracts **one seam** — port + provider binding + pure rules — and layers sub-components and hooks in follow-ups. `MemorySection` is the first canary (self-contained, single consumer, existing test). Boundaries are enforced by a hand-rolled guard added to `pnpm guard` (no `fetch`/`EventSource`/`window`/`localStorage` in feature or pure files; one transport home per route; no cross-slice internal imports); orphaned provider exports are swept by `ts-prune`.
...
Slice logic becomes testable against a hand-written fake implementing the in-slice port — no global `fetch` mocking, no module-path mocks; pure `rules.ts` tests need zero doubles; fixtures typed against `contracts` fail at compile time on a wire change. Transport gets a single source of truth per resource. The cost is that a small transport wrapper no longer sits physically inside the slice (mitigated: the port does, and `ts-prune` sweeps orphans), and a new guard script must be maintained. Because rollout is seam-by-seam and behavior-preserving, each PR is independently reviewable and reversible.
