...
keep Marketing AX safe to embed under an external orchestrator without adding
consumer-specific infrastructure to this repository.

The plan is intentionally upstream-neutral. Marketing AX should refer to this
...
runtime contract while adding normal Marketing AX features. The durable defense is
...
1. An external orchestrator launches Marketing AX with `OD_SANDBOX_MODE=1` and
...
3. Marketing AX contributes design context, skills, previews, artifacts, CLI/UI
...
Without `OD_SANDBOX_MODE=1`, Marketing AX is the normal local product runtime, not
...
- process matching must not classify a foreign process tree as an Marketing AX
...
- Keep the allowlist narrow so normal references to Marketing AX, GitHub
...
- Do not add a generic provider router, provider account pool, global media
  budget system, or external executor API to Marketing AX.
- Do not add consumer-specific conformance tests to Marketing AX CI.
