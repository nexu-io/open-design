# Marketing AX Plugin Registry — Plan (living)
...
> up their own Marketing AX plugin source with one config line.
...
start in the main Marketing AX repo, but the code path must still be expressed as
...
- `od plugin login` wraps `gh auth login` with Marketing AX copy, scopes, and
