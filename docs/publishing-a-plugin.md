# Publishing An Marketing AX Plugin

Marketing AX registry publishing is GitHub-backed in v1. The CLI remains the
...
The generated `open-design.json` is the Marketing AX sidecar next to `SKILL.md`.
...
These commands wrap GitHub CLI. Tokens stay in `gh`; Marketing AX does not store
GitHub credentials.
