...
- Prompt delivery: positional argv (no stdin sentinel; clap declares `prompt: String` as a required field). This means very large composed prompts can hit Windows' ~32 KB `CreateProcess` limit; for typical chat prompts this is non-issue. Upstream support for a `-` stdin sentinel would let us flip this to `promptViaStdin: true` like the other adapters. To avoid surfacing oversized prompts as a generic `spawn ENAMETOOLONG` / `E2BIG`, the adapter declares `maxPromptArgBytes` (currently 30,000); `/api/chat` runs the composed prompt through the `checkPromptArgvBudget` helper before bin resolution and emits an actionable SSE error (`AGENT_PROMPT_TOO_LARGE`) telling the user to reduce the selected skills / design-system context or pick an adapter with stdin support. The helper is unit-tested for both the oversized and short-prompt branches against the deepseek adapter so the guard can't silently regress.
