...
- Artifacts: reconstructed `text` deltas are fed into the same plain-stream artifact finalizer as §5.13, so `<artifact type="text/html">…</artifact>` blocks still land as project files. Files the CLI writes into the project cwd remain a complementary path.
