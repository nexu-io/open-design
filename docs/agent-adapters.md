...
The contract and the per-CLI defs live under `apps/daemon/src/runtimes/`; the streaming parsers and the spawn/dispatch glue live beside them in `apps/daemon/src/`.
...
├── claude-stream.ts        # eventParser="claude-stream": JSONL → UI events
├── qoder-stream.ts         # eventParser="qoder-stream"   · copilot-stream.ts · json-event-stream.ts
├── pi-rpc.ts               # pi's JSON-RPC transport
├── acp.ts                  # shared ACP JSON-RPC transport (devin / kimi / kilo / kiro / vibe / hermes)
