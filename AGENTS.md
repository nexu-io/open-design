...
- Packaging jobs in `release-prerelease.yml` deliberately run in parallel with the four test gates (`functional_e2e`, `e2e_vitest`, `daemon_unit_tests`, `verify`) rather than behind them. The gate that actually protects users is `publish`, which requires all four to be `success` before any `metadata.json` is written. When you add a test gate to that workflow, add it to `publish`'s `needs` **and** to its `if` — a job absent from `needs` reports an empty `result` and gates nothing.
