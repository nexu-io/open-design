...
- Continuing a turn's remaining tasks (`onContinueRemainingTasks`) travelled on that pinned card and now renders as the 〔继续剩余任务〕 button in the turn footer (`AssistantMessage`'s footer row, `data-testid="assistant-continue-remaining"`), next to the status line it follows from. It is drawn only for the last turn that still owes work: a stopped or unfinished TodoWrite snapshot with no pending complete question form, and never for a turn whose strategy task recorded a written deliverable (`strategyTaskDelivered`).
