...
- **Prerelease and stable have no Linux entry or DAG lane.** Do not add an `enable_linux_x64` input, Linux output, metadata field, build, smoke, or card row back to either workflow. Preview retains its optional Linux policy independently.
- **Stable reuses formal prerelease Plan results.** Only workloads with an explicit shared recipe and a `release-prerelease / prerelease-v1` trusted source may cross the channel boundary. Stable still rebuilds version-bound packaging, signs, notarizes, smokes, and prepares public metadata. Its `publish` input defaults to `false`; the default run completes prepublish validation without R2 or GitHub Release side effects.
