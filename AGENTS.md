...
- Prerelease builds macOS x64 (Intel) only on request. `release-prerelease.yml` takes an `enable_mac_x64` input that defaults to `false`, and `notify-release-feishu.yml` (the push-to-`release/**` entry point) forwards `false`, because Intel is the pipeline's critical path. Intel is opt-in the same way Linux is opt-in via `vars.ENABLE_STABLE_LINUX`; when it is off, its `releaseTargets`/`platforms` entry is simply absent from the published `metadata.json`, exactly as Linux's is today.
- **The last prerelease before a stable promotion must be dispatched with `enable_mac_x64` enabled.** Stable ships Intel and stable's gate is validated prerelease artifacts, so an Intel-less prerelease has not validated the Intel artifact stable will ship. This is enforced, not merely documented: `validateStablePrereleaseMetadata` in `tools/release/src/metadata/prepare-stable.ts` requires `platforms.macIntel` (with `enabled: true`, `arch: "x64"`, `signed: true`, and versioned dmg/zip URLs) in the promoted prerelease's metadata, so promoting an Intel-less prerelease fails the stable `metadata` job instead of silently shipping an unvalidated Intel build. `platforms.linux` is deliberately not required there.
- Prerelease is tuned for wall-clock feedback, so three other things are opt-in per run alongside `enable_mac_x64`, and the pre-stable dispatch should turn all of them on together. `enable_smoke` (default `false`) runs the packaged smoke suites; they are advisory (`continue-on-error`) and have never blocked publication, but they cost ~2.5 min on the Windows critical path. `mac_sign_mode` (default `sign-only`) codesigns without the Apple notary round-trip — `notarize` is the pre-stable setting, since stable ships notarized. `win_x64_smoke_mode` still chooses how deep the Windows smoke goes, but only applies when `enable_smoke` is on.
...
- **Automated tests do not gate prerelease delivery.** In `release-prerelease.yml` the four test jobs (`functional_e2e`, `e2e_vitest`, `daemon_unit_tests`, `verify`) are absent from both `build_*` and `publish` — a prerelease exists so a person can install it and test it by hand, and machine CI is a parallel opinion about the same commit rather than a valve on delivery. A flaky suite must never cost the team a day's package. The packaged smoke suites are advisory for the same reason and now run *after* `Publish <platform> platform`, so a smoke result cannot decide whether a package was uploaded. Do not reintroduce a test job into `publish`'s `needs`/`if`; if a check must genuinely block delivery, that is a channel-policy decision, not a workflow edit.
- The advisory results leave the workflow as `workflow_call` outputs so the notification layer can raise the alarm: `functional_e2e_result`, `e2e_vitest_result`, `daemon_unit_tests_result`, `verify_result`, the `tests_all_green` / `failed_test_jobs` rollup (all produced by the `test_signals` job), and `mac_arm64_smoke_result` / `mac_x64_smoke_result` / `win_x64_smoke_result` / `linux_x64_smoke_result`. `version_metadata_url` is the authoritative "did a package ship?" signal — the overall workflow conclusion is not, because it goes red when an advisory job fails even though the package shipped. When you add an advisory job, add it to `test_signals` rather than to `publish`.
- `test_signals` funnels those results through a real job output instead of the documented `jobs.<job_id>.result` expression, because actionlint — which `ci.yml` runs at a pinned version — has rejected that property since 2023 (rhysd/actionlint#343). Keep it that way until that issue closes.
