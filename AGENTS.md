...
- Runtime target is Node `~24` and `pnpm@11.15.0`; use Corepack so the pnpm version pinned in `package.json` is selected.
...
- `corepack enable` fails with EPERM on Windows (cannot write shims to `Program Files`). Use `npm install -g pnpm@11.15.0` instead.
