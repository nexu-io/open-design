...
- **Continued editability:** centralize colors, type sizes, spacing, and
  ...
  values.
- **Imagery by type:** choose each image's source from what it depicts.
  - Real referents — a named book cover, a real product, a brand mark, a real
    ...
    user data.
  - Illustrative, fictional, or atmospheric subjects — generic dishes, example
    people and scenes, hero and background imagery, sets that must share one
    style, subjects stock libraries lack — are generated by default when image
    generation is available; otherwise use a licensed stock photo.
  ...
  never de-realize content to avoid acquiring the real asset.
