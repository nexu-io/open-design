...
general orchestration Skill; this profile says what a marketing task
...
copy, legal constraints, and required size variants. Put the visual concept,
palette, type, composition, image language, safe areas, and variant rules in
the design notes.

Never silently drop a missing field: decide per the general orchestration
Skill whether to ask in the one question form, or convert the gap into
...
separate order. Where upstream instructions and the plan are silent, carry
...
line, and writes the print parameters above into the design notes and the
delivery notes.
...
visual approach and record it in the design notes: big-type poster styles make
...
Build one concept and its variant set in one context: the key visual and the
core message first, then each required size as its own composition — never a
mechanical scaling of another — naming the exact output size of every file.
