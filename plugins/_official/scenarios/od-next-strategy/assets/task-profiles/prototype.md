...
- **Imagery:** follow the Core imagery-by-type rule. Real entities in the
  interface (book covers, products, brand marks, real places) use their real
  images, localized into the project; never fabricate them with image
  generation. Illustrative and atmospheric imagery (generic dishes, example
  people and scenes, hero backgrounds) is generated by default.
