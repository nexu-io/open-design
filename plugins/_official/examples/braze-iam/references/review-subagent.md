...
2. 채점 5축 (각 축 감점 사유를 개별 발견으로 기록):
   ...
   - **디자인·컴포지션 (비전)** — 에셋 PNG(`scene` 모드는 씬 PNG) + 발송본
     ...
     - 공통 (양 모드):
       ...
       · **레이아웃 문법 6항** (정본 = visual-layout-patterns.md v2 139핀,
         2026-07-14 개정 — §1·§3·§5·§7·§12):
         ⓐ 히어로 = §3 등재 유형 중 하나 + 시선 앵커 1개인가 — 마스코트/캐릭터
           행동 히어로 = P0, layer 히어로에 풀 일러스트 씬 = P0 (배경 포함은
           scene 모드 전용), 마케팅 IAM의 비주얼 제로 = P1 (§11)
         ⓑ 배경-오브제 대비 — 히어로 오브제 채도 > 배경 (비비드 필수).
           파스텔-온-파스텔 워시 = P1, 팔레트 공식(뉴트럴+비비드 1+악센트 ≤1,
           오버레이형은 비주얼 무드 1+반전색+악센트 ≤1) 위반 = P1
         ...
         ⓔ **오버레이 가독 조건 (§1-b)** — 텍스트가 비주얼 위에 얹힌 레이아웃이면
           스크림/여백존/고대비 반전 중 1개 이상 실재하는가. 무대비 오버레이 = P1
           (판독 곤란 수준은 아래 WCAG 실측에서 P0)
         ⓕ **variant 레이아웃 분화 (§12.3)** — A/B가 기획안 `layout.divergenceAxis`
           대로 실제 1축 이상(유형/존 문법/히어로 유형) 갈라졌는가. 카피·색만 다른
           A/B = P1. 단 brief에 "동일 레이아웃 — 사용자 지정" 오버라이드가 기록돼
           있으면 통과. 창작 레이아웃(§12.2)인데 brief에 `창작 레이아웃 — 근거`
           표기가 없으면 P1
