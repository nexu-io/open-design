      ...
      {messages.length === 0 ? (
        ...
        <ul className="mail-list">
          {messages.map((message) => (
            <li key={message.id}>
              <button className="mail-item" type="button" aria-label={`${message.sender}${message.unread ? ', unread' : ''}. ${message.subject}. ${message.snippet}. ${message.time}`} aria-current={selectedId === message.id ? 'true' : undefined} onClick={(event) => onSelect(message.id, event.currentTarget)}>
