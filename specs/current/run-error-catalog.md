...
> ⚠️ **上面这张候选表已被 OPEND-2807 推翻(2026-09-16 落地),原文保留只为留档。**
> 产品的终局是 A / B / C **一个都没选**:卡上恒是三颗,两颗常驻次级无条件在场,
> 第三颗按环境二选一 —— Cloud 给〔重试〕,CLI / BYOK 给〔切换到 OpenDesign Cloud〕。
> 阶梯算出来的那组动作**整组离开卡面**,不再是「留在卡上、降为次级」。
> 判据、三个例外(宿主接不住 / `git_bash_missing` / 余额为 0)、四条代价和埋点收窄,
> 全部记在 `specs/current/chat-panel-decisions-sheet.md` 的「OPEND-2807 报错卡终局」一节。

### 6.T 主按钮阶梯 ↔ §5 流程表的对应(别让两份规格各说各话)

> ⚠️ **这张对应表自 OPEND-2807(2026-09-16)起整节失效,保留为历史记录。**
> 它描述的「次级动作的分档」在今天的卡上没有落点了:阶梯那组动作整组下卡,
> 卡上只有两颗常驻次级加一颗按环境二选一的主动作,阶梯档位不再决定任何按钮。
> 阶梯本身(`primaryActionForFailure`)仍在算,但只用于标题 / 正文 / 交接。
> 终局见 `specs/current/chat-panel-decisions-sheet.md`「OPEND-2807 报错卡终局」。
>
> 下面这段是 §6.ZB 当时的注记,同样作为历史保留:
>
