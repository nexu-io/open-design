...
|---|---:|---|---|---|---|
| R1 | P0 | Daemon TypeScript enforcement needs ongoing maintenance. | `apps/daemon` typechecks across source and tests. Most `@ts-nocheck` suppressions have been removed; six high-conflict daemon core files remain suppressed and are tracked in W3 (`server.ts`, `cli.ts`, `agents.ts`, `projects.ts`, `runs.ts`, `memory-llm.ts`). | New daemon payload, DB row, agent event, or task-state changes can reintroduce drift if they bypass shared contracts or typed boundaries. | Keep daemon source and tests under TypeScript enforcement; drive the remaining `@ts-nocheck` files to zero and keep new code suppression-free; route shared API/SSE/error shapes through `packages/contracts`; add runtime validation for untrusted daemon inputs. |
...
| W3 | Partial | Migrate project-owned code to TypeScript | R1 | W2 for highest-value shared types | Daemon source and tests have most `@ts-nocheck` suppressions removed (including `memory.ts`, `memory-extractions.ts`, `workspace-contract.ts`, and `library-install.ts`). High-conflict daemon core files are deferred for follow-up to avoid blocking parallel PRs: `apps/daemon/src/server.ts`, `apps/daemon/src/agents.ts`, `apps/daemon/src/projects.ts`, `apps/daemon/src/runs.ts`, `apps/daemon/src/cli.ts`, and `apps/daemon/src/memory-llm.ts`. Remaining work is to migrate those files, restore full daemon typecheck coverage, and rerun `pnpm --filter @open-design/daemon typecheck`, `pnpm typecheck`, and `pnpm guard`. |
