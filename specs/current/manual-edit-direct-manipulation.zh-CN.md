...
协议增量:`od-edit-text-selection` 增 `format {bold,italic,underline,strike} | null`;`od-edit-apply-dom` 增 `op: replace|insert-after|insert-at-index|append-child|prepend-child|remove|apply-content`(默认 replace,向后兼容)。
