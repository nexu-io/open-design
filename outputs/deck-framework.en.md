# Deck framework directive — English original

> Source: `packages/contracts/src/prompts/deck-framework.ts`
> Rendering: `renderDeckFrameworkDirective('filesystem')`
> Snapshot: current `codex/system-prompt-optimization` working tree on 2026-07-24.
> Scope: the complete legacy V1 Deck framework directive retained for the `current` and `current_outcome` experiment / rollback variants. It is no longer the production default. When an active Deck skill already provides an `assets/template.html` seed, runtime composition suppresses the generic directive to avoid two competing frameworks; that skill's own `SKILL.md` and referenced resources remain separate context.
...
## Execution-profile difference

The directive above is the exact `filesystem` rendering used when project files are the source of truth. In `text_artifact` mode, only these three strings differ:

- Workflow step 7: `Emit one complete <artifact> block containing the deck HTML`
- Self-check heading: `Pre-emit self-check — run this BEFORE writing the <artifact> tag`
- Final action phrase: `emitting the artifact`
