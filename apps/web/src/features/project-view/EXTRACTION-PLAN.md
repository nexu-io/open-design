...
- **Status:** done. Landed as `useAmrBalanceGate(...)` / `useWiredAmrBalanceGate(...)` — took option (b) from this entry's own coupling note: the gate CHECK itself (the `await checkAmrBalanceGate()` call, its queue/park side effects, and the `messagesConversationIdRef`-guarded re-entry logic) is too entangled with `handleSend`'s own control flow (Cluster 17, not yet extracted) to move without touching that closure, so it stays inline exactly where it was. The hook owns everything that check reads/writes instead: `amrBalanceGateBlock`/`setAmrBalanceGateBlock`, `amrLowBalanceWarn`/`setAmrLowBalanceWarn`, `amrGateInFlightConversationsRef`, `amrGatePausedQueueConversationsRef`, `pendingAmrRetry`/`setPendingAmrRetry`, `handleSwitchToAmrAndRetry`, and the AMR-login-status poll effect that fires the armed retry once AMR is selected and signed in. `ProjectView.tsx`'s inline declarations at the state-block site (previously right after `chatSeed`) and at the post-`handleRetry` site (previously `pendingAmrRetry`/`handleSwitchToAmrAndRetry`/the poll `useEffect`) were both deleted; the orchestrator now destructures the same names from one `useWiredAmrBalanceGate(...)` call positioned right after `handleRetry` (Cluster 17, still inline) — the hook takes `handleRetry` as a param per the deps-bag pattern, so call-site ordering after `handleRetry`'s own definition is required and preserved. The still-inline gate-check block inside `handleSend`, the queue-auto-drain effect's paused-check, and the dialog JSX (`AmrBalanceDialog`/`AmrLowBalanceDialog`, Cluster 22 territory) all read/write the identical destructured names, so none of their ~15 call sites needed edits. Added a `fetchAmrLoginStatus(): Promise<{ loggedIn: boolean } | null>` port method (bound directly to the existing `providers/daemon.ts` `fetchVelaLoginStatus`, matching the `fetchRunStatus: fetchChatRunStatus` precedent — no new provider file needed) so the poll effect stays fetch-free per the guard; the port method's return type is an inline structural type rather than the imported `VelaLoginStatus`, since the guard also rejects `import type` from `providers/` outside `dependencies.ts`. `ProjectView.tsx`'s now-dead raw `fetchVelaLoginStatus`/`AmrWalletSnapshot` imports were removed (the gate-check block's own `AmrWalletSnapshot`-typed reads are inferred from `checkAmrBalanceGate()`'s return, no explicit import needed there). Added the new port method to all 9 existing slice test files' local `makePort()`/harness fakes (the same sync-drift risk flagged in Clusters 8/9/15/18's notes) plus 2 new `dependencies.test.ts` cases and a new `useAmrBalanceGate.test.tsx` (10 tests) covering the disabled/enabled switch-and-retry paths, the poll effect's not-yet-armed/wrong-agent/signed-in/timeout/teardown branches (via fake timers), and the dialog-state setters' identity/value behavior.
