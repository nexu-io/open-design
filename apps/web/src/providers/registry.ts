...
export type WebDeploymentInfo =
	ProjectDeploymentsResponse["deployments"][number];
...
export function isDeployProviderId(
	value: unknown,
): value is WebDeployProviderId {
	return (
		typeof value === "string" &&
		(DEPLOY_PROVIDER_IDS as readonly string[]).includes(value)
	);
}
...
export async function ensureDesignSystemWorkspace(
	...
	try {
		const resp = await fetch(
			`/api/design-systems/${encodeURIComponent(id)}/workspace`,
			{
				method: "POST",
			},
		);
...
export async function fetchDesignSystemGenerationJob(
	...
	try {
		const resp = await fetch(
			`/api/design-systems/generation-jobs/${encodeURIComponent(id)}`,
		);
		if (!resp.ok) return null;
		const json = (await resp.json()) as { job?: DesignSystemGenerationJob };
		return json.job ?? null;
...
export async function fetchDesignSystemRevisions(
	...
	try {
		const resp = await fetch(
			`/api/design-systems/${encodeURIComponent(id)}/revisions`,
		);
		if (!resp.ok) return [];
...
export async function startDesignSystemRevisionJob(
	...
	try {
		const resp = await fetch(
			`/api/design-systems/${encodeURIComponent(id)}/revision-jobs`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(input),
...
export async function startDesignSystemTokenContractRebuildJob(
	...
	try {
		const resp = await fetch(
			`/api/design-systems/${encodeURIComponent(id)}/token-contract/rebuild-jobs`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(input),
...
export async function fetchConnectorDiscovery(
	...
	if (connectorDiscoveryCache && !options.refresh)
		return connectorDiscoveryCache;
	if (connectorDiscoveryPromise && !options.refresh)
		return connectorDiscoveryPromise;
...
export async function fetchConnectorDetail(
	...
	try {
		...
		if (options.toolsLimit !== undefined)
			params.set("toolsLimit", String(options.toolsLimit));
		if (options.toolsCursor) params.set("toolsCursor", options.toolsCursor);
		const query = params.toString();
		const resp = await fetch(
			`/api/connectors/${encodeURIComponent(connectorId)}${query ? `?${query}` : ""}`,
		);
		if (!resp.ok) return null;
...
async function decodeConnectorError(resp: Response): Promise<string> {
	try {
		...
		return (
			...
			`Connector request failed (${resp.status})`
		);
	} catch {
...
export async function connectConnector(
	...
	try {
		...
		return {
			connector: json.connector ?? null,
			...(json.auth === undefined ? {} : { auth: json.auth }),
		};
	} catch (err) {
		renderConnectorAuthError(
			...
			err instanceof Error && err.message
				? err.message
				: "Could not start connector authentication.",
		);
		return {
			connector: null,
			error:
...
function openConnectorAuthRedirect(
...
function renderConnectorAuthLoading(
	authWindow: Window | null,
...
function renderConnectorAuthInfo(
	authWindow: Window | null,
	copy: { title: string; body: string },
): void {
	if (!authWindow) return;
...
function renderConnectorAuthRedirect(
	authWindow: Window,
	redirectUrl: string,
): void {
	try {
...
function renderConnectorAuthError(
	authWindow: Window | null,
	message: string,
): void {
	if (!authWindow) return;
...
export async function disconnectConnector(
	...
	try {
		const resp = await fetch(
			`/api/connectors/${encodeURIComponent(connectorId)}/connection`,
			{
				method: "DELETE",
			},
		);
...
export async function cancelConnectorAuthorization(
	connectorId: string,
): Promise<ConnectorDetail | null> {
	try {
		const resp = await fetch(
			`/api/connectors/${encodeURIComponent(connectorId)}/authorization/cancel`,
			{
				method: "POST",
			},
		);
...
export async function fetchLatestGithubReleaseInfo(): Promise<LatestGithubReleaseInfo | null> {
	try {
		...
		if (!resp.ok) return null;
		const json =
			(await resp.json()) as Partial<OpenDesignGithubLatestReleaseResponse>;
		if (typeof json.tag_name !== "string" || typeof json.html_url !== "string")
			return null;
		return {
...
export async function fetchDeployConfig(
	...
	try {
		const resp = await fetch(
			`/api/deploy/config${deployProviderQuery(providerId)}`,
		);
		if (!resp.ok) return null;
	...
	try {
		...
		if (!resp.ok) {
			...
			throw new Error(
				payload?.error?.message ||
					payload?.message ||
					`Could not save deploy config (${resp.status})`,
			);
		}
		return (await resp.json()) as WebDeployConfigResponse;
	} catch (err) {
	...
	try {
		...
		if (!resp.ok) {
			...
			throw new Error(
				payload?.error?.message ||
					payload?.message ||
					`Could not load Cloudflare zones (${resp.status})`,
			);
		}
...
export async function createSocialSharePayload(
	...
	if (!resp.ok) {
		...
		throw new Error(
			payload?.error?.message ||
				payload?.message ||
				`Share payload failed (${resp.status})`,
		);
	}
...
export async function updateLiveArtifact(
	...
	if (!artifact)
		throw new LiveArtifactRefreshError(
			"Update response did not include a live artifact.",
			resp.status,
		);
	return artifact;
}
...
export type LiveArtifactPreviewVariant =
	| "rendered"
	| "template"
	| "rendered-source";
...
export function liveArtifactPreviewUrl(
	...
	const variantQuery =
		variant === "rendered" ? "" : `&variant=${encodeURIComponent(variant)}`;
...
export async function renameProjectFile(
	...
	const resp = await fetch(
		`/api/projects/${encodeURIComponent(projectId)}/files/rename`,
		{
			method: "POST",
			headers: { "Content-Type": "application/json" },
