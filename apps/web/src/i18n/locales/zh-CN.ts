...
export const zhCN: Dict = {
  ...
  'settings.mcpCapabilitiesTitle': '你的代理能做什么',
  'settings.mcpCapabilityRead':
    '读取或搜索项目中的任何文件 (HTML, JSX, CSS, JSON, SVG, Markdown)。',
  'settings.mcpCapabilityPull':
    '一次调用即可拉取设计包：入口文件及其引用的所有 CSS 变量、组件和字体。',
  'settings.mcpCapabilityDefault':
    '默认使用你在 Open Design 中打开的项目和文件，这样你可以说"把这个加到我的应用里"而无需重新指定设计。',
  'settings.mcpRunningNote':
    'MCP 工具调用需要 Open Design 正在运行才能成功。如果你在打开 Open Design 之前启动了编码代理，请重启代理以便它能连接到正在运行的守护进程。',
  'settings.mcpRestartNote': '重启你的客户端以识别新服务器。',
  'settings.mcpRestartDetail':
    '大多数编辑器只在启动时加载 MCP 服务器。在 Cursor / VS Code / Antigravity / Windsurf 中，你可以从命令面板运行 `Developer: Reload Window` 来代替完全重启。Zed 和 Claude Code 需要退出并重新打开。',
