...
export const zhCN: Dict = {
  ...
  'settings.mediaProviderMiniMaxNote': '填 TokenPlan key 即可；Base URL 留空会使用 https://api.minimaxi.com/v1。只有代理或自定义网关才需要改 URL。',
  'settings.mediaProviderGoogleNote': '先配置 ~/.config/open-design/google-vertex-config.json 的 Vertex ADC。开启后只会在图片模型下拉显示 Google 图片模型；Veo / Lyria 还没接入真实生成。',
  'settings.mediaProviderExternalEnable': '在模型下拉中显示 Google 图片模型',
  'settings.mediaProviderEnabled': '已启用',
