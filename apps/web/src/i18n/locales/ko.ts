...
export const ko: Dict = {
  ...
  'useEverywhere.dialogAria': 'Open Design 어디서나 사용',
  'useEverywhere.kicker': '통합',
  'useEverywhere.title': 'Open Design을 어디서나 사용',
  'useEverywhere.subtitle': 'Open Design을 어떤 IDE, 에이전트, 스크립트에도 연결하세요. CLI, HTTP, MCP, Skills를 모두 지원합니다. “에이전트용 가이드 복사”를 눌러 Claude Code, Codex, Cursor, openclaw, Hermes에 붙여넣으면 설정을 바로 진행할 수 있습니다.',
  'useEverywhere.closeAria': '어디서나 사용 닫기',
  'useEverywhere.tabsAria': '통합 표면',
  'useEverywhere.handoffTitle': '한 번에 넘겨주기.',
  'useEverywhere.handoffBody': '에이전트가 바로 실행할 수 있는 구조화된 마크다운 가이드를 복사합니다. 설치, 확인, 사용까지 포함됩니다.',
  'useEverywhere.configureMcp': 'MCP 서버 설정',
  'useEverywhere.copyGuide': '에이전트용 가이드 복사',
  'useEverywhere.copySnippetAria': '스니펫 복사: {label}',
  'useEverywhere.copyFailed': '복사 실패',
  'useEverywhere.section.overview.tab': '개요',
  'useEverywhere.section.overview.heading': '에이전트가 있는 곳 어디서나 Open Design이 작동합니다',
  'useEverywhere.section.overview.intro': 'Open Design은 단순한 창이 아닙니다. 로컬 권한 daemon(`od`)과 Skills + Design-Systems + Atoms 레지스트리입니다. 내 컴퓨터에서 실행 중이면 Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, 직접 만든 스크립트 같은 어떤 코드 에이전트든 네 가지 교체 가능한 표면을 통해 생성을 실행하고, 프로젝트를 검사하고, 디자인 아티팩트를 만들 수 있습니다.',
  'useEverywhere.section.overview.bullet.cli': 'CLI: `od <command>`로 headless 스크립트, CI, 셸 자동화를 실행합니다.',
  'useEverywhere.section.overview.bullet.mcp': 'MCP 서버: Open Design을 Model Context Protocol 서버로 연결해 MCP 지원 에이전트가 스킬을 나열하고, 시나리오를 실행하고, 아티팩트를 읽을 수 있게 합니다.',
  'useEverywhere.section.overview.bullet.http': 'HTTP API: `http://127.0.0.1:7456/api/*` REST + SSE 엔드포인트입니다. 웹 UI가 사용하는 것과 같은 표면입니다.',
  'useEverywhere.section.overview.bullet.skills': 'Skills: Claude 호환 `SKILL.md` 팩입니다. Open Design 없이도 PATH에 있는 에이전트가 바로 호출할 수 있습니다.',
  'useEverywhere.section.overview.bullet.artifacts': '표준 아티팩트: daemon 시작 전에 Skills, 번들 기본 플러그인, 커뮤니티 플러그인 예시에서 실제 HTML 프로젝트를 시드합니다.',
  'useEverywhere.section.overview.snippet.start': 'daemon과 웹 UI를 로컬에서 시작',
  'useEverywhere.section.overview.snippet.health': '연결 가능한지 확인',
  'useEverywhere.section.overview.snippet.ingest': '시작 전에 표준 아티팩트 가져오기',
  'useEverywhere.section.overview.footer': 'daemon은 기본적으로 프로젝트 로컬 `./.od/`에 기록합니다. 여러 프로젝트에서 데이터를 공유하려면 `OD_DATA_DIR=~/.open-design`를 설정하세요.',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': '어떤 셸에서도 Open Design 제어',
  'useEverywhere.section.cli.intro': '`od` 바이너리는 daemon과 함께 제공되며 Claude Code / Codex가 생성을 실행할 때 사용하는 동일한 바이너리입니다. 대부분의 하위 명령은 로컬 daemon에 POST하는 얇은 클라이언트이므로 `pnpm tools-dev`로 실행했든 패키지 앱으로 실행했든 같은 방식으로 동작합니다.',
  'useEverywhere.section.cli.bullet.boot': '`od`(인자 없음): daemon을 시작하고 웹 UI를 엽니다.',
  'useEverywhere.section.cli.bullet.media': '`od media generate ...`: 통합 미디어 프로토콜로 이미지 / 비디오 / 오디오 바이트를 생성합니다.',
  'useEverywhere.section.cli.bullet.run': '`od run ...`: 프롬프트 + 스킬로 프로젝트 실행을 시작합니다.',
  'useEverywhere.section.cli.bullet.plugins': '`od plugin install <source>` / `od plugin apply <id>`: 커뮤니티 플러그인을 설치하고 적용합니다.',
  'useEverywhere.section.cli.bullet.registry': '`od skills list` / `od design-systems list`: 로컬에서 사용할 수 있는 항목을 확인합니다.',
  'useEverywhere.section.cli.bullet.doctor': '`od status` / `od doctor`: daemon 상태를 확인하고 PATH에 있는 에이전트 CLI를 감지합니다.',
  'useEverywhere.section.cli.snippet.media': '설정된 미디어 제공자를 통해 이미지 생성',
  'useEverywhere.section.cli.snippet.run': '시나리오 플러그인을 headless로 실행하고 이벤트를 JSON lines로 스트리밍',
  'useEverywhere.section.cli.snippet.inventory': '로컬에서 사용할 수 있는 스킬과 디자인 시스템 목록 확인',
  'useEverywhere.section.cli.snippet.seeded': 'CLI로 시드된 아티팩트 확인',
  'useEverywhere.section.cli.snippet.doctor': '환경과 감지된 에이전트 확인 (Claude, Codex, Cursor 등)',
  'useEverywhere.section.cli.footer': '모든 하위 명령은 `--daemon-url http://127.0.0.1:<port>`를 받아 특정 실행 중인 daemon을 대상으로 삼을 수 있습니다. 테스트용 두 번째 샌드박스 인스턴스에 유용합니다.',
  'useEverywhere.section.mcp.tab': 'MCP 서버',
  'useEverywhere.section.mcp.heading': '어떤 코딩 에이전트에도 Open Design을 MCP 서버로 노출',
  'useEverywhere.section.mcp.intro': 'Open Design은 Model Context Protocol 서버(`od mcp`)를 제공합니다. Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, Hermes 같은 MCP 지원 클라이언트가 셸 명령을 직접 실행하지 않고도 Open Design 도구(스킬 목록, 미리보기 렌더, 미디어 생성, 플러그인 실행)를 발견할 수 있습니다. daemon은 주요 클라이언트별로 바로 붙여넣을 수 있는 설치 스니펫을 `GET /api/mcp/install-info`로 제공합니다.',
  'useEverywhere.section.mcp.bullet.stdio': 'Stdio 전송: 추가 포트 없이 클라이언트가 `od mcp`를 직접 실행합니다.',
  'useEverywhere.section.mcp.bullet.autodiscover': 'sidecar로 실행되면 로컬 IPC 상태 소켓을 통해 live daemon URL을 자동으로 찾습니다.',
  'useEverywhere.section.mcp.bullet.daemonUrl': '일반 설치에서는 `--daemon-url http://127.0.0.1:<port>`로 fallback하여 MCP 프로세스가 항상 실행 중인 daemon을 찾게 합니다.',
  'useEverywhere.section.mcp.bullet.dataDir': '생성된 MCP 프로세스가 daemon과 같은 위치에 기록하도록 `OD_DATA_DIR`를 고정합니다. 패키지된 macOS 앱 번들에서 EPERM을 피합니다.',
  'useEverywhere.section.mcp.snippet.generic': '일반 MCP 클라이언트 설정 (Cursor, Claude Code, Codex 등에서 동작)',
  'useEverywhere.section.mcp.snippet.installInfo': '또는 daemon에 현재 설치에 맞는 스니펫 요청',
  'useEverywhere.section.mcp.snippet.liveArtifacts': '라이브 아티팩트 MCP 변형 (대시보드 읽기 및 새로고침)',
  'useEverywhere.section.mcp.footer': 'Open Design 앱에서 Settings → Integrations를 열면 JSON을 직접 편집하지 않고도 Cursor, Claude Code, Antigravity, VS Code용 클라이언트별 설치 명령을 복사할 수 있습니다.',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': '웹 UI가 사용하는 것과 같은 REST + SSE 표면',
  'useEverywhere.section.http.intro': '로컬 daemon은 `http://127.0.0.1:7456`에서 HTTP API를 제공합니다(포트 설정 가능). 웹 UI가 호출하는 모든 엔드포인트를 스크립트에서도 사용할 수 있습니다. 스트리밍 엔드포인트(채팅 턴, 프로젝트 실행)는 `@open-design/contracts`의 계약 타입에 맞춘 Server-Sent Events를 내보냅니다.',
  'useEverywhere.section.http.bullet.health': '`GET /api/health`: daemon 생존 확인.',
  'useEverywhere.section.http.bullet.registries': '`GET /api/skills`와 `GET /api/design-systems`: 사용 가능한 레지스트리.',
  'useEverywhere.section.http.bullet.projects': '`GET /api/projects`와 `POST /api/projects`: 프로젝트 목록 조회 및 생성(POST는 프로젝트와 첫 대화를 반환).',
  'useEverywhere.section.http.bullet.chat': '`GET /api/projects/:id/chat`: 대화의 에이전트 이벤트 SSE 스트림.',
  'useEverywhere.section.http.bullet.plugins': '`POST /api/plugins/:id/apply`: 설치된 플러그인을 바인딩하고 렌더링된 예시 쿼리와 입력값을 받습니다.',
  'useEverywhere.section.http.bullet.agents': '`GET /api/agents`: PATH에서 감지된 코드 에이전트 CLI.',
  'useEverywhere.section.http.snippet.skills': '설치된 스킬 목록 보기 (에이전트가 템플릿으로 사용)',
  'useEverywhere.section.http.snippet.project': '프롬프트에서 프로젝트 생성 (전체 서버 측 흐름)',
  'useEverywhere.section.http.snippet.stream': '채팅 턴 스트리밍 (SSE: 각 줄은 JSON-Lines 호환)',
  'useEverywhere.section.http.footer': '모든 요청/응답의 순수 TypeScript 타입은 `@open-design/contracts`에 있습니다. 생성기를 연결하지 않아도 스크립트에서 import해 자동완성을 받을 수 있습니다.',
  'useEverywhere.section.skills.tab': 'Skills & headless',
  'useEverywhere.section.skills.heading': 'Open Design이 실행 중이 아니어도 어떤 에이전트든 쓸 수 있는 Skills',
  'useEverywhere.section.skills.intro': 'Skill은 Claude 호환 `SKILL.md`(YAML front matter + 본문)가 들어 있는 디렉터리입니다. Open Design은 `od:` 네임스페이스(`mode`, `preview`, `design_system`, `inputs` 등)로 형식을 확장해 같은 아티팩트를 Open Design 내부와 Claude Code, Codex, openclaw, Hermes 같은 일반 에이전트 양쪽에서 사용할 수 있게 합니다. 발견 순서가 있어 프로젝트가 자체 스킬을 우선 적용할 수 있습니다.',
  'useEverywhere.section.skills.bullet.discovery': '발견 순서: `./.claude/skills/` → `./skills/` → `~/.claude/skills/` (프로젝트가 우선).',
  'useEverywhere.section.skills.bullet.symlink': '하나의 공유 스킬을 여러 프로젝트에 심볼릭 링크해 복사 없이 공유할 수 있습니다.',
  'useEverywhere.section.skills.bullet.declare': '각 스킬은 커넥터, atoms, 디자인 시스템 요구사항, 갤러리용 `preview` 예시 출력을 선언할 수 있습니다.',
  'useEverywhere.section.skills.bullet.headless': 'Headless: PATH에 `od`가 있는 에이전트는 `od skills list`를 호출한 뒤 어떤 스킬이든 실행할 수 있습니다. 읽기 전용 흐름에서는 daemon이 선택 사항입니다.',
  'useEverywhere.section.skills.bullet.seed': '`pnpm seed:test-projects`는 기본 플러그인 예시와 커뮤니티 플러그인 예시로 같은 아티팩트 형태를 검증한 뒤 결과 `index.html` 프로젝트를 재사용 가능한 테스트 데이터로 저장합니다.',
  'useEverywhere.section.skills.snippet.minimal': '최소 SKILL.md (Claude 호환 front matter + Open Design 확장)',
  'useEverywhere.section.skills.snippet.symlink': '공유 스킬을 프로젝트에 심볼릭 링크 (cc-switch 스타일)',
  'useEverywhere.section.skills.snippet.list': 'Headless: daemon이 현재 보는 스킬 목록',
  'useEverywhere.section.skills.snippet.fixture': 'Headless 아티팩트 fixture 번들',
  'useEverywhere.section.skills.footer': '스펙: `docs/skills-protocol.md`와 `docs/agent-adapters.md`에서 전체 adapter 표면(Claude Code, Codex, Cursor, MCP 지원 에이전트, BYOK API fallback)과 adapter별 스킬 주입 전략을 다룹니다.',
