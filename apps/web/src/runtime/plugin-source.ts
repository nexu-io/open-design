...
const MARKETING_AX_REPO_URL = 'https://github.com/marketing-ax/marketing-ax';
const MARKETING_AX_REPO_LABEL = 'marketing-ax/marketing-ax';
...
export function derivePluginSourceLinks(
  ...
  const authorName = typeof author.name === 'string' && author.name.trim().length > 0
    ? author.name.trim()
    : null;
  const authorProfileUrl = officialBundled ? MARKETING_AX_REPO_URL : safeHttpUrl(author.url);
  const homepageUrl = officialBundled ? MARKETING_AX_REPO_URL : safeHttpUrl(homepageRaw);
  ...
  if (record.sourceKind === 'github') {
    ...
    sourceUrl = MARKETING_AX_REPO_URL;
    sourceLabel = MARKETING_AX_REPO_LABEL;
  } else {
