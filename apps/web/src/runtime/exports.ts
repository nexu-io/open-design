...
function notifyExportSuccess(filename: string): void {
...
export function exportAsHtml(html: string, title: string): void {
  ...
  const filename = `${safeFilename(title, 'artifact')}.html`;
...
export function exportAsMd(source: string, title: string): void {
  const filename = `${safeFilename(title, 'artifact')}.md`;
...
export async function exportProjectAsPdf(opts: {
  ...
  try {
    ...
    void showCompletionNotification({
      status: 'succeeded',
      title: 'Export Complete',
      body: 'PDF export has been initiated.',
    });
    return 'desktop';
  } catch (err) {
...
export function exportAsJsx(
  ...
  const filename = `${safeFilename(title, 'component')}${extension}`;
...
export function exportReactComponentAsHtml(source: string, title: string): void {
  ...
  const filename = `${safeFilename(title, 'component')}.html`;
  const blob = new Blob([doc], { type: 'text/html;charset=utf-8' });
  triggerDownload(blob, filename);
  notifyExportSuccess(filename);
}
...
export async function exportProjectAsZip(opts: {
  ...
  try {
    ...
    void showCompletionNotification({
      status: 'succeeded',
      title: 'Export Complete',
      body: 'Project archive has been downloaded.',
    });
  } catch (err) {
...
export function exportAsPdf(
  ...
  void showCompletionNotification({
    status: 'succeeded',
    title: 'Export Complete',
    body: 'PDF export has been initiated. Check your popup window.',
  });
}
