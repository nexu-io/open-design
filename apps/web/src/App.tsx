...
} from "./components/NewProjectPanel";
...
function normalizeSavedComposioConfig(
	config: AppConfig["composio"],
): AppConfig["composio"] {
...
function clearStaleAmrModelChoiceOnProfileChange(
	...
	if (!nextChoice || !sameAgentModelChoice(previousChoice, nextChoice))
		return next;
...
export async function persistComposioConfigChange(
	...
	sync: (
		config: AppConfig["composio"],
	) => Promise<boolean> = syncComposioConfigToDaemon,
): Promise<AppConfig> {
 ...
 * gesture commits it.
 */
export function isAutosaveDraftOnlyChange(
	next: AppConfig,
	last: AppConfig,
): boolean {
	return (
...
export function resolveSettingsCloseConfig(
	...
	return base.onboardingCompleted
		? base
...
function AppInner() {
	...
	const [settingsInitialSection, setSettingsInitialSection] =
		useState<SettingsSection>("execution");
	const [settingsHighlight, setSettingsHighlight] =
		useState<SettingsHighlight>(null);
	const [integrationInitialTab, setIntegrationInitialTab] =
		useState<IntegrationTab>("mcp");
	...
	const [pendingDesignSystemRevisionJobs, setPendingDesignSystemRevisionJobs] =
		useState<Record<string, DesignSystemGenerationJob>>({});
	...
	>(null);
	const [daemonMediaProvidersFetchState, setDaemonMediaProvidersFetchState] =
		useState<"idle" | "ok" | "error">("idle");
	const [mediaProvidersNotice, setMediaProvidersNotice] = useState<
		string | null
	>(null);
	...
	const clearLocalProject = useCallback(
		(projectId: string, options?: { deleted?: boolean }) => {
	...
	const reconcileFetchedProjects = useCallback(
		(list: Project[], request: ProjectListRequest) => {
			...
			if (request.generation < latestAppliedProjectListGenerationRef.current) {
				const visibleList =
					locallyDeletedProjectIds.size > 0
						? list.filter(
								(project) => !locallyDeletedProjectIds.has(project.id),
							)
						: list;
				if (visibleList.length === 0) return false;
				const hydratableProjects = visibleList.filter((project) =>
	...
	const [amrLoginStatus, setAmrLoginStatus] = useState<VelaLoginStatus | null>(
		null,
	);
	...
	useEffect(() => {
		...
		const byokConfigured = (() => {
			...
			return Object.values(protocols).some((cfg) =>
	...
	useEffect(() => {
		analytics.setUserId(
			amrLoginStatus?.loggedIn === true
				? (amrLoginStatus.user?.id ?? null)
				: null,
		);
	}, [analytics.setUserId, amrLoginStatus]);

	const handleAmrLoginStatusChange = useCallback(
	...
	useEffect(() => {
		...
		(async () => {
			...
			void Promise.all([
				...
				([daemonConfig, daemonComposioConfig, daemonMediaProvidersResult]) => {
					if (cancelled) return;
					...
					const migratedLocalMediaProviders =
						shouldSyncLocalMediaProvidersToDaemon(
	 ...
	 */
	const handleConfigPersist = useCallback(
		async (next: AppConfig, options?: { forceMediaProviderSync?: boolean }) => {
	...
	useEffect(() => {
		...
		return () =>
			window.removeEventListener(
				APP_CONFIG_CHANGED_EVENT,
				handleAppConfigChanged,
			);
		...
		async (
			...
			if (!result) {
				trackProjectCreateResult(
					analytics.track,
					{
						...
						project_id: null,
						project_kind: projectKindToTracking(
							kind,
							input.metadata?.videoModel,
						),
						fidelity,
			...
			if (userWorkingDir) {
				try {
					...
					console.warn(
						"Failed to set working directory for new project",
						userWorkingDir,
						err,
					);
			...
			if (!workingDirHandoffFailed && pendingFiles.length > 0) {
				...
				if (partial) {
					console.warn(
						"Some Home attachments failed to upload",
						uploadResult.failed,
					);
				}
			...
			if (
				...
				(derivedPendingPrompt !== undefined ||
	...
	const handleImportClaudeDesign = useCallback(
		async (file: File): Promise<ImportClaudeDesignOutcome> => {
			try {
	...
	const handleImportFolderResponse = useCallback(
	...
	const handleOpenLiveArtifact = useCallback(
		(projectId: string, artifactId: string) => {
			navigate({
				kind: "project",
				projectId,
				fileName: liveArtifactTabId(artifactId),
			});
		},
		[],
	);
	...
	const handleDesignSystemRevisionJobConsumed = useCallback(
		(designSystemId: string, jobId: string) => {
			setPendingDesignSystemRevisionJobs((current) => {
	...
	useEffect(() => {
		...
		(async () => {
			...
			if (!applied) return;
			const fetchedProject = locallyDeletedProjectIdsRef.current.has(
				route.projectId,
			)
				? undefined
	...
	if (route.kind === "marketplace") {
		...
		appMain = (
			<DesignSystemDetailView
				...
				initialRevisionJob={
					pendingDesignSystemRevisionJobs[route.designSystemId] ?? null
				}
		...
		appMain = (
			<ProjectView
				...
				routeConversationId={
					route.kind === "project" ? route.conversationId : null
				}
				config={config}
				agents={agents}
