...
function buildFallbackDesignMdFromState(state: SetupState): string {
  ...
  const overview =
    ...
    || state.notes.trim()
    || sourceNotes
    || 'Design system generated from source material supplied in LeastGen Studio.';
  return [
    '---',
...
function buildCreationAgentPrompt(
  ...
  return [
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that LeastGen Studio attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real LeastGen Studio design-system package.',
