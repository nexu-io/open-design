...
export function EntryView({
  ...
  const updateConnector = useCallback((connector: ConnectorDetail | null) => {
    if (!connector) return;
    setConnectors((curr) => {
      const index = curr.findIndex((candidate) => candidate.id === connector.id);
      if (index < 0) return sortConnectorsForDisplay([...curr, connector]);
      const next = [...curr];
      next[index] = connector;
      return sortConnectorsForDisplay(next);
    });
  }, []);
            ...
            <>
              ...
              {topTab === 'connectors' ? (
                <ConnectorsTab
                  ...
                  toolsLoading={connectorsLoading}
                  toolsLoaded={!connectorsLoading}
          ...
          {needsComposioKey ? (
            ...
            >
              <div className="connector-gate-card">
                ...
                >
                  {t('settings.orbit.gateActionNoKey')}
                </button>
              </div>
            </div>
          ) : null}
