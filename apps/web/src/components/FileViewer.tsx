...
import {
  ...
  setCachedPreviewViewport,
  previewViewportStateKey,
  useWiredViewerToolbarMenus,
  ViewerToolbar,
  MarkdownViewer,
  useWiredDeployFlow,
  DeployModal,
...
// Re-export the slice's public inspect-override API from the orchestrator so
// prior consumers (and the FileViewer.* suite) keep importing from this file.
export {
  applyInspectOverridesToSource,
  parseInspectOverridesFromSource,
  serializeInspectOverrides,
...
// MarkdownViewer (mode/autosave/shiki-highlight/code-block-copy/scroll-sync)
...
function HtmlViewer({
  ...
  const [previewViewport, setPreviewViewportState] = useState<PreviewViewportId>(
    () => getCachedPreviewViewport(fileViewportKey) ?? 'desktop',
  );
  const setPreviewViewport = useCallback((viewport: PreviewViewportId) => {
    setCachedPreviewViewport(fileViewportKey, viewport);
  ...
  useEffect(() => {
    setPreviewViewportState(getCachedPreviewViewport(fileViewportKey) ?? 'desktop');
  }, [fileViewportKey]);
  ...
  const {
    ...
    t,
    analytics,
    resetCopiedDeployLink: deployLinkCopy.resetCopiedDeployLink,
    closeDeployMenu: () => setDeployMenuOpen(false),
  });
  ...
  return (
    <div className={`viewer html-viewer${inTabPresent ? ' is-tab-present' : ''}`}>
      <ViewerToolbar
        ...
        setToolbarMoreOpen={setToolbarMoreOpen}
        toolbarMoreRef={toolbarMoreRef}
        setVersionModalOpen={setVersionModalOpen}
        reloadHtmlPreview={reloadHtmlPreview}
        fireArtifactToolbarClick={fireArtifactToolbarClick}
        selectMode={selectMode}
        versioningAvailable={versioningAvailable}
        source={source}
        showPreviewToolbarControls={showPreviewToolbarControls}
        previewViewport={previewViewport}
        setPreviewViewport={setPreviewViewport}
        showDeckNavigation={showDeckNavigation}
        slideState={slideState}
        postSlide={postSlide}
        handleCopyScreenshot={handleCopyScreenshot}
        boardMode={boardMode}
        commentCreateMode={commentCreateMode}
        boardTool={boardTool}
        drawOverlayOpen={drawOverlayOpen}
        manualEditMode={manualEditMode}
        activateCommentTool={activateCommentTool}
        activateDrawTool={activateDrawTool}
        activateManualEditTool={activateManualEditTool}
        activateCommentCreateTool={activateCommentCreateTool}
        visibleSideCommentsCount={visibleSideComments.length}
      />
            ...
            <div className="chrome-file-action-menus" ref={shareRef}>
              {canShare ? (
                <ShareMenu
                  ...
                  onToggleMenu={openDeployMenu}
                  onCloseMenu={() => setDeployMenuOpen(false)}
                  ...
                  onShareGuideToast={(message) => setShareGuideToast(message)}
                  onOpenInNewTab={(url) => window.open(url, '_blank', 'noopener')}
                  onOpenDeployModal={(providerId) => {
                    void openDeployModal(providerId as WebDeployProviderId);
                  }}
                  ...
                  socialShareMenuLabel={socialShareMenuLabel}
                />
              ) : null}
              {canDownload ? (
      ...
      {deployModalOpen && typeof document !== 'undefined' ? (
        <DeployModal
          t={t}
          onClose={closeDeployModal}
          deployModalKicker={deployModalKicker}
          deployModalTitle={deployModalTitle}
          deployModalSubtitle={deployModalSubtitle}
          activeProjectSocialShare={activeProjectSocialShare}
          socialShareBlockedState={socialShareBlockedState}
          socialShareDisplayUrl={socialShareDisplayUrl}
          socialShareUnavailableMessage={socialShareUnavailableMessage}
          socialShareBlockedDeployment={socialShareBlockedDeployment}
          deployLinkCopy={deployLinkCopy}
          ...
          onChangeDeployProvider={(nextProviderId) => {
            void changeDeployProvider(nextProviderId as WebDeployProviderId);
          }}
          deployProvider={deployProvider}
          deployProviderLabel={deployProviderLabel}
          deployTokenInputRef={deployTokenInputRef}
          deployToken={deployToken}
          setDeployToken={setDeployToken}
          savingDeployConfig={savingDeployConfig}
          onSaveDeployConfig={() => {
          ...
          cloudflareDomainPrefix={cloudflareDomainPrefix}
          setCloudflareDomainPrefix={setCloudflareDomainPrefix}
          cloudflareZonesLoading={cloudflareZonesLoading}
          onLoadCloudflareZones={() => {
            void loadCloudflareZones();
          }}
          cloudflareZoneId={cloudflareZoneId}
          setCloudflareZoneId={setCloudflareZoneId}
          cloudflareZones={cloudflareZones}
          cloudflareZonesError={cloudflareZonesError}
          cloudflareHostnamePreview={cloudflareHostnamePreview}
          teamId={teamId}
          setTeamId={setTeamId}
          teamSlug={teamSlug}
          setTeamSlug={setTeamSlug}
          deployError={deployError}
          deployResultCards={deployResultCards}
