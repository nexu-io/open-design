...
// pdfjs-dist is loaded lazily inside PdfCanvasPage. Importing it at
// module top-level evaluates `display/canvas.js`, which references
// `DOMMatrix` and breaks Vitest's jsdom environment (and SSR).
let pdfjsModulePromise: Promise<typeof pdfjsLib> | null = null;
async function loadPdfjs(): Promise<typeof pdfjsLib> {
  if (!pdfjsModulePromise) {
    pdfjsModulePromise = import('pdfjs-dist').then((m) => {
      // pdfjs needs a separate worker bundle; we ship it under
      // /public so it's served same-origin without bundler
      // url-import gymnastics.
...
function PdfCanvasPage({
  ...
  // Load the PDF document once per URL. pdfjs is lazy-imported here
  // (rather than at module scope) so jsdom-based test environments
  // and SSR builds don't hit `display/canvas.js`'s DOMMatrix
  // reference.
  useEffect(() => {
    let cancelled = false;
    ...
    void (async () => {
      ...
      if (cancelled) return;
      task = lib.getDocument({ url: pdfUrl, withCredentials: true });
      try {
