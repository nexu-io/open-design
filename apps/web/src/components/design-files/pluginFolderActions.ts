...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the LeastGen Studio community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic LeastGen Studio CLI workflow from the current project workspace:',
    '',
...
// LeastGen Studio issue URL and belongs to the "LeastGen Studio PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an LeastGen Studio issue URL and belongs to the "LeastGen Studio PR" button.',
    '',
    'Run this deterministic LeastGen Studio CLI workflow from the current project workspace:',
    '',
