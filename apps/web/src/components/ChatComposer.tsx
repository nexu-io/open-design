...
const COMPOSER_DRAFT_RENDER_DELAY_MS = 80;
const COMPOSER_DRAFT_SAVE_DELAY_MS = 120;
...
export const ChatComposer = forwardRef<ChatComposerHandle, Props>(
  function ChatComposer(
    ...
    const draftRenderTimerRef = useRef<number | null>(null);
    const draftSaveTimerRef = useRef<number | null>(null);
    const paletteUpdateTimerRef = useRef<number | null>(null);
    const latestDraftSaveRef = useRef<{ key: string | undefined; draft: string }>({
      key: draftStorageKey,
      draft,
    });
    ...
    useEffect(() => {
      latestDraftSaveRef.current = { key: draftStorageKey, draft: draftRef.current };
    }, [draftStorageKey]);
    ...
    useEffect(
      () => () => {
        ...
        if (draftRenderTimerRef.current !== null && typeof window !== 'undefined') {
    ...
    const composerMentionEntities = useMemo(
      ...
      [connectors, enabledMcpServers, pluginsForComposer, skills, staged],
    );
    const deferredDraft = useDeferredValue(draft);
    const composerMentionParts = useMemo(
      () => buildInlineMentionParts(deferredDraft, composerMentionEntities),
      [composerMentionEntities, deferredDraft],
    );
    ...
    function pickSlash(cmd: SlashCommand) {
      ...
      const before = currentDraft.slice(0, slash.cursor);
      const after = currentDraft.slice(slash.cursor);
    ...
    function commitDraftRender(value: string): void {
      if (draftRenderTimerRef.current !== null && typeof window !== 'undefined') {
        window.clearTimeout(draftRenderTimerRef.current);
        draftRenderTimerRef.current = null;
      }
    ...
    function deferDraftRender(): void {
      if (typeof window === 'undefined') {
        setDraft(draftRef.current);
        return;
      }
      if (draftRenderTimerRef.current !== null) {
        window.clearTimeout(draftRenderTimerRef.current);
      }
      draftRenderTimerRef.current = window.setTimeout(() => {
        draftRenderTimerRef.current = null;
        setDraft(draftRef.current);
      }, COMPOSER_DRAFT_RENDER_DELAY_MS);
    }

    function scheduleDraftSave(value: string): void {
      latestDraftSaveRef.current = { key: draftStorageKey, draft: value };
        ...
        void (async () => {
          ...
          try {
            ...
            if (detail.action === 'send') {
              ...
              const prompt = [draftRef.current.trim(), detail.note].filter(Boolean).join('\n');
    ...
    function handleChange(e: React.ChangeEvent<HTMLTextAreaElement>) {
      ...
      if (typeof window !== 'undefined') {
        if (paletteUpdateTimerRef.current !== null) {
          window.clearTimeout(paletteUpdateTimerRef.current);
        }
        paletteUpdateTimerRef.current = window.setTimeout(updatePalettes, 0);
      } else {
        updatePalettes();
      }
    }
    ...
    const filteredFiles = mention ? firstMatchingProjectFiles(projectFiles, deferredMentionQuery, 12) : [];
    const filteredPlugins = mention
      ? pluginsForComposer
          .filter((p) => {
            if (!deferredMentionQuery) return true;
            return (
              p.title.toLowerCase().includes(deferredMentionQuery) ||
              p.id.toLowerCase().includes(deferredMentionQuery) ||
              (p.manifest?.description ?? '').toLowerCase().includes(deferredMentionQuery) ||
              (p.manifest?.tags ?? []).join(' ').toLowerCase().includes(deferredMentionQuery)
            );
          })
          .slice(0, 8)
      : [];
    ...
    const filteredSkills = mention
      ? skills
          ...
          .filter((s) => skillMatchesQuery(s, deferredMentionQuery))
          .sort((a, b) => skillMentionRank(a, deferredMentionQuery) - skillMentionRank(b, deferredMentionQuery))
      : [];
    const hasComposerPayload =
      hasDraftText || staged.length > 0 || currentCommentAttachments().length > 0;
...
function pruneStagedSkillsForDraft(prev: SkillSummary[], draft: string): SkillSummary[] {
  if (prev.length === 0) return prev;
  if (!draft.includes('@')) return [];

  const next = prev.filter((skill) =>
    new RegExp(`(^|\\s)@${escapeRegExp(skill.id)}(\\s|$)`).test(draft),
  );
  return next.length === prev.length ? prev : next;
}

function firstMatchingProjectFiles(
  files: ProjectFile[],
  query: string,
  limit: number,
): ProjectFile[] {
