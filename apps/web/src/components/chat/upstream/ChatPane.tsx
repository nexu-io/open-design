...
import { toHostMessage } from './types';
import { QuoteBar } from '../QuoteBar';
import { QueuedSendStack } from '../QueuedSendStack';
import { ChatScrollEdge } from '../ChatScrollEdge';
...
import historyStyles from '../ConversationHistoryDock.module.css';
import { hasOdCard, OD_NEXT_STRATEGY_ID, type ProjectMediaTask } from './contracts';
...
import { fetchProjectMediaTasks, projectRawUrl } from './host-api';
...
} from './components/NextStepActions';
...
import { ChatFileIcon, QueueTrashIcon } from '../primitives/icons';
...
import { ExportLogsAction } from '../ExportLogsAction';
...
interface Props {
  ...
  // Identifies the conversation represented by the live message array.
...
/** 使用用户提供的 discuss-line.svg，沿用面板头的尺寸与文字颜色。 */
...
export function ChatPane({
  ...
  const openAmrPlans = useCallback((entrySource: 'chat_error_upgrade' | 'chat_upgrade_card') => {
    const attribution = recordAmrEntry(analytics.track, entrySource === 'chat_upgrade_card' ? 'chat_error_upgrade' : entrySource, new Date(), {
  ...
  const recoveryAnalyticsProps = useCallback((
    ...
    const task = buildRecoveryTaskAnalytics(displayMessages.map(toHostMessage), toHostMessage(assistantMessage), actionType);
    return {
