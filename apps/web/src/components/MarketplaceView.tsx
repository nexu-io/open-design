...
export function MarketplaceView() {
  ...
  const trustLabel = (trust: Marketplace['trust'] | InstalledPluginRecord['trust']) => {
    if (trust === 'official') return t('trust.official');
    if (trust === 'bundled') return t('trust.official');
    if (trust === 'trusted') return t('trust.trusted');
    return t('trust.restricted');
  };
    ...
    <div className="marketplace-view" data-testid="marketplace-view">
      ...
      <section className="marketplace-view__grid" data-testid="marketplace-grid">
        {visible.length === 0 && !loading ? (
          <div className="marketplace-view__empty">
            {t('marketplace.emptyBeforeCommand')} <code>od plugin install &lt;source&gt;</code>{' '}
            {t('marketplace.emptyAfterCommand')}
          </div>
        ) : null}
        {visible.map((p) => (
          <button
          ...
          >
            ...
            <div className="marketplace-view__card-meta">
              ...
              <span>{t('marketplace.trustMeta', { trust: trustLabel(p.trust) })}</span>
      ...
      <section className="marketplace-view__catalogs" data-testid="marketplace-catalogs">
        ...
        {marketplaces.length === 0 ? (
          <div>
            {t('marketplace.catalogsEmptyBeforeCommand')} <code>od marketplace add &lt;url&gt;</code>.
          </div>
        ) : (
          <ul>
            {marketplaces.map((m) => (
              <li key={m.id}>
                ...
                <span className="marketplace-view__catalog-trust">
                  {t('marketplace.trustMeta', { trust: trustLabel(m.trust) })}
                </span>
                {' · '}
                <a href={m.url} target="_blank" rel="noreferrer">{m.url}</a>
                {' · '}
                {t('marketplace.pluginCount', { count: m.manifest.plugins?.length ?? 0 })}
              </li>
            ))}
          </ul>
        )}
