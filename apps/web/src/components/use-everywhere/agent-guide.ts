...
// everything it needs to install LeastGen Studio, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional LeastGen Studio version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# LeastGen Studio — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants LeastGen Studio wired into your workflow. LeastGen Studio is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported LeastGen Studio version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the LeastGen Studio app (packaged).');
  lines.push('');
  if (installInfo) {
