	...
	function McpClientSection({ onServersChanged, onDirtyChange }, ref) {
		...
		useEffect(() => {
			...
			void (async () => {
				...
				if (!data) {
					setError(
						"Could not reach the local daemon. Make sure Open Design is running, then reopen this panel.",
					);
					setLoaded(true);
					return;
				}
		...
		const save = async (): Promise<boolean> => {
			...
			if (!data) {
				setError(
					"Save failed. Check that the daemon is running and try again.",
				);
				return false;
			}
			const fresh = rowsFromServers(data.servers);
				...
				{rows.length === 0 ? (
					<div className="empty-card">
						...
						<p className="hint">
							...
							Pages, GitHub, Filesystem…) or set up a custom stdio / HTTP
							server.
						</p>
					</div>
				) : (
					<div className="mcp-rows">
						{rows.map((row, idx) => (
							<McpRow
								...
								onMoveDown={
									idx < rows.length - 1 ? () => moveRow(idx, 1) : undefined
								}
							/>
						))}
					</div>
...
function PickerPanel({
	...
	const renderGroups = CATEGORY_ORDER.map((cat) => {
		...
		const defaultOpen =
			...
			cat.id === "web-capture";
		return (
			<details key={cat.id} className="mcp-picker-group" open={defaultOpen}>
...
function PickerCard({ tpl, onPick }: { tpl: McpTemplate; onPick: () => void }) {
	return (
			...
			<div className="mcp-row-head">
				...
				<div className="mcp-row-actions">
					...
					<button
						...
						aria-label={
							expanded ? "Collapse this MCP server" : "Expand this MCP server"
						}
				...
				<>
						...
						<details className="mcp-row-info">
							...
							<div className="mcp-row-info-body">
								{template.description ? (
									<p className="mcp-row-info-desc hint">
										{template.description}
									</p>
								) : null}
								{template.example ? (
									...
									>
										...
										<span className="mcp-row-info-example-text">
											"{template.example}"
										</span>
									</p>
								) : null}
					...
					{isHttpLike && row._isNew ? (
						<div className="mcp-oauth-hint hint">
							Save first, then click <strong>Connect</strong> to grant Open
							Design access via the provider's OAuth flow.
						</div>
					) : null}
					...
					<div className="mcp-row-grid">
						...
						<label className="mcp-row-field">
							...
							<select
								...
								onChange={(e) =>
									onChange({
										transport: e.target.value as DraftRow["transport"],
									})
								}
							>
					...
					{row.transport === "stdio" ? (
						<>
							...
							>
								...
								>
									<div className="mcp-json-helper-toggle-content">
										<span className="mcp-json-helper-eye">
											<Icon name="eye" />
										</span>

										<span className="mcp-json-helper-toggle-text">
											Need help? Map your MCP server's JSON config using the
											example below.
										</span>
									</div>
								...
								{showMcpExample && (
									<div className="mcp-json-helper-example">
...
function McpOAuthControl({ serverId }: { serverId: string }) {
	...
	const [busy, setBusy] = useState<
		"idle" | "starting" | "awaiting" | "disconnecting" | "refreshing"
	>("idle");
	const [error, setError] = useState<string | null>(null);
	...
	const expiresLabel =
		status?.expiresAt && status.expiresAt > 0
			...
			: null;
	const isAwaiting =
		busy === "awaiting" || (Boolean(pendingAuthUrl) && !connected);
				...
				{connected ? (
					...
					<>
						...
						<span>
							...
							<span className="hint">
								Click Connect to grant Open Design access via the provider's
								OAuth flow.
							</span>
						</span>
					</>
