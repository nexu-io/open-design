    ...
    async (
      ...
      if (ext === '.html') {
        ...
        if (pointerTarget) {
          ...
          if (options.shouldOpen?.() !== false) openPersistedFile(pointerTarget);
          return { ok: true as const, fileName: pointerTarget };
        }
      }
      ...
      if (file) {
        ...
        if (options.shouldOpen?.() !== false) openPersistedFile(file.name);
    ...
    const recoverArtifacts = async () => {
      ...
      try {
        ...
        for (const message of recoveryMessages) {
          ...
          if (recoveredExistingArtifact) {
            savedArtifactRef.current = recoveredExistingArtifact.name;
            if (shouldOpenRecoveredArtifact()) {
              requestOpenFile(recoveredExistingArtifact.name, 'internal');
            }
          } else {
            savedArtifactRef.current = null;
          ...
          if (producedArtifactToOpen && shouldOpenRecoveredArtifact()) {
            requestOpenFile(producedArtifactToOpen, 'internal');
          }
            ...
            try {
              ...
              if (artifactToPersist?.html) {
                ...
                if (sameTurnWrite) {
                  ...
                  completionSelectedAutoOpen = true;
                  if (shouldOpenCompletedArtifact()) requestTurnOpenFile(sameTurnWrite.name);
                } else {
                  ...
                  const persistence = await persistArtifact(
                    ...
                    nextFiles,
                    finalText,
                    { openFile: requestRunOpenFile, shouldOpen: shouldOpenCompletedArtifact },
                  );
                  if (persistence.ok) artifactPersistenceSucceeded = true;
                  else artifactPersistenceError = persistence.error;
