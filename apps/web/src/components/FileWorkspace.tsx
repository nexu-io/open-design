      ...
      <div className="ws-tabs-shell">
        ...
        >
          ...
          {tabNames.map((name) => {
            ...
            // Wix Japan rebrand: result.json is the deck preview
            // entry for the wix-ja-slide skill — show it labelled as
            // "プレビュー" rather than the technical filename. Live
            // artifacts (upstream addition) keep their dynamic title.
            const friendlyLabel =
              liveArtifact?.title
                ?? (name === 'result.json' ? t('workspace.deckPreview') : name);
            return (
              <Tab
                key={name}
