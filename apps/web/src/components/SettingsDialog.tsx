...
export function isValidApiBaseUrl(value: string): boolean {
  ...
  // The internal-IP / SSRF decision belongs to the daemon, which is the
  // single source of truth and honors the operator's OD_ALLOWED_INTERNAL_HOSTS
  // allowlist — a value the browser cannot see (#3225). A `forbidden` result
  // here means a syntactically-valid URL that points at an internal address;
  // we keep it UI-valid so the operator can run the connection test / model
  // fetch and get the daemon's authoritative answer (allowed when listed,
  // a clear "Internal IPs blocked" otherwise). Only genuinely malformed URLs
  // stay invalid client-side.
