...
function SkillPluginCandidateCard({
  ...
  async function share(action: "contribute-open-design") {
    ...
    try {
      ...
      setNotice({
        message: `Marketing AX contribution task started for ${data?.path ?? "the draft"}.`,
      });
    } catch (err) {
      setNotice({ message: err instanceof Error ? err.message : String(err) });
    } finally {
...
interface Props {
  ...
  // Click handler for the post-completion "Share to Marketing AX" submission
...
function hasPluginFinalActionHint(content: string): boolean {
  return /\b(Add to My plugins|Marketing AX PR|Publish repo|plugin publish|ready to publish|ready to add)\b/i.test(
    content,
  );
}
