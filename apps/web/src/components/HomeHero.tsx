...
const HOME_PROMPT_EXAMPLES: Record<Locale, Record<string, string[]>> = {
  ...
  "he": {
    prototype: [
      "עצבו אתר ממיר למערכת CRM מבוססת AI, עם אזור פתיחה ברור, סיפור יכולות, הוכחות וקריאה להתנסות",
      "צרו לוח בקרה שולחני למאגר הידע של הצוות, עם חיפוש, עדכונים אחרונים, הרשאות ונקודות כניסה לשיתוף פעולה",
      "עצבו מחדש את תהליך ההצטרפות למוצר SaaS פיננסי, כך שמשתמשים חדשים יחברו נתונים, ישלימו את ההגדרה ויראו ערך במהירות",
      "צרו אב־טיפוס לאפליקציית אימון כושר לנייד, הכוללת הגדרת יעדים, תוכניות שבועיות, דיווחי אימון וסיכום התקדמות",
    ],
    wireframe: [
      "צרו מסגרת תיל ללוח בקרה של SaaS, עם ניווט, מדדי מפתח, טבלת נתונים ומצב ריק",
      "שרטטו זרימה בסיסית לתשלום: סל קניות, משלוח, תשלום ומסכי אישור",
      "עצבו מסך לניהול תוכן עם מצבי רשימה, פרטים ועריכה ברמת נאמנות אפורה ופשוטה",
      "צרו מסגרת תיל לאשף הצטרפות הכולל חשבון, סביבת עבודה, הזמנה ושלבי הפעלה ראשונים",
    ],
    mobile: [
      "עצבו אפליקציית בנקאות לנייד עם מסכי יתרה ראשיים, עסקאות, העברות וניהול כרטיסים",
      "תכננו אפליקציית משלוחי אוכל הכוללת עיון, פרטי מסעדה, סל קניות ומעקב הזמנה",
      "צרו אב־טיפוס לאפליקציית מעקב הרגלים עם תצוגת היום, פרטי רצף, הוספת הרגל ומסכי נתונים",
      "עצבו אפליקציית הודעות עם רשימת שיחות, שיחה, פרופיל ומסכי הגדרות",
    ],
    document: [
      "נסחו קורות חיים בני עמוד אחד למעצב או מעצבת מוצר בכירים, עם תקציר, ניסיון, מיומנויות ועבודות נבחרות",
      "כתבו הצעת פרויקט מלוטשת עם סעיפים של בעיה, גישה, לוח זמנים ותקציב",
      "צרו דף מוצר אחד הכולל את ההצעה, יכולות מרכזיות, תמחור וקריאה לפעולה",
      "הפיקו תקציר פגישה מסודר עם סדר יום, החלטות, אחראים והצעדים הבאים",
    ],
    deck: [
      "חקרו את הזדמנות השוק להשקת מוצר, כולל מתחרים, משתמשי יעד, השערות תמחור ונרטיב ההשקה",
      "צרו דוח סטטוס שבועי לצוות עם התקדמות, סיכונים, שינויים במדדים וסדרי עדיפויות לשבוע הבא",
      "עצבו מצגת למשקיעים עם גודל השוק, מודל צמיחה, יתרון המוצר ותחזית לשלוש שנים",
      "צרו מצגת סקירה עסקית אסטרטגית הכוללת ביצועים רבעוניים, גורמי שורש, הזדמנויות והפעולות הבאות",
    ],
    image: [
      "צרו כרזה של סביבת עבודה מבוססת AI בסגנון זכוכית, עם שיתוף פעולה בין מסכים, תאורה רכה ואווירת השקה יוקרתית",
      "צרו תמונת גיבור למסחר אלקטרוני עבור אוזניות אלחוטיות חדשות, המדגישה חומריות, הקשר שימושי ויתרונות מרכזיים",
      "עצבו ויזואל מפתח מינימליסטי להשקת מוצר טכנולוגי, עם קומפוזיציה נקייה, מיקוד חזק במוצר וטקסט השקה מאופק",
      "צרו סדרת טיזרים לרשתות החברתיות להשקת מוצר, כולל ספירה לאחור, תקריב, חשיפת יתרון וויזואל ליום ההשקה",
    ],
    video: [
      "צרו סרטון חשיפת מוצר באורך שמונה שניות, העובר מצללית לתקריב ומסתיים בסמל המותג",
      "הפיקו סרטון הדגמה ליכולת באפליקציה, העוקב אחר מסע המשתמש, המצבים המרכזיים והתוצאה הסופית",
      "צרו פתיח אנכי למותג עם טיפוגרפיה קצבית, תקריבי מוצר וסיום נקי עם הלוגו לסרטון קצר",
      "הפכו אתר לפרסומת חברתית של 15 שניות באמצעות חילוץ הבטחת הפתיחה, רגעי האינטראקציה וקריאה ברורה לפעולה",
    ],
    hyperframes: [
      "בנו סרטון קצר להשקת מוצר עם כתוביות, כרטיסי כותרת, צילומי יכולות, מעברים קצביים וקריאה לפעולה בסיום",
      "צרו הדמיית נתונים המגיבה לאודיו, שבה עמודות, חלקיקים וכותרות מגיבים לקצב הקריינות",
      "צרו סיום לוגו של שלוש שניות באמצעות התכנסות קווים, אלסטיות עדינה ומערכת צבעי המותג",
      "צרו מפת נתיבי טיסה מונפשת המציגה צמתי ערים, צמיחת מסלולים, נתוני מרחק ופריים סיכום",
    ],
    audio: [
      "צרו צליל הפעלה למוצר שמרגיש קליל, אמין וקצת עתידני, ומתאים להשקת אפליקציה שולחנית",
      "צרו מוזיקת פתיחה באורך 20 שניות לפודקאסט, עם פתיחה חמימה, פעימה ברורה ומעבר נקי לקריינות",
      "צרו לופ אווירה רציף לאפליקציית מדיטציה, עם מרקמי טבע רכים, חמימות בתדרים נמוכים וקצב רגוע",
      "הפיקו סט צלילי התראה ממותג למצבי הצלחה, תזכורת ושגיאה, תוך שמירה על זהות קולית אחידה",
    ],
  },
  "fr": {
    prototype: [
