...
describe('ChatComposer infinite re-render regression (#2097)', () => {
  ...
  it('defers draft persistence outside the keystroke path', () => {
    const key = 'od:chat-composer:draft:project-1:conv-1';
    ...
    try {
      renderComposer({ draftStorageKey: key });
      act(() => {
      ...
      const textarea = screen.getByTestId('chat-composer-input') as HTMLTextAreaElement;
      fireEvent.change(textarea, {
        target: { value: 'latency check', selectionStart: 13, selectionEnd: 13 },
      });
      ...
      expect(setItem).toHaveBeenCalledWith(key, 'latency check');
    } finally {
      setItem.mockRestore();
  ...
  it('stops scanning project files once the mention picker has enough results', async () => {
    const projectFiles: ComponentProps<typeof ChatComposer>['projectFiles'] = Array.from(
      { length: 12 },
      (_, index) => ({
        name: `file-${index}.ts`,
    ...
    for (let index = 0; index < 100; index += 1) {
      projectFiles.push({
        name: `late-${index}.ts`,
        get path(): string {
          throw new Error('mention picker scanned past the visible file cap');
        },
        type: 'file',
        size: 1,
        mtime: 1,
    ...
    await waitFor(() => expect(screen.getAllByText(/src\/file-\d+\.ts/)).toHaveLength(12));
  });
