...
describe('DesignSystemCreationFlow', () => {
  ...
  it.skip('opens the project as soon as the workspace exists and prepares the first chat task afterward', async () => {
    ...
    await waitFor(() => expect(mocks.patchProject).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        pendingPrompt: expect.stringContaining('Create this project as a complete LeastGen Studio design system workspace.'),
      }),
    ));
  ...
  it.skip('adds website source links with Enter and keeps them out of GitHub intake', async () => {
    const system: DesignSystemDetail = {
      ...
      title: 'LeastGen Studio Website Design System',
      category: 'Custom',
      summary: 'LeastGen Studio website source.',
      swatches: [],
      surface: 'web',
      body: '# LeastGen Studio Website Design System\n',
      source: 'user',
      status: 'draft',
      isEditable: true,
    ...
    const project: Project = {
      ...
      name: 'LeastGen Studio Website Design System',
      skillId: null,
      designSystemId: system.id,
      createdAt: 1,
      updatedAt: 1,
    ...
    fireEvent.change(screen.getByPlaceholderText(/Mission Impastabowl/i), {
      target: { value: 'LeastGen Studio website source' },
    });
    continueToGeneration();
    continueToGeneration();
  ...
  it.skip('allows GitHub repo links without Composio by using local GitHub intake', () => {
    ...
    expect(screen.getByText('LeastGen Studio account')).toBeTruthy();
  ...
  it('keeps a manual GitHub authorization action when the automatic popup is blocked', async () => {
    ...
    mocks.connectConnector.mockResolvedValue({
      ...
      error: 'Popup blocked. Allow popups for LeastGen Studio and try again.',
    });
    ...
    try {
      ...
      expect(screen.getByText('Popup blocked. Allow popups for LeastGen Studio and try again.')).toBeTruthy();
