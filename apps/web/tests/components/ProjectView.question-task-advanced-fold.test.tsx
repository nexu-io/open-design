...
// OPEND-2944 (already-advanced half): the page is refreshed AFTER the daemon
// has already moved the OD Next task on to its successor Run. The recovery
// scan's status probe for the persisted Run therefore projects a different
// `activeRunId`, so `taskRunAdvanced` re-points the row at that successor
// before any stream is opened — there is no `onRunCreated` hand-off to fix it
// up later.
//
// That re-point spreads the predecessor row, so it keeps the predecessor's
// `strategyTaskRunIndex` on a row that now follows a different Run. It is the
// same defect the live send path and the reattach `onRunCreated` hand-off were
// fixed for: `foldStrategyTaskTurns`, the fork boundary and the
// successor-absorption rule all read that field, and it would tell them the
// row still sits at the task's previous position.
//
// The strict rule is shared with those paths: a projection that does not name
// this Run exactly once yields `undefined`, never the predecessor's index and
// never a guess.
...
/**
 * The status probe the recovery scan makes for the persisted request Run,
 * after the daemon has already claimed the successor. `activeRunId` names a
 * Run the reloaded row has never followed — that difference is exactly what
 * `taskRunAdvanced` detects.
 */
...
/**
 * A refreshed page whose persisted row still names the request Run and the
 * request's task position — the daemon advanced the task while the tab was
 * gone, so nothing local knows about the successor yet.
 */
...
/**
 * Renders the reloaded page and lets the recovery scan discover that the task
 * already sits on the successor Run.
 *
 * The subscription it then opens is for the SUCCESSOR — there is no
 * `onRunCreated` hand-off on this path, so the row's task position has one
 * writer only: the `taskRunAdvanced` re-point itself.
 */
async function reattachAfterTaskAdvanced() {
  reattachDaemonRun.mockImplementation(async (options: DaemonReattachOptions) => {
    expect(options.runId).toBe(PRODUCTION_RUN);
    options.onRunStatus?.('running');
    options.handlers.onAgentEvent({ kind: 'done_key', key: 'production-key' });
    options.handlers.onDelta(PRODUCTION_TEXT);
    options.handlers.onAgentEvent({ kind: 'text', text: PRODUCTION_TEXT });
...
describe('OPEND-2944 a reloaded page whose task had already advanced', () => {
  ...
  it('adopts the successor Run\'s daemon-issued task position', async () => {
    ...
    // Same fold verdict as the other two paths: one logical task, one author.
  ...
  it.each(['legacy', 'wrong-run', 'duplicate'] as const)(
    ...
    async (mode) => {
      mappingMode = mode;
      await reattachAfterTaskAdvanced();
