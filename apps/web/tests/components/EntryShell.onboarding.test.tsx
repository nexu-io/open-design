...
describe('EntryShell onboarding Open Design AMR runtime', () => {
  ...
  it('keeps the active OpenAI draft when its onboarding preset is reselected', async () => {
    const sentinelApiKey = 'onboarding-provider-sentinel';
    vi.useFakeTimers();
    const fetchMock = vi.fn(async (input, init) => {
      const url = String(input);
    ...
    fireEvent.click(screen.getByRole('button', { name: /Bring your own key/i }));
    chooseOnboardingOption('Quick fill provider', /^OpenAI$/);
    fetchMock.mockClear();
    await act(async () => {
      await vi.advanceTimersByTimeAsync(500);
    });
    ...
    expect({
      apiKey: (screen.getByLabelText('API key') as HTMLInputElement).value,
      baseUrl: (screen.getByLabelText('Base URL') as HTMLInputElement).value,
      model: screen.getByRole('button', { name: 'gpt-4o' }).textContent,
      providerRequestBodies,
    }).toEqual({
      apiKey: sentinelApiKey,
      baseUrl: 'https://api.openai.com/v1',
