...
describe('ChatComposer /search command', () => {
  ...
  it('expands /search into a first-action research command prompt', async () => {
    ...
    expect(prompt).toContain(
      'POSIX: "$MAX_NODE_BIN" "$MAX_BIN" research search --query "<search query>" --max-sources 5',
    );
    expect(prompt).toContain(
      'PowerShell: & $env:MAX_NODE_BIN $env:MAX_BIN research search --query "<search query>" --max-sources 5',
    );
    expect(prompt).toContain(
      'cmd.exe: "%MAX_NODE_BIN%" "%MAX_BIN%" research search --query "<search query>" --max-sources 5',
    );
  ...
  it('shows the active imported-folder file and sends it as edit context', async () => {
    ...
    expect(screen.getAllByText('Ask Marketing AX to change index.html...').length).toBeGreaterThan(0);
