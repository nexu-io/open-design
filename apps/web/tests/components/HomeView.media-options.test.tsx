...
describe('HomeView media composer options', () => {
  ...
  // Skipped after the Home composer footer simplification (main's PR #3692
  // path adopted on merge 2026-06-08): chip-level model/ratio/size/resolution
  // pills were removed; the agent now asks for those via AskUserQuestion
  // during the run. The SenseAudio discrete-size + 480p/720p/1080p resolution
  // selection flow lives on the Settings → Media model picker and the in-run
  // discovery turn, not the home composer footer. Re-enable if those pills
  // ever come back to the home composer in a future iteration.
  it.skip('reveals SenseAudio image Size and video Resolution options matching the docs', async () => {
    // Body removed when the Home composer footer was simplified on main
    // (PR #3692): chip-level model/size/resolution/duration pills no longer
    // render. The assertions below targeted those pills via `optionTexts` and
    // a `chooseOption`/`openOption` helper that lived alongside the old DOM.
    // Re-author the test against whatever surface owns SenseAudio
    // discrete-size + resolution selection next if it returns to the home
    // composer.
  });
