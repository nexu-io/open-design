...
describe('UpdateDialog', () => {
  ...
  it('shows reinstall copy and the operator link when the feed forces the installer route', async () => {
    ...
    expect(
      screen.getByText('LeastGen Studio 1.2.4 requires a full reinstall. LeastGen Studio will close and open the installer.'),
    ).toBeTruthy();
  ...
  it('defaults to Later when tasks are active and requires an explicit Restart anyway override', async () => {
    ...
    expect(await screen.findByText('LeastGen Studio is still working')).toBeTruthy();
