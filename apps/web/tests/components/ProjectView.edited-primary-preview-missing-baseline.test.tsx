...
async function recoverRunningTurn({
  declareFocus = true, baselinePresent = false, editSucceeded = true,
}: { declareFocus?: boolean; baselinePresent?: boolean; editSucceeded?: boolean } = {}) {
  // The real interrupted run lacks both its persisted baseline and terminal
  // artifactPaths. Keep authoritative paths only for the failed-Edit guard so
  // that its complete inventory does not manufacture successful tool evidence.
  const artifactPathsPresent = !editSucceeded;
  ...
  const message: ChatMessage = {
    ...
    sessionMode: 'chat',
    ...(baselinePresent ? { preTurnFileNames: ['index.html'] } : {}),
    lastRunEventId: '4',
  };
  expect(Object.hasOwn(message, 'preTurnFileNames')).toBe(baselinePresent);
  ...
  renderProjectView({ resolvedDir: '/var/fixture/projects/project-1', metadata: { kind: 'prototype' } });
  ...
  const events: NonNullable<ChatMessage['events']> = [
    { kind: 'tool_use', id: 'edit-dark', name: 'Edit', input: { file_path: '/private/var/fixture/projects/project-1/index.html' } },
    { kind: 'tool_result', toolUseId: 'edit-dark', content: editSucceeded ? 'Edited index.' : 'Edit failed.', isError: !editSucceeded },
    { kind: 'tool_use', id: 'write-backup', name: 'Write', input: { file_path: '/private/var/fixture/projects/project-1/reference/light-paper.html' } },
...
describe('OPEND-2950: interrupted recovery without a persisted file baseline', () => {
  afterEach(() => {
    cleanup();
    vi.clearAllMocks();
  ...
  it('keeps a successfully edited explicit primary focused when the recovered message has no baseline', async () => {
    // Observed in opend2950-integrated-r-5facda4d, wire 525 onward:
    // no preTurnFileNames, successful Edit + Write, show/open index, no
    // artifactPaths. The final inventory must retain BOTH main and backup.
    await recoverRunningTurn();
    expect(focusedTab()).toBe('index.html');
  });

  it('retains the same focus with the old R baseline and the same managed alias paths', async () => {
    // Single-input control: /private/var tool paths and /var resolvedDir are
    // identical to the failure case; only the persisted baseline is restored.
    await recoverRunningTurn({ baselinePresent: true });
    expect(focusedTab()).toBe('index.html');
  });

  it('keeps the ranking fallback when there is no explicit focus declaration', async () => {
    await recoverRunningTurn({ declareFocus: false });
    expect(focusedTab()).toBe('reference/light-paper.html');
  });

  it('does not treat an explicit declaration or authoritative inventory as successful Edit proof', async () => {
    await recoverRunningTurn({ editSucceeded: false });
    expect(focusedTab()).toBe('reference/light-paper.html');
  });
});
