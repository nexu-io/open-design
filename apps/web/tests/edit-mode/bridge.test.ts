...
describe('manual edit bridge target normalization', () => {
  ...
  it('preserves a stylesheet-authored none text transform override', async () => {
    const dom = new JSDOM(
      `<style>.parent { text-transform: uppercase; } .child { text-transform: none; word-spacing: 3px; }</style><main class="parent"><h1 class="child" data-od-id="hero">Two words</h1></main>${buildManualEditBridge(true)}`,
      { runScripts: 'dangerously', url: 'http://localhost' },
    );
    ...
    expect(postMessage).toHaveBeenCalledWith(expect.objectContaining({
      ...
      targets: expect.arrayContaining([expect.objectContaining({
        id: 'hero',
        styles: expect.objectContaining({ textTransform: 'none', wordSpacing: '3px' }),
      })]),
    }), '*');
