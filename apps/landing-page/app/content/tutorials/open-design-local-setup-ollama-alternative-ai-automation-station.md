...
Then open the local URL it prints in your terminal. You need Node `~24` and pnpm `11.15.x`; Corepack will pick the pinned pnpm version for you. The port is assigned dynamically, so use whatever address the command shows rather than guessing one.
