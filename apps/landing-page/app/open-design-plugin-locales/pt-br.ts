  ...
  agentInstall: {
    ...
    steps: [
      ...
      },
      {
        ...
        body:
          'Preserve um MCP open-design existente e habilitado. Caso contrário, execute o instalador do MCP fornecido pelo aplicativo Open Design instalado. Em sistemas POSIX, execute o caminho resolvido de od somente depois que a verificação retornar open-design-cli:mcp-install:v1. Se esse caminho estiver ausente, tiver sido ocultado por /usr/bin/od ou não for possível concluir a execução, use /usr/bin/open com o pacote de aplicativo macOS assinado e verificado e --headless --mcp-install codex; depois, faça polling apenas do snapshot filtrado do MCP. Se nenhum iniciador verificado estiver disponível, pare, instrua o usuário a acessar Open Design Settings → MCP server para copiar o trecho específico do cliente com caminho absoluto e repita esta etapa. Nunca tente adivinhar uma porta localhost nem execute codex mcp login; o login da Vela pertence ao Open Design.',
      },
      {
        title: 'Verifique, informe e inicie uma nova tarefa',
        body:
          'Verifique o id de plugin open-design@open-design, confirme que a versão instalada corresponde exatamente a plugin.version em release-manifest.json e use o snapshot filtrado do MCP para confirmar que há um stdio MCP habilitado chamado open-design. Inspecione somente name, enabled, transport.type e command; os valores brutos de args, env, env_vars, headers, token, bearer token, API key e das credenciais da Vela jamais devem aparecer na saída ou nos relatórios. O campo command deve ser um caminho absoluto de inicialização do Open Design. Informe o que foi instalado e qualquer pré-requisito restante do Open Design ou de login da Vela. Inicie uma nova tarefa do Codex para que ela carregue o snapshot do plugin instalado e, depois, invoque @open-design.',
      },
    ],
  },
  demo: {
