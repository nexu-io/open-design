    ...
    steps: [
      {
        ...
        body:
          'Прочитайте plugin.minimumCodexCliVersion и plugin.minimumOpenDesignVersion из release-manifest.json. Перед любым изменением конфигурации убедитесь, что установленный Codex CLI и проверенная среда выполнения Open Design, данные о которой получены при проверке через Open Design CLI, соответствуют указанным в манифесте минимальным требованиям. Если проверку Open Design выполнить невозможно, версия среды выполнения недоступна или слишком старая, остановитесь и направьте пользователя на официальную страницу загрузки; не загружайте установщик без ведома пользователя и не запускайте непроверенные скрипты.',
      },
      {
        title: 'Перед изменением конфигурации проверьте её',
        body:
          'Прочитайте требуемые селектор и версию из release-manifest.json, затем сравните их с состоянием Codex, полученным при проверке. Проверяйте состояние MCP только по выводу codex mcp list с удалёнными конфиденциальными данными: отсутствие строки open-design ожидаемо и не является ошибкой. Никогда не сохраняйте вывод codex mcp get open-design --json, поскольку значения переменных окружения транспортного слоя могут содержать учётные данные. Если имя маркетплейса open-design ссылается на другой источник, остановитесь и сообщите о конфликте. Сохраняйте open-design@open-design только в том случае, если установленная версия совпадает с версией, указанной в release-manifest.json.',
      },
      {
        title: 'Установите только отсутствующие компоненты',
        body:
          'Выполняйте команду для маркетплейса, только если канонический маркетплейс отсутствует. Если open-design@open-design отсутствует, выполните команду установки плагина. Если установлена другая версия, запросите у пользователя подтверждение перед обновлением или переустановкой; пропускайте установку только при наличии точно требуемой версии. Результат alreadyAdded: true означает успех. Не редактируйте конфигурацию Codex вручную и не копируйте файлы плагина в домашний каталог Codex.',
      },
      {
        title: 'Настройте локальный MCP Open Design',
        body:
          'Не изменяйте уже включённый MCP open-design. В противном случае запустите установщик MCP, входящий в установленное приложение Open Design. В системах POSIX запускайте od по найденному пути, только если его проверка возвращает open-design-cli:mcp-install:v1; при любом другом результате, в том числе если найден /usr/bin/od, переходите к Open Design Settings → MCP server или операции --headless --mcp-install codex из подписанного пакета. Никогда не угадывайте порт localhost и не запускайте codex mcp login; вход в Vela выполняется в Open Design.',
      },
      {
        title: 'Проверьте результат, отчитайтесь и начните новую задачу',
        body:
          'Убедитесь, что идентификатор плагина — open-design@open-design, что его установленная версия точно совпадает с plugin.version в release-manifest.json, и по выводу codex mcp list с удалёнными конфиденциальными данными подтвердите, что включён stdio MCP с именем open-design. Его команда должна быть абсолютным путём запуска Open Design и не должна включать bearer token, API key или учётные данные Vela. Сообщите, что было установлено и какие предварительные требования Open Design или входа в Vela ещё не выполнены. Начните новую задачу Codex, чтобы она загрузила снимок установленного плагина, затем вызовите @open-design.',
      },
    ],
  },
  demo: {
