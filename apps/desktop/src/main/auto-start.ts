...
function buildRunCommand(namespace: string, ipc: string): string {
  ...
  const entryScript = join(workspaceRoot, "apps/desktop/dist/main/index.js");
  ...
  const stamp = [
    ...
    `--od-stamp-namespace="${escapeForRegAddValue(namespace)}"`,
    `--od-stamp-ipc="${escapeForRegAddValue(ipc)}"`,
...
function generatePlist(exePath: string, appId: string, namespace: string, ipc: string): string {
  ...
  return [
    ...
    `<string>--od-stamp-app=${APP_KEYS.DESKTOP}</string>`,
