...
export function strategyTaskCreateIdentityFixture(
  taskType: Exclude<StrategyTaskTypeV2, 'generic'> = 'prototype',
) {
  const promptBundleText = taskType === 'prototype'
    ? TEST_PROMPT_BUNDLE
    : serializeOdNextPromptBundleV2({
        coreSystemPrompt: {
          executionBoundary: 'Frozen test execution boundary.',
        ...
        sessionSkills: {
          ...
          taskTypeSkill: { skillName: taskType, body: 'Frozen test task skill.' },
        },
        activeStages: [
          { name: 'discovery', atoms: [{ name: 'discovery-question-form', body: 'Frozen atom.' }] },
        ],
