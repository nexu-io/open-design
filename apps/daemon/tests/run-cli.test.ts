...
describe('od run CLI', () => {
  it.each(['pi', 'codex', 'claude', 'dsh', 'ohmypi', 'none'] as const)('starts %s with a prompt file and keeps its harness/model when continuing', async (runtime) => {
    ...
    const started = await runCli([
      ...
      '--amr-runtime', runtime, '--model', 'gpt-6-astra', '--prompt-file', promptFile,
