...
describe('chat run service shutdown', () => {
  ...
  it('cancels a queued run immediately without waiting for child process shutdown', async () => {
    ...
    expect(run.events.some((event: { data?: { label?: string; detail?: string } }) =>
      event.data?.label === 'cancellation' && event.data?.detail === 'cancel_requested',
    )).toBe(true);
  });
  ...
  describe('cancel kill fallback', () => {
    ...
    it('sends SIGTERM immediately and escalates to SIGKILL after the cancel grace window', async () => {
      ...
      expect(run.events).toEqual(expect.arrayContaining([
        expect.objectContaining({
          event: 'agent',
          data: expect.objectContaining({ label: 'cancellation', detail: 'cancel_requested' }),
        }),
        expect.objectContaining({
          event: 'agent',
          data: expect.objectContaining({ label: 'cancellation', detail: 'child_signal', signal: 'SIGTERM' }),
        }),
        expect.objectContaining({
          event: 'agent',
          data: expect.objectContaining({ label: 'cancellation', detail: 'child_signal', signal: 'SIGKILL' }),
        }),
        expect.objectContaining({
          event: 'agent',
          data: expect.objectContaining({ label: 'cancellation', detail: 'child_terminated', childExited: true }),
        }),
      ]));
    });
    ...
    it('uses ACP abort before falling back to process signals', async () => {
      ...
      expect(run.events).toEqual(expect.arrayContaining([
        expect.objectContaining({
          event: 'agent',
          data: expect.objectContaining({ label: 'cancellation', detail: 'session_abort_requested' }),
        }),
        expect.objectContaining({
          event: 'agent',
          data: expect.objectContaining({ label: 'cancellation', detail: 'child_terminated', signal: 'SIGKILL' }),
        }),
      ]));
    });
