...
describe("isCacheableExternalUrl", () => {
	it("accepts absolute http(s) media urls", () => {
		expect(
			isCacheableExternalUrl("https://res.cloudinary.com/x/portal_bg.png"),
		).toBe(true);
		expect(isCacheableExternalUrl("http://images.higgs.ai/a/b.jpg?v=2")).toBe(
			true,
		);
		expect(isCacheableExternalUrl("https://d8j0.cloudfront.net/clip.mp4")).toBe(
			true,
		);
		expect(isCacheableExternalUrl("https://cdn.example.com/a.webp#frag")).toBe(
			true,
		);
	});

	it("accepts fonts, CSS, JS, and Google Fonts; rejects relative, data and non-http urls", () => {
		expect(
			isCacheableExternalUrl("https://fonts.googleapis.com/css2?family=Inter"),
		).toBe(true);
		expect(isCacheableExternalUrl("https://cdn.example.com/app.js")).toBe(true);
		expect(isCacheableExternalUrl("https://cdn.example.com/styles.css")).toBe(
			true,
		);
		expect(isCacheableExternalUrl("https://cdn.example.com/font.woff2")).toBe(
			true,
		);
...
describe("assertSafePublicUrl (up-front rejection)", () => {
	...
	it("rejects localhost and literal private IPs before any socket opens", () => {
		...
		expect(() =>
			assertSafePublicUrl("http://[::ffff:127.0.0.1]/x.png"),
		).toThrow(expect.objectContaining({ status: 400 }));
	});

	it("accepts a public host (DNS is validated later, at connection time)", () => {
		expect(
			assertSafePublicUrl("https://res.cloudinary.com/x/a.png").hostname,
...
describe("createValidatingLookup (DNS-rebinding / TOCTOU guard)", () => {
	...
	function run(
		...
		return new Promise<{ err: Error | null; address?: unknown }>((resolve) => {
			createValidatingLookup(lookupImpl as never)(
				"host.example",
				{ all: false },
	...
	it("passes a public resolved address through", async () => {
		const { err, address } = await run((_h, _o, cb) =>
...
describe("createPluginAssetCache", () => {
	...
	it("rejects oversized assets (declared Content-Length)", async () => {
		...
		await expect(
			cache.get("https://res.cloudinary.com/x/big.png"),
	...
	it("stops reading a no-Content-Length body once it exceeds the cap (no full buffering)", async () => {
		...
		await expect(
			cache.get("https://res.cloudinary.com/x/stream.png"),
		).rejects.toMatchObject({ status: 413 });
	...
	it("refuses non-cacheable urls without fetching", async () => {
		...
		await expect(
			cache.get("/api/plugins/x/asset/hero.png"),
	...
	it("refuses a literal private-IP url before fetching (SSRF up-front guard)", async () => {
		...
		await expect(
			cache.get("https://169.254.169.254/x.png"),
		).rejects.toMatchObject({ status: 400 });
		expect(calls).toBe(0);
	});
