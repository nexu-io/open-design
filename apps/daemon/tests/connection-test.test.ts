...
describe('POST /api/test/connection provider mode', () => {
  ...
  it('denies deployment provider connection tests before creating a run session', async () => {
    await withDeploymentProviderEnv({
      OD_PROVIDER_ORCHESTRATOR_BASE_URL: 'https://gateway.example.test/v1',
      OD_PROVIDER_ORCHESTRATOR_API_KEY: 'deployment-secret',
      OD_PROVIDER_ORCHESTRATOR_RUN_SESSION_URL: 'https://authority.example.test/api/runs',
...
`,
  ...
  it('launches Kimi connection tests through direct stream-json mode', async () => {
...
if (
  ...
  args[2] !== '--output-format' ||
  args[3] !== 'stream-json' ||
  args[4] !== '--model' ||
  args[5] !== 'moonshot-v1-32k'
) {
  console.error('missing direct stream-json args');
  process.exit(1);
}
...
`,
        async () => {
          ...
          expect(args[3]).toBe('stream-json');
          expect(args.slice(4)).toEqual(['--model', 'moonshot-v1-32k']);
        },
      );
    } finally {
      await fsp.rm(markerDir, { recursive: true, force: true });
