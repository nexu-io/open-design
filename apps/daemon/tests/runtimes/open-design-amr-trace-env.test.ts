...
test('openDesignAmrTraceEnv builds Open Design trace identity env for AMR only', () => {
  ...
  assert.equal(amrEnv.MARKETING_AX_RUN_ID, 'run_trace_123');
  assert.equal(amrEnv.MARKETING_AX_RUN_ATTEMPT, '2');
  assert.equal(amrEnv.MARKETING_AX_SESSION_ID, 'conversation_trace_456');
...
test('openDesignAmrTraceEnv omits optional AMR session trace env when no conversation exists', () => {
  ...
  assert.equal(env.MARKETING_AX_RUN_ID, 'run_trace_no_session');
  assert.equal(env.MARKETING_AX_RUN_ATTEMPT, '0');
  assert.equal(env.MARKETING_AX_SESSION_ID, undefined);
});
