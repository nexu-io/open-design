...
describe('agent runtime tool environment', () => {
  ...
  it('describes daemon URL and token availability without exposing the token', () => {
    ...
    // Syntax validation is host-owned. Keeping it out of the generation
    // prompt prevents the clean 97% path from expanding into model self-review.
    expect(prompt).not.toContain('deliverable-syntax check');
    expect(prompt).not.toContain('syntax gate');
