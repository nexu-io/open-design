 ...
 * successful `vela login` and Marketing AX just surfaces a small projection.
...
describe('resolveAmrProfile', () => {
  it('defaults to "prod" when MARKETING_AX_AMR_PROFILE is unset or empty', () => {
    expect(resolveAmrProfile({})).toBe('prod');
    expect(resolveAmrProfile({ MARKETING_AX_AMR_PROFILE: '   ' })).toBe('prod');
  });

  it('honors MARKETING_AX_AMR_PROFILE when set to a known profile', () => {
    expect(resolveAmrProfile({ MARKETING_AX_AMR_PROFILE: 'prod' })).toBe('prod');
    expect(resolveAmrProfile({ MARKETING_AX_AMR_PROFILE: 'local' })).toBe('local');
    expect(resolveAmrProfile({ MARKETING_AX_AMR_PROFILE: 'test' })).toBe('test');
  });
  ...
  it('warns for unknown MARKETING_AX_AMR_PROFILE values and falls back to prod', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    expect(resolveAmrProfile({ MARKETING_AX_AMR_PROFILE: 'evil' })).toBe('prod');
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining('MARKETING_AX_AMR_PROFILE'),
    );
    warn.mockRestore();
  });
});
...
describe('readVelaLoginStatus', () => {
  it('returns loggedIn=false when ~/.amr/config.json is absent', () => {
    const status = readVelaLoginStatus({ MARKETING_AX_AMR_PROFILE: 'local' });
  ...
  it('isolates profiles — a logged-in "local" does not imply logged-in "prod"', () => {
    ...
    expect(readVelaLoginStatus({ MARKETING_AX_AMR_PROFILE: 'local' }).loggedIn).toBe(true);
    expect(readVelaLoginStatus({ MARKETING_AX_AMR_PROFILE: 'prod' }).loggedIn).toBe(false);
  });
  ...
  it('treats malformed JSON as logged-out rather than crashing', () => {
    ...
    expect(readVelaLoginStatus({ MARKETING_AX_AMR_PROFILE: 'local' }).loggedIn).toBe(false);
  });
...
describe('readVelaCredentialRevision', () => {
  it('changes when file-backed logout clears the active profile credentials', async () => {
    ...
    const before = readVelaCredentialRevision({ MARKETING_AX_AMR_PROFILE: 'local' });
    ...
    forgetVelaLogin({ MARKETING_AX_AMR_PROFILE: 'local' });

    const after = readVelaCredentialRevision({ MARKETING_AX_AMR_PROFILE: 'local' });
...
describe('forgetVelaLogin', () => {
  it('removes only the resolved profile credentials and preserves the rest of the config', () => {
    ...
    forgetVelaLogin({ MARKETING_AX_AMR_PROFILE: 'local' });
    expect(readVelaLoginStatus({ MARKETING_AX_AMR_PROFILE: 'local' }).loggedIn).toBe(false);
    expect(readVelaLoginStatus({ MARKETING_AX_AMR_PROFILE: 'prod' }).loggedIn).toBe(true);
  ...
  it('is a no-op when the resolved profile does not exist', () => {
    ...
    expect(() => forgetVelaLogin({ MARKETING_AX_AMR_PROFILE: 'local' })).not.toThrow();
    const next = JSON.parse(readFileSync(file, 'utf8'));
    expect(next.profiles.prod.runtimeKey).toBe('rt-prod');
  });
...
describe('spawnVelaLogin', () => {
  it('returns an actionable error when no vela binary can be resolved', async () => {
    ...
    const originalResourceRoot = process.env.MAX_RESOURCE_ROOT;
    try {
      process.env.PATH = '';
      delete process.env.MAX_RESOURCE_ROOT;
      await expect(
        spawnVelaLogin({
      ...
      if (originalResourceRoot === undefined) delete process.env.MAX_RESOURCE_ROOT;
      else process.env.MAX_RESOURCE_ROOT = originalResourceRoot;
    }
  });
