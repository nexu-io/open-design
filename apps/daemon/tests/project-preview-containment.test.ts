...
describe("project preview containment routes", () => {
	...
	it("returns a scoped preview URL with sandbox guidance and serves it with an opaque-origin CSP", async () => {
		...
		expect(body.url).toMatch(
			/\/preview\/[A-Za-z0-9_-]{8,128}\/pages\/index\.html$/u,
		);
	...
	it("serves minted preview HTML and assets without bearer headers when access token auth is enabled", async () => {
		const previousToken = process.env.OD_ACCESS_TOKEN;
		const token = `preview-token-${randomUUID()}`;
		process.env.OD_ACCESS_TOKEN = token;
		...
		try {
			...
			const writeIndex = await fetch(
				`${tokenBaseUrl}/api/projects/${projectId}/files`,
				{
					method: "POST",
					headers: authHeaders,
					body: JSON.stringify({
						...
						content:
							'<!doctype html><title>Hosted Preview</title><link rel="stylesheet" href="../styles/app.css">',
					}),
				},
			);
			expect(writeIndex.ok).toBe(true);

			const writeAsset = await fetch(
				`${tokenBaseUrl}/api/projects/${projectId}/files`,
				{
					method: "POST",
					headers: authHeaders,
			...
			if (previousToken === undefined) delete process.env.OD_ACCESS_TOKEN;
			else process.env.OD_ACCESS_TOKEN = previousToken;
		}
	});
	...
	it("rejects invalid preview scopes and escaping preview-url paths", async () => {
		...
		const invalidScope = await fetch(
			`${baseUrl}/api/projects/${projectId}/preview/bad/index.html`,
		);
