...
} from "../src/integrations/collab-cloud.js";
...
describe("durable Team comment relay outbox", () => {
	it("K4 reuses a durable event key across lost ACK, restart, and same-slug resume but rotates it for a changed mutation or alias", () => {
		const db = seededDb();
		const first = createCommentRelayOutboxStore(db, () => 100);
		const input = {
			workspaceId: "workspace-a",
			workspaceMemberId: "owner-a",
			teamId: "workspace-a",
			relayScope: "personal" as const,
			projectId: "p1",
			expectedOwnerMemberId: "owner-a",
			comment: previewCommentToCloud(
				comment({ authorMemberId: "owner-a" }),
				"owner-a",
			),
			publication: {
				slug: "same-alias",
		...
		first.enqueue(input);
		const original = first.listDue(100)[0]!;
		expect(original.eventKey).toMatch(/^[a-f0-9-]{36}$/);
		first.defer(original, {
			nextAttemptAt: 100,
			error: "remote committed, ACK lost",
		});
		expect(first.listDue(100)[0]?.eventKey).toBe(original.eventKey);
		expect(first.acknowledge(first.listDue(100)[0]!, "delivered")).toBe(true);
		closeDatabase();
		const reopened = openDatabase(tempDir!);
		const second = createCommentRelayOutboxStore(reopened, () => 100);
		second.enqueue({
			...input,
			publication: { ...input.publication, token: "rev-2" },
		});
		expect(second.listDue(100)[0]?.eventKey).toBe(original.eventKey);
		second.enqueue({
			...input,
			comment: previewCommentToCloud(
				comment({
		...
		const changed = second.listDue(100)[0]!;
		...
		second.enqueue({
			...
			publication: { ...input.publication, token: "rev-3" },
		});
		expect(second.listDue(100)[0]?.eventKey).toBe(changed.eventKey);
		second.enqueue({
			...input,
			comment: previewCommentToCloud(
				comment({
		...
		expect(second.listDue(100)[0]?.eventKey).not.toBe(changed.eventKey);
	});
	...
	it("backfills legacy personal file paths idempotently without touching malformed or unrelated rows", () => {
		tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "od-comment-relay-legacy-"),
		);
    ...
    `);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 100, 100, 100)
    `);
		const payload = (id: string, filePath: string) =>
			JSON.stringify(
		...
		insert.run(
			...
			"other-principal",
			"owner-b",
			payload("other-principal", "index.html"),
		);
		insert.run(
			"workspace-a",
			"owner-a",
		...
		expect(
			db
				.prepare(
					`SELECT file_path FROM comment_relay_outbox WHERE comment_id = 'exact'`,
				)
				.get(),
		).toEqual({ file_path: "index.html" });
		expect(
			db
				.prepare(
					`SELECT file_path FROM comment_relay_outbox WHERE comment_id = 'malformed'`,
				)
				.get(),
		).toEqual({ file_path: "" });
		migrateCommentRelayOutbox(db);
		...
		publications.set(exactScope, {
			url: "https://example.test/index",
			slug: "index",
			fileName: "index.html",
		});
		expect(
			publications.deleteIfRevisionMatches(
				exactScope,
				publications.getRevision(exactScope)!,
			),
		).toBe(true);
		expect(
			db
				.prepare(
					`SELECT comment_id, file_path FROM comment_relay_outbox ORDER BY comment_id`,
				)
				.all(),
		).toEqual([
	...
	it("rolls back publication deletion when personal outbox cancellation is aborted", () => {
		...
		outbox.enqueue({
			...
			projectId: "p1",
			expectedOwnerMemberId: "owner-a",
			comment: previewCommentToCloud(comment({ id: "atomic-row" }), "owner-a"),
		});
		db.exec(
			`CREATE TRIGGER abort_personal_outbox_delete BEFORE DELETE ON comment_relay_outbox BEGIN SELECT RAISE(ABORT, 'forced cancellation failure'); END;`,
		);

		const revision = publications.getRevision(scope)!;
		expect(() => publications.deleteIfRevisionMatches(scope, revision)).toThrow(
			"forced cancellation failure",
		);
		expect(publications.get(scope)?.slug).toBe("index");
		expect(outbox.listDue(100).map((record) => record.commentId)).toEqual([
			"atomic-row",
		]);
	...
	it("delivers an active personal owner publication through a durable outbox after restart", async () => {
		...
		const scope = (
			projectId: string,
			filePath: string,
			current: WorkspaceCollabContext,
		) =>
			commentRelayScope({
				binding,
				context: current,
		...
		const first = createCollabCloudService({
			...
			validateCommentRelayProjectBinding: (record) =>
				commentRelayLocalBindingMatches(record, binding),
		...
		expect(
			first.enqueueComment(
				comment({ authorMemberId: owner.workspaceMemberId }),
				owner,
			),
		).toBe(true);
		...
		closeDatabase();
		const reopened = openDatabase(tempDir!);
		const restartedPublications = createSqlitePublicFilePublicationStore(
			reopened,
			() => 100,
		);
		...
		const restarted = createCollabCloudService({
			client: clientWithPush(async (teamId, projectId) => {
				pushed.push({ teamId, projectId });
				return { seq: 9 };
			}),
	...
	it("fails closed for no publication, a different file, stopped publication, and a switched personal identity", async () => {
		...
		outbox.enqueue({
			...
			teamId: owner.workspaceId,
			relayScope: "personal",
			projectId: "p1",
			expectedOwnerMemberId: owner.workspaceMemberId,
			comment: previewCommentToCloud(
				comment({ id: "other-file", filePath: "other.html" }),
		...
		outbox.enqueue({
			...
			expectedOwnerMemberId: owner.workspaceMemberId,
			comment: previewCommentToCloud(
				comment({ id: "other-project", projectId: "p2" }),
				owner.workspaceMemberId,
			),
		});
		outbox.enqueue({
			workspaceId: owner.workspaceId,
			workspaceMemberId: "other-owner",
			teamId: owner.workspaceId,
			relayScope: "personal",
			projectId: "p1",
			expectedOwnerMemberId: "other-owner",
			comment: previewCommentToCloud(
				comment({ id: "other-principal" }),
				"other-owner",
			),
		});
		outbox.enqueue({
		...
		expect(
			publications.deleteIfRevisionMatches(
				publicationScope,
				publications.getRevision(publicationScope)!,
			),
		).toBe(true);
		publications.set(
			{
		...
		expect(
			outbox
				...
				.sort(),
		).toEqual(["other-file", "other-principal", "other-project", "team-row"]);
		...
		expect(controls.map((record) => record.commentId).sort()).toEqual([
			"other-file",
			"other-principal",
			"other-project",
			"team-row",
		...
		expect(
			restarted.enqueueComment(
				comment({
					id: "after-resume",
					note: "after-resume",
					authorMemberId: owner.workspaceMemberId,
				}),
				owner,
			),
		).toBe(true);
	...
	it.each([
		...
		"validates every personal publication in a mixed identity batch (%s)",
		async (_order, filePaths) => {
			const db = seededDb();
			const owner = context("owner", {
				workspaceId: "workspace-personal",
				workspaceType: "personal",
			...
			const publications = createSqlitePublicFilePublicationStore(
				db,
				() => 100,
			);
	...
	it("does not let a stale different-creator project block a valid personal project in the same identity batch", async () => {
		...
		const bindings = new Map<string, CommentRelayLocalProjectBinding>([
			...
			[
				"p2",
				{
					...
					visibility: "personal",
					resourceState: "active",
					createdByWorkspaceMemberId: otherCreator,
				},
			],
		]);
		const publications = createSqlitePublicFilePublicationStore(db, () => 100);
		for (const [projectId, creator] of [
			["p1", owner.workspaceMemberId],
			["p2", otherCreator],
		] as const) {
			publications.set(
				{
				...
				{
					url: `https://example.test/${projectId}`,
					slug: projectId,
					fileName: "index.html",
				},
			);
		...
		const scope = (
			...
			commentRelayScope({
				binding: bindings.get(projectId),
				context: current,
				projectId,
				filePath,
				publications,
		...
		const service = createCollabCloudService({
			...
			validateCommentRelayProjectBinding: (record) =>
				commentRelayLocalBindingMatches(record, bindings.get(record.projectId)),
			resolveCommentRelayWorkspaceContext: async () => freshIdentity,
			listRemoteProjectRelayBindings: async () => [],
			listProjectIds: () => [],
			resolveLocalConversationId: () => "conv-local",
		...
		outbox.enqueue({
			...
			relayScope: "personal",
			projectId: "p2",
			expectedOwnerMemberId: otherCreator,
			comment: previewCommentToCloud(
				comment({ id: "stale-p2", projectId: "p2" }),
				owner.workspaceMemberId,
			),
		});
	...
	it.each<CollabMemberRole>(["owner", "admin", "member"])(
		...
		async (role) => {
			...
			expect(
				service.enqueueComment(
					comment({ authorMemberId: queuedContext.workspaceMemberId }),
					queuedContext,
				),
			).toBe(true);
			expect(outbox.count()).toBe(1);
	...
	it("retries a failed push after restart and confirms the cloud sequence", async () => {
		...
		const secondService = createCollabCloudService({
			...
			resolveLocalConversationId: () => "conv-local",
			mergeComment: () => "unchanged",
			onCommentPushed: ({ commentId, seq, authorKey, memberId }) =>
				confirmed.push({ commentId, seq, authorKey, memberId }),
			now: () => 200,
			retryDelayMs: () => 0,
		});
		...
		expect(confirmed).toEqual([
			{
				...
				seq: 42,
				authorKey: "a".repeat(64),
				memberId: queuedContext.workspaceMemberId,
			},
		]);
	...
	it("retries in the running daemon after the relay recovers", async () => {
		...
		service.enqueueComment(
			comment({ authorMemberId: "member-owner" }),
			queuedContext,
		);
	...
	it("coalesces edits and delete tombstones without losing their latest state", async () => {
		...
		service.enqueueComment(
			comment({ note: "edited note", updatedAt: 20 }),
			queuedContext,
		);
		expect(outbox.count()).toBe(1);
		await service.flushPendingComments();
		...
		service.enqueueCommentDeletion(
			comment({ note: "edited note", updatedAt: 20 }),
			queuedContext,
		);
		await service.flushPendingComments();
	...
	it.each([
		...
		"terminally cancels after $name without pushing",
		async ({ remoteOwner }) => {
			const db = seededDb();
			const queuedContext = context("admin");
			let pushes = 0;
	...
	it("reports a missing remote owner while conditionally acknowledging only that revision", async () => {
		...
		const catalog =
			deferred<Array<{ projectId: string; ownerMemberId: string }>>();
		const pushed: string[] = [];
		...
		expect(
			service.enqueueComment(
				comment({ id: "missing-owner", note: "old revision" }),
				queuedContext,
			),
		).toBe(true);
		expect(
			service.enqueueComment(
				comment({ id: "valid-project", projectId: "p2" }),
				queuedContext,
			),
		).toBe(true);
		...
		expect(
			service.enqueueComment(
				comment({ id: "missing-owner", note: "new revision", updatedAt: 20 }),
				queuedContext,
			),
		).toBe(true);
	...
	it("reuses one fresh authority and one catalog snapshot for an exact identity batch", async () => {
		...
		for (const [projectId, commentId] of [
			...
			expect(
				service.enqueueComment(
					comment({ id: commentId, projectId }),
					queuedContext,
				),
			).toBe(true);
		}
	...
	it("keeps a newer revision queued when a batched push acknowledges an older payload", async () => {
		...
		service.enqueueComment(
			comment({ note: "newer note", updatedAt: 20 }),
			queuedContext,
		);
	...
	it.each<{
		...
		"does not push after $name even while the remote catalog is stale",
		async ({ binding }) => {
			const db = seededDb();
			const queuedContext = context("member");
			let pushes = 0;
			...
			const deps = Object.assign(
				{
					...
					validateCommentRelayProjectBinding: (
						record: Parameters<typeof commentRelayLocalBindingMatches>[0],
					) => commentRelayLocalBindingMatches(record, binding),
					listProjectIds: () => [],
					resolveLocalConversationId: () => "conv-local",
					mergeComment: () => "unchanged",
	...
	it("acknowledges only the rejected revision for a structured 410 SHARE_STOPPED and does not resurrect it after SQLite reopen", async () => {
		...
		const stdout = fs.readFileSync(
			new URL(
				"./fixtures/vela-cli-comment-push-share-stopped-927e0a62e7.stdout.json",
				import.meta.url,
			),
			"utf8",
		);
		...
		const service = createCollabCloudService({
			...
			resolveLocalConversationId: () => "conv-local",
			mergeComment: () => "unchanged",
			onCommentPushed: (event) => confirmed.push(event),
			onError: (error) => errors.push(error),
			now: () => 1_400,
			retryDelayMs: () => 0,
		});
		...
		const reopened = openDatabase(tempDir!);
		const restartedOutbox = createCommentRelayOutboxStore(
			reopened,
			() => 1_400,
		);
		expect(restartedOutbox.count()).toBe(0);
	...
	it.each([
		...
		"retries non-terminal structured failures without using message text",
		async (failure) => {
			const db = seededDb();
			const queuedContext = context("member");
