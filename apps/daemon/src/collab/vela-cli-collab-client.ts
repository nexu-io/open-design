import {
	...
	type CommentBatchPushRequest,
	type CommentBatchPushResponse,
} from "@open-design/contracts";
import {
	parseCommentBatchReceipt,
	validateCommentBatchRequest,
} from "./comment-batch-receipt.js";
import type {
	CollabCloudComment,
...
function toCloudComment(value: unknown): CollabCloudComment {
	...
	const author =
		wire.author &&
		typeof wire.author === "object" &&
		!Array.isArray(wire.author)
...
export function createVelaCliCollabClient(
	options: VelaCliCollabClientOptions = {},
) {
	...
	async function runJson<T>(
		args: string[],
		workspaceId: string,
		commandOptions?: Pick<VelaCommandOptions, "input">,
	): Promise<T> {
		...
		try {
			...
			throw new Error("Invalid JSON from Vela collaboration command", {
				cause,
			});
		}
	}
	...
	return {
		...
		async listMembers(
			...
			return Array.isArray(payload.members)
				? payload.members.map(toDirectoryEntry)
				: [];
		},

		async pushCommentBatch(
			workspaceId: string,
			projectId: string,
			request: CommentBatchPushRequest,
		): Promise<CommentBatchPushResponse> {
			if (!workspaceId.trim() || !projectId.trim())
				throw new Error("explicit batch scope is required");
			validateCommentBatchRequest(request);
			// Snapshot before awaiting the process: correlation cannot follow caller mutation.
			const captured: CommentBatchPushRequest = {
				comments: request.comments.map((item) => ({ ...item })),
			};
		...
		async pushComment(
			...
			const payload = await runJson<{
				...
				[
					...
					...(historicalAuthorMemberId ? ["--historical-author-member-id", historicalAuthorMemberId] : []),
				],
				_teamId,
			...
			return {
				...
				...(typeof authorKey === "string" && authorKey.trim()
					? { authorKey: authorKey.trim() }
					: {}),
			};
		},
		...
		async heartbeatPresence(
			...
			return Array.isArray(payload.viewers)
				? payload.viewers.map(toPresenceMember)
				: [];
		},
 ...
 */
export function collabCloudErrorFromVelaFailure(
	error: unknown,
): CollabCloudError | null {
	const stdout =
		error !== null &&
		typeof error === "object" &&
...
function commentCommandError(error: unknown): Error {
	...
	let payload: unknown;
	try {
		payload = JSON.parse(velaCommandStdout(error));
	} catch {
		return original;
	}
...
const defaultRunVelaCollab: RunVelaCollab = (args, workspaceId, options) =>
	runVelaCommand(["collab", ...args], {
		...
		...(args[0] === "presence"
			...
			: args[0] === "comment" &&
					(args[1] === "push" || args[1] === "pull" || args[1] === "push-batch")
				? { timeoutMs: COMMENT_COMMAND_TIMEOUT_MS }
				: {}),
	}).catch((error: unknown) => {
		throw args[0] === "comment" && args[1] === "push"
			? commentCommandError(error)
			: error;
	});
...
export function shouldUseVelaCliCollabTransport(
	...
	return (
		env.OD_TEAM_PROJECTS_TRANSPORT?.trim() === "vela-cli" ||
