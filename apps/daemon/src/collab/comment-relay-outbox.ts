...
import {
	acknowledgePublishedCommentMutation,
	currentCommentRelayPublicationMapping,
	migrateCommentRelayPublicationMappings,
} from "./comment-relay-publication-mapping.js";
...
export interface CommentRelayOutboxRecord extends CommentRelayOutboxIdentity {
	...
	revision: number;
	/** Stable for one logical mutation even after ACK, stop, restart, or same-alias resume. */
	eventKey: string;
	attemptCount: number;
...
export interface CommentRelayOutboxStore {
	enqueue(
		input: CommentRelayOutboxIdentity & {
			projectId: string;
			expectedOwnerMemberId: string | null;
			comment: CollabCloudComment;
			publication?: CommentRelayPublicationWitness;
...
export function commentRelayLocalBindingMatches(
	...
	const currentOwnerMemberId =
		binding.createdByWorkspaceMemberId?.trim() || null;
...
export function migrateCommentRelayOutbox(db: SqliteDb): void {
    ...
    CREATE TABLE IF NOT EXISTS comment_relay_mutation_keys (
      workspace_id TEXT NOT NULL, workspace_member_id TEXT NOT NULL,
      project_id TEXT NOT NULL, comment_id TEXT NOT NULL,
      source_file_path TEXT NOT NULL, publication_slug TEXT NOT NULL,
      payload_fingerprint TEXT NOT NULL, event_key TEXT NOT NULL,
      PRIMARY KEY (workspace_id, workspace_member_id, project_id, comment_id)
    );
  ...
  `);
	const columns = db
		.prepare("PRAGMA table_info(comment_relay_outbox)")
		.all() as Array<{ name: string }>;
	if (!columns.some((column) => column.name === "publication_json")) {
		db.exec(
			"ALTER TABLE comment_relay_outbox ADD COLUMN publication_json TEXT",
		);
	}
	if (!columns.some((column) => column.name === "event_key"))
		db.exec("ALTER TABLE comment_relay_outbox ADD COLUMN event_key TEXT");
	// Compatible with rows written before personal publication relay existed.
	try {
		db.exec(
			"ALTER TABLE comment_relay_outbox ADD COLUMN relay_scope TEXT NOT NULL DEFAULT 'team'",
		);
	} catch {
		/* already migrated */
	}
	try {
		db.exec(
			"ALTER TABLE comment_relay_outbox ADD COLUMN file_path TEXT NOT NULL DEFAULT ''",
		);
	} catch {
		/* already migrated */
	}
	...
	db.exec(`UPDATE comment_relay_outbox SET event_key = lower(hex(randomblob(16))) WHERE event_key IS NULL;
    INSERT OR IGNORE INTO comment_relay_mutation_keys(workspace_id,workspace_member_id,project_id,comment_id,source_file_path,publication_slug,payload_fingerprint,event_key)
    SELECT workspace_id,workspace_member_id,project_id,comment_id,file_path,COALESCE(json_extract(publication_json, '$.slug'), ''),'',event_key FROM comment_relay_outbox WHERE event_key IS NOT NULL`);
...
function parsePublicationWitness(
	value: unknown,
): CommentRelayPublicationWitness {
	if (
		!value ||
		typeof value !== "object" ||
		!("slug" in value) ||
		...
		!value.token ||
		!("publicFilePath" in value) ||
		typeof value.publicFilePath !== "string" ||
		!value.publicFilePath
	) {
	...
	return {
		slug: value.slug,
		token: value.token,
		publicFilePath: value.publicFilePath,
	};
}

function parseRecord(
	row: Record<string, unknown>,
): CommentRelayOutboxRecord | null {
	try {
		...
		if (
			...
			(row.expectedOwnerMemberId !== null &&
				typeof row.expectedOwnerMemberId !== "string")
		) {
			return null;
		}
		return {
			...
			...(row.publicationJson == null
				? {}
				: {
						publication: parsePublicationWitness(
							JSON.parse(String(row.publicationJson)),
						),
					}),
...
export function createCommentRelayOutboxStore(
    ...
    INSERT INTO comment_relay_outbox
      (workspace_id, workspace_member_id, team_id, relay_scope, project_id, file_path, comment_id,
       ...
       payload_json, publication_json, event_key, revision, attempt_count, next_attempt_at, last_error,
  ...
  `);
	const countRows = db.prepare(
		`SELECT COUNT(*) AS count FROM comment_relay_outbox`,
	);
	...
	return {
		enqueue(input) {
			db.transaction(() => {
				...
				const publication =
					input.publication ??
					currentCommentRelayPublicationMapping(db, {
						resourceTeamId: input.teamId,
						ownerMemberId: input.workspaceMemberId,
				...
				const fingerprint = createHash("sha256")
					.update(
						JSON.stringify([
							input.comment,
							publication?.slug ?? "",
							publication?.publicFilePath ?? "",
						]),
					)
					.digest("hex");
				const saved = db
					.prepare(`SELECT payload_fingerprint AS fingerprint, event_key AS eventKey FROM comment_relay_mutation_keys
        WHERE workspace_id=? AND workspace_member_id=? AND project_id=? AND comment_id=?`)
					.get(
						input.workspaceId,
						input.workspaceMemberId,
						input.projectId,
						input.comment.id,
					) as { fingerprint: string; eventKey: string } | undefined;
				const eventKey =
					saved?.fingerprint === fingerprint ? saved.eventKey : randomUUID();
				db.prepare(`INSERT INTO comment_relay_mutation_keys
        (workspace_id,workspace_member_id,project_id,comment_id,source_file_path,publication_slug,payload_fingerprint,event_key)
        VALUES(?,?,?,?,?,?,?,?)
        ON CONFLICT(workspace_id,workspace_member_id,project_id,comment_id)
        DO UPDATE SET source_file_path=excluded.source_file_path,publication_slug=excluded.publication_slug,
          payload_fingerprint=excluded.payload_fingerprint,event_key=excluded.event_key`).run(
					input.workspaceId,
					input.workspaceMemberId,
					input.projectId,
					input.comment.id,
		...
		isPublicationCurrent(record) {
			...
			return Boolean(
				db
					.prepare(`SELECT 1 FROM public_file_publications
        WHERE resource_team_id = ? AND owner_member_id = ? AND project_id = ?
          AND file_path = ? AND slug = ? AND revision = ?`)
					.get(
						...
						record.publication.token,
					),
			);
		},
		listDue(timestamp, limit = 64) {
			return (
				listDueRows.all(timestamp, Math.max(1, Math.round(limit))) as Record<
					string,
					unknown
				>[]
			)
				.map(parseRecord)
				.filter(
					(record): record is CommentRelayOutboxRecord => record !== null,
				);
		},
		acknowledge(record, outcome = "discarded") {
			return db.transaction(() => {
				...
				if (changed) {
					markPublishedCommentBackfillOutcome(db, record, outcome);
					if (
						outcome === "delivered" &&
						record.relayScope === "personal" &&
						record.publication
					) {
						acknowledgePublishedCommentMutation(db, {
							resourceTeamId: record.teamId,
							ownerMemberId: record.workspaceMemberId,
							projectId: record.projectId,
							filePath: record.comment.filePath,
							commentId: record.commentId,
							slug: record.publication.slug,
							revision: record.publication.token,
							status: record.comment.status,
							deleted: record.comment.deleted,
						});
					}
				}
				return changed;
		...
		defer(record, input) {
			return db.transaction(() => {
				...
				if (changed)
					markPublishedCommentBackfillOutcome(db, record, "deferred");
				if (changed)
					db.prepare(`INSERT INTO comment_relay_sync_failures(workspace_id, workspace_member_id, project_id, failed_at)
          VALUES (?, ?, ?, ?) ON CONFLICT(workspace_id, workspace_member_id, project_id)
          DO UPDATE SET failed_at=excluded.failed_at`).run(
						record.workspaceId,
						record.workspaceMemberId,
						record.projectId,
						now(),
		...
		count() {
			return Number(
				(countRows.get() as { count?: unknown } | undefined)?.count ?? 0,
			);
		},
	};
}
