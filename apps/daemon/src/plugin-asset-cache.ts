...
/** Extensions that are safe to proxy through the asset cache. */
export const CACHEABLE_PREVIEW_ASSET_EXT =
	/\.(?:png|jpe?g|webp|gif|avif|svg|mp4|webm|mov|m4v|ogg|mp3|wav|woff2?|ttf|otf|css|js|mjs)(?:$|[?#])/i;

/** Legacy alias — kept so existing callers don't break. */
export const CACHEABLE_MEDIA_EXT = CACHEABLE_PREVIEW_ASSET_EXT;
...
export function isCacheableExternalUrl(raw: unknown): raw is string {
	...
	// Google Fonts CSS URLs are always cacheable.
	if (isGoogleFontsCssUrl(value)) return true;
	const pathOnly = value.split(/[?#]/)[0] ?? "";
	return (
		CACHEABLE_PREVIEW_ASSET_EXT.test(pathOnly) ||
		CACHEABLE_PREVIEW_ASSET_EXT.test(value)
	);
}

/** True when the URL points at a Google Fonts CSS API endpoint. */
export function isGoogleFontsCssUrl(raw: string): boolean {
	try {
		const url = new URL(raw);
		return (
			url.hostname === "fonts.googleapis.com" && /^\/css2?\b/.test(url.pathname)
		);
	} catch {
		return false;
	}
...
/** Per-kind byte limit for asset-cache fetches. */
export function maxBytesForAssetKind(
	rawUrl: string,
	defaultMax: number,
): number {
	if (isGoogleFontsCssUrl(rawUrl)) return 2 * 1024 * 1024; // 2 MiB for Google Fonts CSS
	const value = rawUrl.trim();
	const pathOnly = value.split(/[?#]/)[0] ?? "";
	if (/\.(?:css)(?:$|[?#])/i.test(pathOnly)) return 2 * 1024 * 1024; // 2 MiB
	if (/\.(?:js|mjs)(?:$|[?#])/i.test(pathOnly)) return 5 * 1024 * 1024; // 5 MiB
	if (/\.(?:woff2?|ttf|otf)(?:$|[?#])/i.test(pathOnly)) return 10 * 1024 * 1024; // 10 MiB
	return defaultMax; // media
}

/** Rewrite CSS content so `url(...)` references are proxied through the asset
 *  cache. Handles: url(https://...), url('/relative/path'), @import url(...),
 *  and @import "...". Unsupported URLs are left as-is. */
export function rewriteCssAssetUrls(
	...
	let base: URL;
	try {
		base = new URL(stylesheetBaseUrl);
	} catch {
		return css;
	}
	const rewriteUrl = (raw: string): string => {
		...
		// Absolute external URL → proxy through asset cache if cacheable.
		if (/^https?:\/\//i.test(trimmed)) {
			if (isCacheableExternalUrl(trimmed)) return assetCacheRewriteUrl(trimmed);
			return raw;
		}
		// Data URIs and fragments → leave as-is.
		if (/^(?:data:|#)/i.test(trimmed)) return raw;
		// Relative URL → resolve against stylesheet base, then proxy if cacheable.
		try {
			const resolved = new URL(trimmed, base).href;
			if (isCacheableExternalUrl(resolved))
				return assetCacheRewriteUrl(resolved);
		} catch {
	...
	let result = css.replace(/url\(\s*([^)'"\s]+)\s*\)/gi, (match, rawValue) => {
		return `url(${rewriteUrl(String(rawValue))})`;
	});
	// Rewrite @import url(...) and @import "..." .
	result = result.replace(
		/@import\s+url\(\s*([^)'"\s]+)\s*\)/gi,
		(match, rawValue) => {
			return `@import url(${rewriteUrl(String(rawValue))})`;
		},
	);
	result = result.replace(
		/@import\s+(['"])([^'"]+)\1/gi,
		(match, quote, rawValue) => {
			const rewritten = rewriteUrl(String(rawValue));
			// If the URL was rewritten to an asset-cache URL, use url() form.
			if (
				rewritten !== String(rawValue) &&
				/^\/api\/asset-cache\?/.test(rewritten)
			) {
...
export function createValidatingLookup(lookupImpl: DnsLookupCb = dnsLookupCb) {
	return function validatingLookup(
		...
		(
		...
		)(hostname, opts, (err, address, family) => {
			...
			for (const entry of list) {
				...
				if (isPrivateAddress(addr)) {
					return cb(
						new AssetCacheError(400, "host resolves to a private address"),
					);
				}
			}
...
export function createPluginAssetCache(
	opts: PluginAssetCacheOptions,
): PluginAssetCache {
	...
	async function writeToDisk(
		...
		await writeFile(
			`${blobPath}.json`,
			JSON.stringify({ contentType: result.contentType }),
		);
	}
	...
	function resolveContentType(headerValue: string | null, url: URL): string {
		...
		// Accept font MIME types.
		if (/^(?:image|video|audio|font)\//.test(header)) return header;
		// Accept text/css
		if (header === "text/css") return "text/css; charset=utf-8";
		// Accept JavaScript MIME types.
		if (/^(?:application|text)\/(?:javascript|ecmascript)/.test(header))
			return "application/javascript; charset=utf-8";
		const ext = path.posix.extname(url.pathname).toLowerCase();
		const guessed = EXT_TO_MIME[ext];
		if (guessed) {
			// Add charset for text types.
			if (guessed === "text/css") return "text/css; charset=utf-8";
	 ...
	 *  `bodyMaxBytes`. A response without a trustworthy Content-Length must never be
	...
	async function fetchAndStore(
		...
		const kindMaxBytes = maxBytesForAssetKind(rawUrl, maxBytes);
		...
		try {
			...
			if (Number.isFinite(declaredLength) && declaredLength > kindMaxBytes) {
				throw new AssetCacheError(413, "asset exceeds size limit");
			}
			...
			let buf = await readBodyCapped(response, controller, kindMaxBytes);
			// Apply CSS URL rewriting for stylesheets and Google Fonts CSS.
			if (contentType.startsWith("text/css") || isGoogleFontsCssUrl(rawUrl)) {
				contentType = "text/css; charset=utf-8";
				const cssText = buf.toString("utf8");
				const rewritten = rewriteCssAssetUrls(cssText, rawUrl);
				buf = Buffer.from(rewritten, "utf8");
				if (buf.byteLength > kindMaxBytes) {
					throw new AssetCacheError(413, "rewritten CSS exceeds size limit");
				}
			}
			// Validate JS size.
			if (
				contentType.startsWith("application/javascript") &&
				buf.byteLength > kindMaxBytes
			) {
				throw new AssetCacheError(413, "javascript exceeds size limit");
			}
	...
	async function get(rawUrl: string): Promise<AssetCacheResult> {
		if (!isCacheableExternalUrl(rawUrl)) {
			throw new AssetCacheError(
				400,
				"url is not a cacheable external media url",
			);
		}
