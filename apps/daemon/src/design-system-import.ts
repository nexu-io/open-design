...
function renderDesignMd(id: string, name: string, scan: ProjectScan): string {
  ...
  const fonts = scan.fonts.map((font) => `- \`${font.relPath}\``);
  return [
    `# ${name}`,
    '',
    '> Category: Imported',
    ...
    '',
    '- Prefer source-backed rules from token files, theme files, reusable component files, and production UI surfaces.',
    '- Treat local/GitHub code as implementation evidence for token values, component states, and reusable interaction patterns.',
    '- Treat loose logos, icons, and font files as brand cues only; do not infer a complete UI system from assets alone.',
    '- Ignore one-off decorative colors, deprecated drafts, unrelated screenshots, and isolated marketing art unless another source confirms them.',
    '- When sources conflict, record the chosen rule and the conflict in Extraction Confidence instead of blending both silently.',
    '',
    '## Product Context',
    '',
    ...
    '## Typography',
    '',
    fonts.length > 0 ? fonts.join('\n') : '- No font files were found; tokens.css uses the importer fallback font stack.',
    '',
    '## Spacing, Layout, Shape, and Elevation',
    '',
    '- Use `tokens.css` and `source/token-contract.report.json` for spacing scale, container width, gutters, radius, elevation, focus ring, and motion confidence.',
    '- Preserve density and layout rhythm from representative source components before inventing new spacing.',
    '- Treat fallback radius, shadow, focus, and motion values as review-needed unless source evidence confirms them.',
    '',
    '## Component Signals',
    '',
    ...
    '## Component States',
    '',
    '- Document primary, secondary, ghost, destructive, disabled, hover, active, focus, loading, tooltip, and input error states when source evidence exists.',
    '- If a state is missing from the source, mark it unknown rather than inventing a parallel style.',
    '',
    '## Motion & Interaction',
    '',
    '- Use source-backed hover timing, easing, loading motion, page transitions, and reduced-motion behavior when available.',
    '- If motion is not implemented in the source, keep interaction rules conservative and token-backed.',
    '',
    '## Assets',
    '',
    ...
    '## Voice & Brand',
    '',
    '- Preserve product terminology, capitalization, density, and brand tone from README and source UI text when available.',
    '',
    '## Anti-patterns',
    '',
    '- Do not overuse accent color, generic gradients, mismatched radius, unrelated illustration palettes, or component shapes not supported by the source.',
    '- Do not collapse implementation tokens into vague prose when source-backed values exist.',
    '',
    '## Extraction Confidence',
    '',
    '- Label major rules as source-backed, inferred, fallback, or unknown during review.',
    '- Use `source/evidence.md`, `source/tokens.source.json`, and `source/token-contract.report.json` to audit which rules came from real files.',
    '',
    '## Agent Guidance',
    '',
    '- Use `tokens.css` as the first source of truth for color, radius, spacing, type, elevation, focus, and motion.',
