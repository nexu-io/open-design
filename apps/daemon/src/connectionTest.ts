...
export async function validateBaseUrlResolved(
  ...
  // Issue #3225 — an operator who trusts this hostname has opted it out of
  // the guard entirely, so skip the resolved-IP block even though it points
  // into private space. The sync check above already honored a literal-IP
  // allowlist entry; this covers the hostname-that-resolves-private case.
  ...
  for (const addr of addresses) {
    ...
    // A resolved address the operator explicitly allowlisted (they listed
    // the IP rather than the hostname) is permitted; everything else in
    // private space is still blocked.
 ...
 * `OD_ALLOWED_INTERNAL_HOSTS` allowlist (issue #3225), so an internally
 * hosted gateway on an RFC1918 address can be reached when — and only when —
