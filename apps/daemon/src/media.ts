...
type MediaOutputMeta = {
  name?: string;
  size?: number;
};

type MediaTaskTerminalState = {
  status: string;
  file: MediaOutputMeta | null;
  error: { message: string; status: number; code?: string } | null;
  endedAt: number | null;
};

/**
 * Confirm the generated bytes actually landed on disk before a media task
 * is marked done. generateMedia already stat()s after writeFile, but task
 * completion and chat-run status can diverge from reality when metadata
 * outlives the file or a handoff path marks success too early (#2892).
 */
export async function verifyMediaOutputOnDisk(
  projectsRoot: string,
  projectId: string,
  ...
  if (!name) {
    throw new Error('media output metadata is missing file name');
  }
  const safeName = sanitizeName(name);
  const target = path.join(projectsRoot, projectId, safeName);
  const st = await stat(target);
  if (!st.isFile()) {
    throw new Error(`media output path is not a file: ${safeName}`);
  }
  if (st.size <= 0) {
    throw new Error(`media output file is empty: ${safeName}`);
  }
  if (typeof meta.size === 'number' && meta.size > 0 && st.size !== meta.size) {
    throw new Error(
      `media output size mismatch for ${safeName} (expected ${meta.size}, got ${st.size})`,
    );
  }
}

export async function finalizeMediaTaskFromGenerateResult(
  task: MediaTaskTerminalState,
  meta: MediaOutputMeta,
  projectsRoot: string,
  projectId: string,
): Promise<'done' | 'failed'> {
  try {
    await verifyMediaOutputOnDisk(projectsRoot, projectId, meta);
    task.status = 'done';
    task.file = meta;
    task.error = null;
