...
import type { BrowserUseDiscoveryFacts, BrowserUseRunState } from '@marketing-ax/contracts';
...
export function isBrowserUseRequested(...values: unknown[]): boolean {
  return values.some((value) => (
    ...
    (
      ...
      value.includes('Use the selected Marketing AX Browser tab as the bound target.')
    )
  ));
}
...
export function renderBrowserUseUnavailablePrompt(state: BrowserUseRunState | null): string {
  ...
  return [
    ...
    '',
    `Browser automation was requested, but Marketing AX has not confirmed a matching in-app browser backend for this run. Reason: \`${state.reason}\`.`,
