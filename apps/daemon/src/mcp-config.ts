  ...
  const normalized = Array.from(
    new Set(
      (directories ?? [])
        ...
        .flatMap((dir) => allowedDirectoryAliases(dir)),
    ),
  );
...
function allowedDirectoryAliases(dir: string): string[] {
  const normalized = normalizeAllowedDirectory(dir);
  try {
    const canonical = normalizeAllowedDirectory(realpathSync.native(normalized));
