...
async function doWrite(dataDir: string, body: unknown): Promise<McpConfig> {
  ...
  // The config carries user-provided `env` / `headers` maps that can hold
  // MCP secrets — same owner-only lockdown as the sibling token stores.
  try {
    await chmod(file, 0o600);
  } catch (err: unknown) {
    const e = err as { code?: string };
    if (e.code !== 'ENOTSUP' && e.code !== 'EPERM') {
      console.warn('[mcp-config] could not chmod 0600', file, e.code ?? 'unknown');
    }
  }
  return next;
}
