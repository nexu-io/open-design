...
export class ManagedChatGptTenantManager {
  ...
  readonly #previewSubjects = new Map<string, string>();
    ...
    await Promise.all(
      stale.map(async ([subject, entry]) => {
        const stopping = entry.daemon.stop().finally(() => {
          if (this.#entries.get(subject) === entry) {
            this.#entries.delete(subject);
            this.#previewSubjects.delete(entry.previewToken);
          }
  ...
  async stopAll(): Promise<void> {
    ...
    this.#entries.clear();
    this.#previewSubjects.clear();
    await Promise.allSettled(entries.map((entry) => entry.daemon.stop()));
  }

  publicPreviewUrl(subject: string, internalUrl: string, publicBaseUrl: string): string {
    const entry = this.#entries.get(subject);
    if (!entry?.daemon.isRunning()) {
      throw new Error('The managed ChatGPT tenant is not active.');
    }
    const internal = new URL(internalUrl);
    const daemonOrigin = new URL(entry.daemon.url).origin;
    if (internal.origin !== daemonOrigin || !isAllowedTenantPreviewPath(internal.pathname)) {
      throw new Error('Only tenant-owned raw artifact previews can be published.');
    }
    const prefix = `/chatgpt/preview/${encodeURIComponent(entry.previewToken)}`;
    return new URL(`${prefix}${internal.pathname}${internal.search}`, publicBaseUrl).toString();
  }
  ...
  async fetchPreview(
    ...
    const subject = this.#previewSubjects.get(token);
    const entry = subject ? this.#entries.get(subject) : null;
    if (!subject || !entry?.daemon.isRunning() || entry.previewToken !== token) {
      return Response.json({ error: 'preview_not_found' }, { status: 404 });
    }
    const requested = new URL(pathAndSearch, 'http://tenant.invalid');
    if (!isAllowedTenantPreviewPath(requested.pathname)) {
      return Response.json({ error: 'preview_path_forbidden' }, { status: 403 });
    }
    entry.lastUsedAtMs = this.#now();
    const target = new URL(`${requested.pathname}${requested.search}`, `${entry.daemon.url}/`);
  ...
  async #replaceTenant(subject: string, accessToken: string): Promise<string> {
    ...
    if (previous) {
      this.#entries.delete(subject);
      this.#previewSubjects.delete(previous.previewToken);
    ...
    this.#previewSubjects.set(previewToken, subject);
    return daemon.url;
  }
...
function isAllowedTenantPreviewPath(pathname: string): boolean {
  try {
    const decoded = decodeURIComponent(pathname);
    const segments = decoded.split('/');
    return /^\/api\/projects\/[^/]+\/raw\/.+/u.test(decoded)
