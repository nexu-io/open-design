...
export interface BYOKToolContext {
  ...
  projectId: string;
  /** Stored project metadata. When `baseDir` is set, generated BYOK
   *  media must land in that project folder rather than the daemon's
...
export async function executeGenerateSpeech(
  ...
  let dir: string;
  try {
    dir = await ensureProject(ctx.projectsRoot, ctx.projectId, ctx.metadata ?? undefined);
  } catch (err) {
    return {
      ok: false,
