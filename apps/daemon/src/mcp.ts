...
export const MCP_SERVER_INSTRUCTIONS = [
  ...
  'If get_run returns failureAction:"recharge", show rechargeUrl; after the user confirms top-up, retry the original start_run once with the same requestId and resume:true.',
  'Poll get_run until completion. Runs can take 5–30 minutes; status:running with unchanged files is not a hang. Do not cancel and substitute write_file as a shortcut.',
  'Only call cancel_run if the user explicitly asks to abort.',
  'Use get_artifact to pull a complete design bundle; use get_file/search_files/list_files only when narrower access is needed.',
  'Project arguments accept UUIDs or name substrings; confirm resolvedProject if the match is unexpected.',
  'Reference material is available at od://design-systems/<id>/DESIGN.md and od://skills/<id>/SKILL.md.',
].join('\n');
