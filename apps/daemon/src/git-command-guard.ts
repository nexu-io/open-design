  ...
  switch (command.value) {
    ...
    case 'checkout':
      return hasDestructiveCheckoutArg(args, command.index, {
        ...
        : null;
    case 'switch':
      return args.slice(command.index + 1).some(isDestructiveSwitchArg)
        ? 'git switch can discard workspace changes'
        : null;
    case 'restore':
...
function checkoutOptionConsumesSeparateValue(arg: string): boolean {
  return arg === '-b' || arg === '--orphan' || shortOptionBundleContains(arg, 'b');
}
...
option_consumes_next() {
  case "$1" in
    -C|-c|--config-env|--exec-path|--git-dir|--namespace|--object-directory|--super-prefix|--work-tree) return 0 ;;
    *) return 1 ;;
  esac
}
...
case "$cmd" in
  ...
  push)
    ...
    for arg in "$@"; do
      ...
      case "$arg" in
        --force|--force-with-lease*|+*) blocked="git push --force can rewrite remote history" ;;
        -[!-]*) case "\${arg#-}" in *f*) blocked="git push --force can rewrite remote history" ;; esac ;;
      esac
    done
    ;;
  checkout)
    ...
    for arg in "$@"; do
      ...
      case "$arg" in
        --force) blocked="git checkout can discard workspace changes" ;;
