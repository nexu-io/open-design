...
function isDestructiveSwitchArg(arg: string): boolean {
  return arg === '-f' || arg === '--force' || arg === '--discard-changes';
}
...
case "$cmd" in
  ...
  switch)
    ...
    for arg in "$@"; do
      ...
      case "$arg" in -f|--force|--discard-changes) blocked="git switch can discard workspace changes" ;; esac
    done
    ;;
  restore)
