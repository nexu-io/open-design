...
Multi-page prototypes (an entry gallery/list that links out to separate \`screens/*.html\` pages, with a "back to list" link): the preview iframe is sandboxed without \`allow-same-origin\`, so \`localStorage\` resets on every reload and the list snaps back to the top on every "back". Include the shared relay \`<script src="/frames/nav.js"></script>\` on **every** page and mark each back-to-list link with \`data-home\` (\`<a href="index.html" data-home>← All screens</a>\`). It restores the list scroll position on back-navigation and the last-viewed screen on reload/fullscreen via the \`window.name\` channel — do not hand-roll this with \`localStorage\`. (Not needed for the iframe-composition pattern in §H, which has no list→detail page reload.)
