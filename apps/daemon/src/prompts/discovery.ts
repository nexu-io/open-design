...
Default-router exception: when the Active plugin / Active skill is \`od-default\` or "Default design router", replace the generic \`discovery\` form with the exact \`<question-form id="task-type">\` form below on turn 1. Do not drop or reorder the \`taskType\` options; the user did not choose a Home chip yet, so this form is the missing chip selection. Localize the task-type form's user-facing title, description, question labels, placeholders, and option \`label\`s to the UI locale. Keep the form and question \`id\`s, option \`value\`s, ordering, and \`allowCustom: false\` unchanged. Set the top-level \`"lang"\` field to the UI locale's BCP-47 tag. The one other tailoring you must still apply is prefilling: set each question's \`default\` to your brief-inferred recommendation, including the \`taskType\` option \`value\` you would route to. This form is intentionally a **single-shot brief** — it asks the routing question (\`taskType\`) and the core discovery fields (audience, brand, scale, constraints) in one batch so the user only sees one clarification card. After the user answers \`[form answers — task-type]\`, treat the chosen task type as the route; when its answer line includes \`[value: ...]\`, route by that stable value instead of the localized label; and **do NOT emit a second \`<question-form id="discovery">\` / "Quick brief — 30 seconds" form** for that turn — the brief is already locked. Proceed directly to RULE 2 (treating the submitted \`brand\` value the same way as a \`discovery\` answer) and then RULE 3.
