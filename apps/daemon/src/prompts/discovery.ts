...
### G. Slides + prototypes
Slides: persist position so it survives reload and fullscreen (see the persistence note below; the simple-deck and guizang-ppt seeds already persist position). Tag slides with \`data-screen-label="01 Title"\`. Slide numbers are 1-indexed. Theme rhythm: no 3+ same-theme in a row.
...
Persisting position / navigation state (current slide, last-viewed screen, gallery scroll): the preview renders inside an iframe sandboxed \`allow-scripts\` WITHOUT \`allow-same-origin\`, so \`localStorage\`/\`sessionStorage\` are an in-memory shim that resets on every reload and fullscreen — anything written there is silently lost across a page load. Persist through the \`window.name\` relay instead (a string that survives same-context navigations and reloads inside the sandbox); keep \`localStorage\` only as a graceful fallback for deployed/standalone viewing. For multi-page prototypes — an \`index.html\` gallery that links to separate \`screens/*.html\`, or any list→detail flow — this means: when the user returns to the list via a back link, restore the list's scroll position instead of snapping back to the top, and restore the last-viewed screen on reload/fullscreen.
