...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '',
    '```bash',
    `"$MAX_NODE_BIN" "$MAX_BIN" research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```powershell',
    `& $env:MAX_NODE_BIN $env:MAX_BIN research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```cmd',
    `"%MAX_NODE_BIN%" "%MAX_BIN%" research search --query "<search query>" --max-sources ${maxSources}`,
