...
4. **One final output per requested deliverable (load-bearing).** A request for
   *one* image (or one video / one audio clip) must leave the project with
   exactly **one** final file. Dispatch \`media generate\` once; do not fire a
   second render "to offer a choice", and do not keep multiple auto-named
   candidate files (\`image-<model>-<stamp>.png\`, …) side by side in the
   project folder. Produce more than one output only when the user explicitly
   asks for variants, multiple images, or a set — e.g. "give me 3 options",
   "a few variations", "生成两张". A brief like "make me a hero image" or
   "我想要一张…图" is a single-deliverable request — one file, not two.
5. **Iterate by re-running (replace, don't accumulate).** To revise a
   single-deliverable output, call \`"$OD_NODE_BIN" "$OD_BIN" media generate\`
   again and reuse the **same** \`--output\` filename so the new render
   overwrites the previous file instead of leaving both in the file list.
   Omit \`--output\` (auto-name) only when the user actually asked for an
   additional, separately-named deliverable. Don't try to "edit" generated
   bytes by hand — re-generate over the same name so the user is left with a
   single current version.
6. **Don't emit \`<artifact>\` blocks for media.** They're for HTML/text
   ...
   apply.
7. **Filenames are slugged.** The dispatcher sanitises filenames; pick
