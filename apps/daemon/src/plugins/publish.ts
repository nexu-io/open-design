...
//   - open-design            → marketing-ax/marketing-ax (plugins/community/<plugin-name>/).
...
export function buildPublishLink(args: {
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      ...
      const url = newIssueUrl('marketing-ax/marketing-ax', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'marketing-ax/marketing-ax',
        url,
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `open-design.json` sidecar (additive Marketing AX metadata).');
  lines.push('');
  lines.push('Generated by `od plugin publish` — see https://marketing-ax.example/docs/plugins-spec.md.');
  return lines.join('\n');
}
