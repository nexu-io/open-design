...
export interface AttachAcpSessionOptions {
  ...
  /** Pi requires affirmative runtime/model evidence from the spawned Vela. */
...
export function attachAcpSession({
  ...
  const parser = createJsonLineStream((raw, rawLine) => {
    ...
    if (expectedId === 2) {
      if (expectedAmrRuntime === 'pi') {
        if (
          ...
          || !/^pi-[a-f0-9]{32}$/.test(result.durableSessionId)
        ) {
          fail('The selected AMR Pi runtime did not report a valid Pi session and version.', {
            retryable: false, details: { kind: 'amr_runtime_mismatch' },
          });
          return;
        }
        amrRuntimeEvidence = {
          requestedRuntime: 'pi', actualRuntime: 'pi', runtimeVersion: result.runtimeVersion,
        };
    ...
    if (promptRequestId !== null && obj.id === promptRequestId) {
      if (expectedAmrRuntime === 'pi') {
        const actualModel = typeof result.modelId === 'string' ? result.modelId.replace(/^amr\//, '') : null;
        if (
          result.runtime !== 'pi'
          || result.runtimeVersion !== amrRuntimeEvidence?.runtimeVersion
          || !model || actualModel !== model.replace(/^amr\//, '')
        ) {
          emitUsageIfPresent(result.usage);
          fail('AMR Pi returned a different or unverified runtime/model.', {
    ...
    if (sessionId && model && model !== 'default' && obj.id === expectedId) {
      if (expectedAmrRuntime === 'pi') {
        const actualModel = currentModelFromSessionResult(result)?.replace(/^amr\//, '');
        if (!actualModel || actualModel !== model.replace(/^amr\//, '')) {
          fail('AMR Pi did not confirm the selected model; the prompt was not sent.', {
