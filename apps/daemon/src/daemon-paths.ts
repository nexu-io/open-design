...
export function resolveProcessResourcesPath(
  ...
  // <appRoot>/resources from the project root so the resource-root guard accepts
  // the bundled tree without widening it to arbitrary paths.
  return projectRoot ? resolveBundledNodeAppResourcesPath(projectRoot) : null;
}
