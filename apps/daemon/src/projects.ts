...
const RECONCILABLE_EXTENSIONS = new Set(['.html', '.htm']);

export async function ensureArtifactSidecarFor(projectsRoot, projectId, name, metadata?) {
  ...
  const inferredMetadata =
    inferred.metadata && typeof inferred.metadata === 'object' && !Array.isArray(inferred.metadata)
      ? inferred.metadata
      : {};
  const fileTime = new Date(targetStat.mtimeMs).toISOString();
  const validated = validateArtifactManifestInput(
    {
      ...inferred,
      createdAt: fileTime,
      updatedAt: fileTime,
      metadata: { ...inferredMetadata, inferred: true, reconciled: true },
    },
    safeName,
