...
- Spec E team resource sharing (cloud storage) lives in three files: the hub HTTP client `src/integrations/resource-hub.ts`, the neutral cloud-drive SDK `src/resource-drive.ts` (kind-agnostic tree⇄manifest+blobs), and the `od resource` CLI `src/resource-cli.ts`. The daemon speaks the real resource-hub wire protocol — no mocks. For local dev without the vela stack, run the fixture (`pnpm tools-serve start resource-hub`) and set `OD_RESOURCE_HUB_URL` (see `tools/serve/AGENTS.md`). Feature-specific sharing is a consumer layer in `src/resource-sharing/` (kind adapters + orchestrator + the `shared_resources` mapping store), built on top of the SDK — not part of it. It backs the `routes/resources/:kind/:id/{share,pull}` endpoints; design-system is implemented, plugin/skill adapters plug into `adapters.ts` next.
