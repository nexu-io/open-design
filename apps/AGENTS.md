...
- `apps/desktop`: Tauri desktop host runtime. Desktop does not guess the web port; it reads runtime status through sidecar IPC and opens the reported web URL.
- `apps/packaged`: Packaged runtime entries for Tauri sidecars and headless Linux flows. Packaged desktop paths start packaged daemon/web sidecars and delegate host behavior to `apps/desktop`.
