...
- `apps/standalone`: Shell-neutral product composition for the Web + daemon Standalone Closure. It exposes the fixed `bootloader.mjs` handoff-once entry and owns its `./protocol` and `./runtime` subpaths while composing `packages/sidecar`; it owns no Desktop IPC, windows, shell update UI, or artifact selection.
