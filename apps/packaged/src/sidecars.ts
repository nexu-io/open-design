...
const FISH_LOGIN_SHELL_PATH_COMMAND =
  `/usr/bin/printf '${LOGIN_SHELL_PATH_MARKER}%s${LOGIN_SHELL_PATH_MARKER}' (string join : -- $PATH)`;
...
function resolveMacosLoginShellArgs(shell: string): string[] | null {
  ...
  if (shellName === "fish") {
    return ["--interactive", "--login", "--command", FISH_LOGIN_SHELL_PATH_COMMAND];
  }
