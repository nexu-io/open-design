...
it("retains the gold brand clip and overlapping minimum presentation policy", () => {
  ...
  expect(appearance.splash).toMatchObject({ width: 1280, height: 900, minimumVisibleMs: 2000 });
  expect(manifest).not.toHaveProperty("splash");
  expect(manifest).not.toHaveProperty("window");
