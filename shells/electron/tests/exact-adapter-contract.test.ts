...
describe("Electron exact Shell adapter contract", () => {
  ...
  it("accepts only a digest-bound native distribution request", () => {
    ...
    expect(() => parseElectronExactDistributionRequest({ ...request, channel: "" })).toThrow(/identity/u);
    expect(() => parseElectronExactDistributionRequest({ ...request, releaseManifestFile: absolute("release-shell.json") })).toThrow(/fields/u);
  });
});
