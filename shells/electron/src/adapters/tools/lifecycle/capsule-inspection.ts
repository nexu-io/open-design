import { resolveElectronNamespacePaths, resolveElectronSessionNamespace, readElectronCapsuleSelection } from "@open-design/electron-kit";
...
export async function inspectElectronSelectedCapsule(session: ElectronDiagnosticSession, installedRoot: string) {
  ...
  const paths = resolveElectronNamespacePaths(session.baseUserDataRoot, { channel: session.channel,
    namespace: resolveElectronSessionNamespace(session.namespace, session.presentation) });
  ...
  if (physical.manifest.channel !== session.channel) throw new Error("Capsule inspection installation channel mismatch");
