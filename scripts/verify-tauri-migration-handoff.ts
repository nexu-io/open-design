...
async function writeNote(
  ...
  await writeFile(
    ...
    [
      ...
      "```",
      "",
      `That command verifies its own checksum sidecar and the archive checksum, including checksum target filenames, then imports the bundle, pushes the branch, verifies the remote head, writes \`${prBodyPath}\`, attempts \`gh workflow run ci.yml --ref ${note.branch}\` when \`GH_BIN\` or \`gh\` is available, and prints branch-head-aware report download commands.`,
      "",
      "Set `GH_BIN=<path-to-gh>` if the GitHub CLI is not named `gh` on the receiving machine. Set `TAURI_NATIVE_CI_TRIGGER=0` to skip automatic workflow dispatch. Set `TAURI_NATIVE_CI_WAIT=1` to have the command wait for matching CI reports and run the guarded M4→M5 advance after dispatch. Set `TAURI_PR_BODY_PATH=<path>` to override the generated draft PR body location.",
      "",
