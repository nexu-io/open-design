...
async function writeNote(
  ...
  await mkdir(dirname(path), { recursive: true });
  const archivePath =
    note.manifestPath == null ? "/path/to/open-design-tauri-migration-handoff.tar.gz" : `${dirname(note.manifestPath)}.tar.gz`;
  const commandScriptPath = `${archivePath}.commands.sh`;
  const prBodyPath = ".tmp/tauri-migration-pr-body.md";
  await writeFile(
    path,
    [
      ...
      "## Sender Machine",
      "",
      "Before copying to another machine, package this verified handoff directory as a tarball with archive and command checksum sidecars:",
      "",
      "```bash",
      ...
      "## Receiving Machine",
      "",
      "Preferred path when you copied the tarball plus its `.sha256`, `.commands.sh`, and `.commands.sh.sha256` sidecars:",
      "",
      "```bash",
      shellWord(commandScriptPath),
      "```",
      "",
      "Use an explicit archive argument if the sidecars were renamed or copied under another location:",
      "",
      "```bash",
      `${shellWord(commandScriptPath)} ${shellWord(archivePath)}`,
      "```",
      "",
      `That command verifies its own checksum sidecar and the archive checksum, imports the bundle, pushes the branch, verifies the remote head, writes \`${prBodyPath}\`, attempts \`gh workflow run ci.yml --ref ${note.branch}\` when \`gh\` is available, and prints branch-head-aware report download commands.`,
      "",
      "Set `TAURI_NATIVE_CI_TRIGGER=0` to skip automatic workflow dispatch. Set `TAURI_NATIVE_CI_WAIT=1` to have the command wait for matching CI reports and run the guarded M4→M5 advance after dispatch. Set `TAURI_PR_BODY_PATH=<path>` to override the generated draft PR body location.",
      "",
      ...
      "```",
      "",
      "Draft PR fallback if workflow dispatch is unavailable:",
      "",
      "```bash",
      "gh pr create --draft \\",
      "  --base main \\",
      `  --head ${shellWord(note.branch)} \\`,
      ...
      "",
      "To download, verify, record M4 evidence, and apply the guarded M5 default flip in one command, add `--advance`:",
      "",
      "```bash",
      "pnpm exec tsx scripts/download-tauri-m4-reports.ts \\",
      `  --branch ${shellWord(note.branch)} \\`,
      ...
      "```",
      "",
      "If the reports were already downloaded, advance the migration through M4 evidence and M5 defaults:",
      "",
      "```bash",
