...
async function writeNote(
  ...
  await writeFile(
    ...
    [
      ...
      "## Sender Machine",
      "",
      "Before copying to another machine, package this verified handoff directory as a tarball with checksum and command sidecars:",
      "",
      "```bash",
      ...
      "## Receiving Machine",
      "",
      "Preferred path when you copied the tarball plus its `.sha256` and `.commands.sh` sidecars:",
      "",
      "```bash",
      ...
      "```",
      "",
      `That command verifies the archive checksum, imports the bundle, pushes the branch, verifies the remote head, writes \`${prBodyPath}\`, attempts \`gh workflow run ci.yml --ref ${note.branch}\` when \`gh\` is available, and prints branch-head-aware report download commands.`,
      "",
