...
async function writeNote(
  ...
  await writeFile(
    ...
    [
      ...
      "```",
      "",
      "Use an explicit archive argument if the sidecars were copied to another location without renaming the files:",
      "",
      "```bash",
      `${shellWord(commandScriptPath)} ${shellWord(archivePath)}`,
      "```",
      "",
      `That command verifies its own checksum sidecar and the archive checksum, including checksum target filenames, then imports the bundle, pushes the branch, verifies the remote head, writes \`${prBodyPath}\`, attempts \`gh workflow run ci.yml --ref ${note.branch}\` when \`gh\` is available, and prints branch-head-aware report download commands.`,
      "",
