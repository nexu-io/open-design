...
test("package-tauri-migration-handoff creates a tarball and checksum sidecar", async (t) => {
  ...
  assert.match(result.stdout, /GH_BIN=<path-to-gh>/);
  ...
  assert.match(result.stdout, /--remote "\$\{REMOTE:-origin\}"/);
  assert.match(result.stdout, /--wait/);
  assert.match(result.stdout, /--output-dir "\$\{TAURI_M4_REPORT_DIR:-\/tmp\/open-design-tauri-m4-reports\}"/);
  ...
  assert.match(result.stdout, /--remote "\$\{REMOTE:-origin\}"/);
  assert.match(result.stdout, /--report-dir "\$\{TAURI_M4_REPORT_DIR:-\/tmp\/open-design-tauri-m4-reports\}"/);
  assert.match(result.stdout, /--wait-reports/);
  assert.match(result.stdout, /tauri-migration-status/);
  assert.match(result.stdout, new RegExp(`--handoff-dir '${escapeRegExp(handoffDir)}'`));
  assert.match(result.stdout, new RegExp(`--handoff-archive '${escapeRegExp(output)}'`));
  assert.match(result.stdout, /--report-dir "\$\{TAURI_M4_REPORT_DIR:-\/tmp\/open-design-tauri-m4-reports\}"/);
  assert.doesNotMatch(result.stdout, /--remote origin/);
  await access(output);
  const commandScript = await readFile(`${output}.commands.sh`, "utf8");
  assert.match(commandScript, /^#!\/usr\/bin\/env bash/);
  assert.match(commandScript, /generated by scripts\/package-tauri-migration-handoff\.ts/);
  ...
  assert.match(commandScript, /restore_branch=""/);
  assert.match(commandScript, /restore_branch="\$branch"/);
  assert.match(commandScript, /git checkout "\$restore_branch"/);
  ...
  assert.match(commandScript, /## Bug fix verification/);
  assert.match(commandScript, /Not a bug fix/);
  assert.match(commandScript, /## Validation/);
  assert.match(commandScript, /scripts\/download-tauri-m4-reports\.test\.ts scripts\/package-tauri-migration-handoff\.test\.ts scripts\/tauri-migration-status\.test\.ts/);
...
test("package-tauri-migration-handoff command sidecar restores the checked-out branch when push fails", async (t) => {
  ...
  await git(sourceRepo, "push", remotePath, "main:refs/heads/main");
  await git(root, "clone", "--branch", "main", remotePath, targetRepo);
  await git(targetRepo, "checkout", "-b", migrationBranch);
  await mkdir(handoffDir, { recursive: true });
  await git(sourceRepo, "bundle", "create", bundlePath, migrationBranch, "^main");
  const bundleSha256 = createHash("sha256").update(await readFile(bundlePath)).digest("hex");
  await writeFile(
  ...
  await assert.rejects(
    execFileAsync("bash", [`${output}.commands.sh`, output], {
      cwd: targetRepo,
      env: { ...process.env, REMOTE: missingRemotePath, TAURI_NATIVE_CI_TRIGGER: "0" },
      maxBuffer: 1024 * 1024,
    }),
  );
  const currentBranch = (await git(targetRepo, "symbolic-ref", "--short", "HEAD")).stdout.trim();
  assert.equal(currentBranch, migrationBranch);
});
