...
function commandScript(archivePath: string): string {
  return [
    ...
    "else",
    '  echo "shasum or sha256sum is required to verify the handoff archive" >&2',
    "  exit 1",
    "fi",
    ...
    `    printf '%s\\n' "or"`,
    `    printf '%s\\n' "  gh pr create --draft --base main --head $branch --title 'Migrate desktop runtime to Tauri' --body 'Tauri migration branch for native Windows/Linux package smoke.'"`,
    "  fi",
    ...
    `    printf '%s\\n' "  pnpm exec tsx scripts/download-tauri-m4-reports.ts --branch $branch --expected-head $expected_head --wait --output-dir /tmp/open-design-tauri-m4-reports --advance"`,
    `    if [[ "$workflow_dispatched" == "true" ]]; then`,
    `      printf '%s\\n' ""`,
